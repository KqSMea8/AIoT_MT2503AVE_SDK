...
#endif
#if 0
#ifdef __OZ105_SWCHR_SUPPORT__
static bmt_swchr_callback(void* msg_ptr);
#endif
#endif
#endif
...
void bmt_get_avg_adc_channel_voltage(DCL_ADC_CHANNEL_TYPE_ENUM ch, kal_uint32 *voltage, kal_uint32 time)
{
	...
	for(i = 0;i < (time + 2);i++)
	{
		...
		min = (min == 0)?*voltage:((min > *voltage)?*voltage:min);
		max = (max == 0)?*voltage:((max < *voltage)?*voltage:max);
		sum+=*voltage;	
	}

	if(time != 0)
		*voltage = (sum - max - min)/time;
	else
		*voltage = 0;
}
...
#ifdef MTK_SLEEP_ENABLE
kal_uint8 BMT_Query_L1SM_Handle(void)
{
	return BMTLockARM_handle;
}
#endif
...
//#ifdef __BQ25601_SWCHR_SUPPORT__
        //BATPHYS->ICHARGE =(BMT_VOL_RESULT[BMT_ADC_VISENSE] - BMT_VOL_RESULT[BMT_ADC_VBAT])*100; //Rsense = 10m oh 
//#else
...
static bmt_swchr_callback(void* msg_ptr)
{
    ...
    kal_prompt_trace(MOD_BMT,"[BMT]%s: %d, vbus:%d,chr_type:%d ", __func__, now_time, vbus_v,chr_type);

    if(bmt_read_chr_status() == CHARGER_PLUG_IN ){//charger in
	    //bmt_charging_control_handler(BMT_CHARGING_CMD_CHARGING_ENABLE, &charge_en);
	    if ( (chr_type == PW_AC_CHR) || (chr_type == PW_AC_NON_STD_CHR) || vbus_v > 4300)
	    {
		    bmt_timer_config(KAL_TICKS_1_SEC * 2); //0.5s to measure adc

		    bluewhale_set_ilimit();
		    bluewhale_set_ilimit_target(2000);

		    if (bluewhale_ilimit_fixed()){
			    bmt_enable_sleepmode(KAL_TRUE); 
			    bmt_set_timer(BMT_SWCHR_TIMER, KAL_TICKS_1_SEC * 30);
		    }
		    else{
			    bmt_set_timer(BMT_SWCHR_TIMER, KAL_TICKS_1_SEC * 2);
		    }
	    }
	    /*else if (chr_type == PW_USB_CHR){//usb
...
void BMT_exit_tiny_enable_charging(void)
{
	...
	if(bmt_read_chr_status() == CHARGER_PLUG_IN)
	{
		...
		else if(chr_type == PW_USB_CHR || chr_type == PW_USB_CHARGING_HOST_CHR)
		{
...
void  BMT_enter_tinylog_mode(module_type srcid)
{
	bmt_sendMes2BMT(srcid,MSG_ID_BMT_STOP_CHARGING_REQ);
}


void  BMT_exit_tinylog_mode(module_type srcid)
{
	bmt_sendMes2BMT(srcid,MSG_ID_BMT_ENABLE_CHARGING_REQ);
}
