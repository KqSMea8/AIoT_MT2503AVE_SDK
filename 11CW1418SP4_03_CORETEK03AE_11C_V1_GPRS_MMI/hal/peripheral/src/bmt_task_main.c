...
extern short bluewhale_charger_init(void);
extern short bluewhale_stop_charge(void);
#endif
#endif

#ifdef __BQ25601_SWCHR_SUPPORT__
extern void bq25601_charger_init(void);
#endif
...
static void bmt_chr_indication()
{
...
#if 0    
#ifdef __OZ105_SWCHR_SUPPORT__
	bluewhale_charger_init();
#endif
#endif

#ifdef __BQ25601_SWCHR_SUPPORT__
	bq25601_charger_init();
#endif
    ...
    if (bmt_read_chr_status() == CHARGER_PLUG_IN)
    {
       ...
       bmt_set_timer(BMT_SWCHR_TIMER, KAL_TICKS_1_SEC);	
#endif
#endif
       if ((previous_chr_status == bmt_chr_uninit) || (previous_chr_status != bmt_chr_in))
       {
		  	...
		  	if((chr_type == PW_AC_CHR) || (chr_type == PW_AC_NON_STD_CHR))
		  	{
		  		#if defined(__TINY_SYS__)  // when tiny system is enabled 
		  		{
					kal_bool tinylog_mode;

					tinylog_mode = csd_ap_special_mode_en();
					
					if(tinylog_mode == KAL_TRUE)
					{
				...
				#endif
#if 0
#ifndef __OZ105_SWCHR_SUPPORT__
...
#if 0
#ifdef __OZ105_SWCHR_SUPPORT__
       //bmt_stop_timer(BMT_SWCHR_TIMER);
#endif
#endif
    }
    else
...
static void bmt_usb_chr_indication(ilm_struct *current_ilm)
{
	...
	//kal_prompt_trace(MOD_BMT, "[oz115]%s, set_charging_current=%d, bmt_usb_chr_current=%d\n", __func__, set_charging_current, bmt_usb_chr_current);
	...
	if(bmt_usb_chr_current != set_charging_current)
	{
		...
		else // usb current != 0
		{
			if((PMU_CHARGE_CURRENT_0_00_MA == bmt_usb_chr_current) || (PMU_CHARGE_CURRENT_INIT == bmt_usb_chr_current))	// avoid 100mA/500mA switch to start charging twice, and usb current != default value
			{
				...
				#endif
#if 0
#if !defined(__OZ105_SWCHR_SUPPORT__)
				chr_algorithm_control_handler(BMT_CHARGING_ALGO_CMD_ENABLE, &charging_algo_enable);
#endif
#endif
...
#if defined(__TINY_IOT_FAST_BOOT__)      
...
/* under construction !*/
   ...
   while(1)
   {
	...
	case MSG_ID_TST_INJECT_STRING:
		...
		case MSG_ID_BMT_ENABLE_CHARGING_REQ:
			BMT_exit_tiny_enable_charging();
			break;
			
		case MSG_ID_BMT_STOP_CHARGING_REQ:	
			BMT_enter_tiny_disable_charging();
			break;
#endif			
       	 default:
            break;
