...
const VolToTempStruct vol_temp_slope = 
{
	ADC_TEMPV2_FACTOR,
	ADC_TEMPV3_FACTOR,
	ADC_TEMPV4_FACTOR,
...
#define MT6236_E3_ADC_FACTOR_VCHARGER 473 // 369/39*50
   ...
   else if(chann == PMU_ADC_VBATTEMP_CH_NUM)
   	{
		volt_factor = PMU_ADC_FACTOR_VBATTEMP;
   	}
#endif
   else
...
kal_uint32 AP_add_evshed_to_excepted_list(event_scheduler *es);
#endif
...
void adc_sche_timer_init(void)
{
	...
	BMT_CTRL_GET_EVENT_SCHEDULER_TIMER_INDEX adc;

	adc_sche_event_scheduler_ptr = evshed_create("ADC Sche Timer",MOD_BMT,0,255);

	bmt_handle = DclBMT_Open(DCL_BMT, FLAGS_NONE);
	adc.id = (void *)adc_sche_event_scheduler_ptr;
	DclBMT_Control(bmt_handle, BMT_CMD_GET_EVENT_SCHEDULE_TIMER_INDEX, (DCL_CTRL_DATA_T *)&adc);
	DclBMT_Close(bmt_handle);
	
	evshed_set_index(adc_sche_event_scheduler_ptr,adc.timer_index);

	#if defined(__TINY_SYS__)
	AP_add_evshed_to_excepted_list(adc_sche_event_scheduler_ptr);
	#endif
}
...
kal_int32 volt2temp(kal_int32 _volt)
{
	...
	else if(_volt>vol_temp_slope.TEMPV3)
		return ((vol_temp_slope.TEMPA23+(_volt*vol_temp_slope.TEMPM23/vol_temp_slope.TEMPSCALE))/1000);
	else if(_volt>vol_temp_slope.TEMPV4)
		return ((vol_temp_slope.TEMPA34+(_volt*vol_temp_slope.TEMPM34/vol_temp_slope.TEMPSCALE))/1000);
	else if(_volt>vol_temp_slope.TEMPV5)
		return ((vol_temp_slope.TEMPA45+(_volt*vol_temp_slope.TEMPM45/vol_temp_slope.TEMPSCALE))/1000);
	else
		return ((vol_temp_slope.TEMPA45+(_volt*vol_temp_slope.TEMPM45/vol_temp_slope.TEMPSCALE))/1000);
}	 
...
static void adc_calibration_data_check(void)
{
...
#if defined(__MT6261_PRODUCTION_IMPROVE__)
kal_uint32 efuse_getAdcEfuse(void);
kal_bool efuse_efuseBackupValid(void);
#endif //#if defined(__MT6261_PRODUCTION_IMPROVE__)
...
void adc_read_efuse_data(double *slope, double *offset)
{
	...
	*offset = 0;
...
#if defined(__MT6261_PRODUCTION_IMPROVE__)	
	if(efuse_efuseBackupValid()==KAL_FALSE)
	{
    ...
    {
	    efuseA = (efuse_getAdcEfuse() & EFUSE_ADC_A_MASK) >> EFUSE_ADC_A_SHIFT;
	    efuseB = (efuse_getAdcEfuse() & EFUSE_ADC_B_MASK) >> EFUSE_ADC_B_SHIFT;        
    }
#else
	efuseA = (DRV_ADC_Reg32(EFUSE_ADC_A) & EFUSE_ADC_A_MASK) >> EFUSE_ADC_A_SHIFT;
	efuseB = (DRV_ADC_Reg32(EFUSE_ADC_B) & EFUSE_ADC_B_MASK) >> EFUSE_ADC_B_SHIFT;
...
kal_bool adc_read_calibration_data(ADC_CALIDATA *adc_cali)
{
   ...
   if(efuse_efuseBackupValid()==KAL_FALSE)
   {
    regValue=DRV_ADC_Reg32(EFUSE_ADC_BASE);
   }
   else
   {
    regValue=efuse_getAdcEfuse();
   }
#else
    regValue=DRV_ADC_Reg32(EFUSE_ADC_BASE);
#endif //#if defined(__MT6261_PRODUCTION_IMPROVE__)   
   ...
   if (regValue & EFUSE_ADC_ENABLE)
   {
      adc_read_efuse_data(&slope, &offset);
      for (i=0;i<ADC_MAX_CHANNEL;i++) //all channel use the same calibration value
      {
         adc_cali->ADCSlope[i]  = (double)slope * (PMU_ADC_FACTOR_VBAT/50);  
         adc_cali->ADCOffset[i] = (double)offset * (PMU_ADC_FACTOR_VBAT/50);
      }
      return KAL_TRUE;
   }
...
/*
* FUNCTION
*	   adc_nvram_data_check
*
* DESCRIPTION                                                           
*   	This function is to check the calibration data from NVRAM
*
* CALLS  
*
* PARAMETERS
...
*
* PARAMETERS
*	   nvram_read_cnf_struct: adc calibration data 
*	
* RETURNS
*	   void
*
...
#endif //#if defined(DRV_ADC_CALIBRATION_EFUSE)
   {
	   adc_nvram_data_check(parm_ptr);
   }

   kal_mem_cpy( &adc_customized_para.adc_cali_param, pdu_ptr, sizeof(ADC_CALIDATA) );	
   drv_trace2(TRACE_GROUP_1, BMT_MSG_ADC_PARAM_SLOPE_OFFSET, adc_customized_para.adc_cali_param.ADCSlope[0], adc_customized_para.adc_cali_param.ADCOffset[0]);	

#endif  // #if ( (!defined(L4_NOT_PRESENT)) && (!defined(DUMMY_PROTOCOL)) )  
   adc_calibration_data_check();
}
  ...
  // ilm_struct     *ADC_CALI_ilm;
   ...
   {               
    ...
    //  }	 
	  ...
	  adc_BmtSendilm(MOD_USB,MSG_ID_BMT_USB_READ_CALI_DONE_IND,NULL,DRIVER_PS_SAP);
   }   
