...
/*
   ...
   if(port!=uart_port3 && baudrate != UART_BAUD_AUTO)	// UART_BAUD_AUTO work around
      ...
      {
         #if defined(__TINY_SYS__)
            DRV_UART_WriteReg(UART_RATE_FIX_REG(UART_BASE),0x05);    
...
void  AP_uart_enter_suspend(AP_BtSS_mode mode)

{
  UART_PORT  port;
   kal_uint32 UART_BASE;

  kal_uint8 max_port=0;
  #ifdef __UART3_SUPPORT__
    max_port=3;
  #else 
     max_port=2;
...
kal_uint32 uart_suspend_resume=0;
void  AP_uart_suspend_resume(AP_BtSS_mode mode)

{
  ...
  if(AP_BtSS_get_mode()==AP_BTSS_SUSPEND)
   ...
   for(port=0;port<max_port;port++)  
   	{
       UART_BASE = UART_BaseAddr[port];
         /*set rate-fix 0x5 ,make sure that keep the UART settings before AP SWTICH TO TINY */   
        DRV_UART_WriteReg(UART_RATE_FIX_REG(UART_BASE),0x05);  
#if defined(DRV_UART_USE_VFIFO_MODE)
      DMA_Start(UARTPort[port].Rx_DMA_Ch);
	  DMA_Start(UARTPort[port].Tx_DMA_Ch);	 
...
void UART_HISR_Internal(UART_PORT port)
{
   ...
   IIR = DRV_UART_Reg(UART_IIR(UART_BaseAddr[port]));
 	...
 	kal_prompt_trace(MOD_DRV_HISR,"UART_HISR_Internal happend,IIR is %x ",IIR);
   ...
   switch ( IIR & UART_IIR_INT_MASK )
   {
   	case UART_IIR_CTI:
			...
			if( UART_IsVfifoSetting(port, RX_VFIFO) )
           	...
           	{
           kal_prompt_trace(MOD_DRV_HISR,"timeout interrupt happend");
           		UART_RecHandler(&UARTPort[port]);
           	}
		#else
        	if( UART_IsVfifoSetting(port, RX_VFIFO) )
