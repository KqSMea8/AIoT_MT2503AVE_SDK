...
#ifdef OZ115_SWCHR
	kal_prompt_trace(MOD_BMT, "[oz115]%s\n: %d", __func__, *(DCL_UINT32 *)data);
	bluewhale_set_chg_volt(*(DCL_UINT32 *)data/1000);
#endif
...
#ifdef OZ115_SWCHR
	kal_prompt_trace(MOD_BMT, "[oz115]%s\n", __func__);
	//bluewhale_set_ilimit_target(*(DCL_UINT32 *)data/100);
...
#ifdef OZ115_SWCHR
	kal_prompt_trace(MOD_BMT, "[oz115]%s\n", __func__);
	*(DCL_UINT32 *)data = bluewhale_get_vbus_current()*100;
#endif
	return status;
}
...
static DCL_STATUS bmt_charging_enable(void *data)	
{
	...
	bmt_find_and_set_the_nearest_charger_high_voltage(PMU_VOLT_10_500000_V);
	//kal_prompt_trace(MOD_BMT, "[oz115]charger_high_voltage:%d\n", PMU_VOLT_10_500000_V);
        //hv 10.5V, if vbus > 10.5, disable VCDT_HV_VTH
	...
	{
		if (bmt_get_chr_type() == PW_USB_CHR)
			bluewhale_ac_charge();//bluewhale_usb_charge();
		else
			bluewhale_ac_charge();
	}

	if(chr_detect != DCL_TRUE||enable != KAL_TRUE)
	{    
