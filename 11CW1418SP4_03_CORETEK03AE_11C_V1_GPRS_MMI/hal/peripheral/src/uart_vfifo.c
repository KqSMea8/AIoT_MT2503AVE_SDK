...
/* under construction !*/
 ...
 //   U_SetBaudRate(port, UART_BAUD_AUTO, UARTPort[port].ownerid);
   ...
   //kal_prompt_trace(MOD_DRV_HISR,"U_GetBytes_VFIFO Length: %d,data_count: %d", Length,data_count);
   return data_count;	   
}
...
DMA_Flush(ch);
   ...
   //EnableTxIntr(UART_BaseAddr[port]);
     kal_prompt_trace(MOD_DRV_HISR,"U_PutBytes_VFIFO1 Length: %d,data_count: %d", Length,real_count);
   return real_count;
#else /*DRV_DEBUG*/
     kal_prompt_trace(MOD_DRV_HISR,"U_PutBytes_VFIFO2 Length: %d,data_count2: %d", Length,Length);
   return Length;
#endif /*DRV_DEBUG*/
}
// toy recommend: do not handle any rx data in UART_RecTimeOutHandler(), otherwise single vfifo will fail
...
void UART_RecTimeOutHandler(UART_PORT port)
{
   ...
   //kal_bool btmp; //avoid Warning 534: Ignoring return value of function 'GPTI_StartItem"
	kal_prompt_trace(MOD_DRV_HISR,"UART_RecTimeOutHandler happend");
...
void UART_TrxHandler_VFIFO(UART_PORT port)
{
   ...
   kal_uint8 ch;
	...
	kal_prompt_trace(MOD_DRV_HISR,"UART_TrxHandler_VFIFO happend");
...
/* under construction !*/
   ...
   if(UARTData->port_no >= MAX_UART_PORT_NUM)
      return;
	kal_prompt_trace(MOD_DRV_HISR,"UART_RecHandler_VFIFO happend");
