...
#if defined(__FUDIFFNET_ENABLE__) && defined(__EXT_BOOTLOADER__)
#include "fudiffnet.h"
#endif /* __FUDIFFNET_ENABLE__ && __EXT_BOOTLOADER__ */
...
#include "init.h"
#include "config_hw.h"
#if defined(__TINY_IOT_FAST_BOOT__) && defined(__INIT_LCM_IN_BOOTLOADER__)
#include "lcd_fota.h"
#endif
...
kal_bool   g_enterFUDIFFNET = KAL_FALSE;
...
#if defined (__BROM_USBDL_V2__) || defined(__FUNET_ENABLE__) || defined(__FUDIFFNET_ENABLE__)
...
#elif defined(__FUNET_ENABLE__) || defined(__FUDIFFNET_ENABLE__)
	g_usbdlRetnValue = *RETN_FLAG;
...
kal_bool bl_FUDIFFNET_Triggered()
{
	// full download
	if(g_usbdlRetnValue & RETN_FUNET_FULL_DL_EN)
    {
...
#if defined(__GPS_FW_UPDATE_SUPPORT__)
#define GPS_MARKER_FILE_NAME    (L"Z:\\gps_image.marker")
...
void bl_FUNET_GPS_update()
{
	...
	fHandle = FS_Open(GPS_MARKER_FILE_NAME, FS_CREATE_ALWAYS);
	if (fHandle >= 0)
	{
		for (i = 0; i < GPS_MARKER_NUM; i++)
		{
...
void FUNETProcedure(void)
{
    ...
    #if defined(__LCD_DRIVER_IN_BL__)
        bl_DL_InitLCD();
        //BL_ShowUpdateFirmwareProgress(50);
...
kal_bool bl_FUDIFFNET_IsGoing()
{
#if defined(__FUDIFFNET_ENABLE__)
	return bl_IsFUDIFFNETGoing();
#else
	return KAL_FALSE;
#endif
...
void FUDIFFNETProcedure(void)
{
	...
	if( bl_FUDIFFNET_IsGoing() ) {
      g_enterFUDIFFNET = KAL_TRUE;
      g_bootupDisabled = KAL_TRUE;
      g_bootupStatus = BL_BOOTUP_FAIL_REASON_FUDIFFNET_IN_PROGRESS;
	}
	
	if(g_enterFUDIFFNET || bl_FUDIFFNET_Triggered())
	{
	   BL_FUDIFFNET_ERROR_CODE status;
		   
	   WacthDogRestart();
	   status = bl_UpdateDiffMain(g_bootupDisabled);
		
	   if(status == BL_FUDIFFNET_ERROR_NONE)
	   {
		  //Impossible to get here. 
	   }
	   else if(status == BL_FUDIFFNET_ERROR_NO_DL_PACKAGE_FOUND)
	   {
		  //Continue bootup
	   }
	   else
	   {
		  dbg_print("FUDIFFNET error%d\n\r", status);
		  //while(1);    // Continue bootup
	   }
	}
