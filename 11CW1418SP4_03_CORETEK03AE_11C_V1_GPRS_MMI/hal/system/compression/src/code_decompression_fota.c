 ...
 *****************************************************************************/
kal_bool ZImageDecompressByPartitionWithBuffer
...
kal_uint32 g_workingSize;
kal_uint32 g_workingBuffer;

kal_bool SetZImageWorkingMemory(kal_uint32 workingBuffer, kal_uint32 workingSize)
{
    g_workingBuffer = workingBuffer;
    g_workingSize = workingSize;
    if ((g_workingBuffer == NULL) || (g_workingSize == 0))
    {
        return KAL_FALSE;
    }
    return KAL_TRUE;
...
kal_bool ZImageDecompressByPartition(kal_uint32 source, kal_uint32 destination,
                                 ...
                                 kal_uint32 *notifyCurrentLength, ZImageNotifyFunction notifyFunction)
{
      ...
      kal_uint8 *workingBuffer = (kal_uint8 *)g_workingBuffer;
      kal_bool decompress_result = KAL_TRUE;
      if (g_workingSize < ZIMAGE_DECOMPRESS_BUFFER_SIZE)
      {
         ASSERT(0);
      }

   decompress_result = ZImageDecompressByPartitionWithBuffer(source, destination,
                                         notifyLength, notifyTotalLength, notifyCurrentLength, notifyFunction,
                                         workingBuffer, ZIMAGE_DECOMPRESS_BUFFER_SIZE);

   kal_mem_set((void*) g_workingBuffer, 0x0, ZIMAGE_DECOMPRESS_BUFFER_SIZE);
