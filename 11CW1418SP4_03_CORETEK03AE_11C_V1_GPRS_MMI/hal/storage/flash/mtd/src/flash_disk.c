...
#define RAW_DISK1_DRIVER_DATA &FlashDiskDriveData[RAW_DISK1]
#endif
...
kal_uint32 adups_currW=0;
kal_uint32 Adups_erase_disk(void)
{
#if defined(__NOR_SUPPORT_RAW_DISK__)
   kal_uint32 result=FLASH_DISK_DONE;
   kal_uint32 i,region;
 #if (NOR_BOOTING_NOR_DISK_NUM > 1)
    NOR_FLASH_DISK_Data * D = RAW_DISK1_DRIVER_DATA;
 #else
   ...
   for(region = 0; region < D->FlashInfo.ActualRegionNum; region++)
   {
      for (i=0; i<D->FlashInfo.RegionBlocks[region]; i++)
      {
        ...
        result=eraseRawDiskBlock(D, i);
   	kal_prompt_trace(0,"Adups_erase_disk result=%d",result);
...
kal_int32 Adups_write_buffer(kal_uint8* data_ptr, kal_uint32 len)
{
#if defined(__NOR_SUPPORT_RAW_DISK__)
   kal_int32 result;

   //if(adups_currW==0)
   	//result=Adups_erase_disk();

  //if(result != FLASH_DISK_DONE )
  	//return result;

    #if (NOR_BOOTING_NOR_DISK_NUM > 1)
   result = writeRawDiskData(RAW_DISK1_DRIVER_DATA,adups_currW,data_ptr,len);
	#else
   result = writeRawDiskData(RAW_DISK0_DRIVER_DATA,adups_currW,data_ptr,len);
	#endif

   kal_prompt_trace(0,"Adups_write_buffer result=%d,address=%x",result,adups_currW);
...
kal_int32  Adups_read_buffer(kal_uint32 addr, kal_uint8* data_ptr, kal_uint32 len)
{
#if defined(__NOR_SUPPORT_RAW_DISK__)
   kal_int32 result;
 #if (NOR_BOOTING_NOR_DISK_NUM > 1)
   ...
   NOR_FLASH_DISK_Data * D = RAW_DISK0_DRIVER_DATA;
   	...
   	bl_print(0,"Adups_read_buffer disk_size=%d",D->DiskSize);
#endif
 #if (NOR_BOOTING_NOR_DISK_NUM > 1)
   result = readRawDiskData(RAW_DISK1_DRIVER_DATA,addr,data_ptr,len);
 #else
...
void Adups_erase_backup_region(void)
{
 #if defined(__NOR_SUPPORT_RAW_DISK__)
 #if (NOR_BOOTING_NOR_DISK_NUM > 1)
    NOR_FLASH_DISK_Data * D = RAW_DISK1_DRIVER_DATA;
 ...
 backupblocknum=block_size/D->FlashInfo.BlockSize[0];

 kal_prompt_trace(0,"Adups_erase_backup_region norblocksize=%d, backupblocknum=%d",block_size,backupblocknum);

 for (i=D->FlashInfo.RegionBlocks[0]-backupblocknum; i<D->FlashInfo.RegionBlocks[0]; i++)
 {
     result=eraseRawDiskBlock(D, i);
      kal_prompt_trace(0,"Adups_erase_backup_region result=%d",result);
  }
#endif
}
...
kal_int32 Adups_write_backup_region(kal_uint8* data_ptr, kal_uint32 len)
{
#if defined(__NOR_SUPPORT_RAW_DISK__)
   kal_int32 result;
   kal_uint32 adups_writebase=0;
    adups_uint32 block_size=AdupsGetBlockSize();
 ...
 adups_writebase=(D->DiskSize)-block_size;

   result = writeRawDiskData(D,adups_writebase,data_ptr,len);

   kal_prompt_trace(0,"Adups_write_backup_region result=%d,address=%x",result,adups_writebase);
   
   return result;
#endif		
}

kal_int32  Adups_read_backup_region(kal_uint8* data_ptr, kal_uint32 len)
{
#if defined(__NOR_SUPPORT_RAW_DISK__)
   kal_int32 result;
   kal_uint32 adups_readbase=0;
   adups_uint32 block_size=AdupsGetBlockSize();
 #if (NOR_BOOTING_NOR_DISK_NUM > 1)
    NOR_FLASH_DISK_Data * D = RAW_DISK1_DRIVER_DATA;
 #else
 ...
 adups_readbase=(D->DiskSize)-block_size;

   result = readRawDiskData(D,adups_readbase,data_ptr,len);

   kal_prompt_trace(0,"Adups_read_backup_region result=%d,address=%x",result,adups_readbase);
