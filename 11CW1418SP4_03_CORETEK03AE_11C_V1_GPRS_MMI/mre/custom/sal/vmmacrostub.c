...
if((mmi_gpio_get_current_battery_level()-BATTERY_LEVEL_0) < 0)
	...
	if ((mmi_gpio_get_current_battery_level()  - BATTERY_LEVEL_0) * BATTERY_LEVEL_PERCENTAGE >= BATTERY_LEVEL_PERCENTAGE*BATTERY_LEVEL_TOTAL)
		return 100;
	else
		return (mmi_gpio_get_current_battery_level()  - BATTERY_LEVEL_0) * BATTERY_LEVEL_PERCENTAGE;
#elif defined(BAT_4_LEVEL_)
...
extern void vm_cell_info_change(void);
...
static void vm_sal_cell_reg_rsp(l4c_nbr_cell_info_ind_struct *msg_ptr)
{
	...
	if(msg_ptr)
	{
		...
		if(s_vm_sal_cur_cell_info->arfcn != cell_info.nbr_meas_rslt.nbr_cells[cell_info.serv_info.nbr_meas_rslt_index].arfcn ||
...
#define VM_SUPPORT_KEY_CODE_NUM			(69 + 3)
#else
#define VM_SUPPORT_KEY_CODE_NUM			(23 + 3)
#endif
...
MMI_BOOL vm_transfer_sys_2_mre_qwerty_key( S16 mmi_key_code,  S16 mmi_key_type, U16 ucs2_value, U32 special_key_flag)
{
	...
	*/
	...
	#ifndef __MMI_WRISTBAND_SUPPORT__ /* HEART_RATE only the case one key(pwr key) configuration in HW, if NOT the case */
    if (KEY_POWER == mmi_key_code)
    {
        return MMI_FALSE;
    }
    MMI_TRACE(MMI_MRE_TRC_MOD_VMIO, TRC_MRE_VMIO_KEYCODE_S, mmi_key_code, mmi_key_type, 1111, __LINE__);
	#endif
#else
	_vm_pcb_t* pcb = (vm_pmng_get_pcb_ptr())[VM_PMNG_BACKGROUND];
#endif
    MMI_TRACE(MMI_MRE_TRC_MOD_VMIO, TRC_MRE_VMIO_KEYCODE_S, pcb, pcb ? pcb->keyExHandler : 0, 1112, __LINE__);
...
#endif
    MMI_TRACE(MMI_MRE_TRC_MOD_VMIO, TRC_MRE_VMIO_KEYCODE_S, mmi_key_code, mmi_key_type, 111, __LINE__);
    MMI_TRACE(MMI_MRE_TRC_MOD_VMIO, TRC_MRE_VMIO_KEYCODE_S, keycode, keyevent, 111, __LINE__);
	if (keyevent
#ifndef __IOT__
	&& keycode // in non IOT, the keycode must has a mapping keycode
#endif
	)
	{
        ret = vm_pmng_deal_key_event(keycode, keyevent);
        MMI_TRACE(MMI_MRE_TRC_MOD_VMIO, TRC_MRE_VMIO_KEYCODE_E1, ret, 0, 111, __LINE__);
	}
	s_vm_usc2_value = 0;
	s_vm_special_key_flag = 0;
    MMI_TRACE(MMI_MRE_TRC_MOD_VMIO, TRC_MRE_VMIO_KEYCODE_E, ret, 0, 111, __LINE__);
...
VMBOOL vm_get_chipRID(VMCHAR* pUid, VMINT32 buf_len)
{
#ifdef __MTK_TARGET__
    return (VMBOOL)SST_Get_ChipRID((kal_char*)pUid, (kal_int32)buf_len);
#else
    return VM_FALSE;
#endif
}
