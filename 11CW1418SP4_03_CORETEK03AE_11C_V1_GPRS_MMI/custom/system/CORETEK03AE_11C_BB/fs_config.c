...
//#ifdef __FS_CARD_DOWNLOAD__
    ...
    #define MAX_DRIVES 1
#elif defined(__FS_FUNET_ENABLE__)
		#define MAX_DRIVES 2
...
// Cache: Cache Size, (Minimum is 24KB: 16KB dedicate buffer + 8KB Legacy RTF buffer)
#define CACHE_SIZE            (24 FS_KB)
    #ifdef __FS_DEDICATED_BUFFER__
        #if ((CACHE_SIZE) < (24 FS_KB))
        #error File System cache size is too small, minimum requried size is 24 KB.
        #endif
    #define CACHE_BLOCKS          (((CACHE_SIZE-DEDICATED_BUFFER_SIZE)/512) + 4)  // Cache: Number of allocatable blocks
    #else
    #define CACHE_BLOCKS          (CACHE_SIZE/512) // Cache: Number of allocatable blocks (without dedicate buffer)
    #endif // __FS_DEDICATED_BUFFER__
#define CACHE_BLOCKS_OVERHEAD (CACHE_BLOCKS*(24+sizeof(FS_Buffer))) // Cache: Fixed FS Buffer overhead
#define CACHE_POOL_OVERHEAD   (184)                                 // Cache: Fixed ADM pool overhead
#define CACHE_POOL_SIZE       ((CACHE_SIZE+CACHE_POOL_OVERHEAD+CACHE_BLOCKS_OVERHEAD)) // Cache: Memory Pool Size
UINT FS_CACHE_POOL_SIZE = CACHE_POOL_SIZE;
UINT FS_CACHE_SIZE      = CACHE_SIZE;
...
FS_Device gFS_DeviceList[] = {
        ...
        {FS_DEVICE_FDISK, 0, FS_DEVICE_REMOVABLE, &FS_MsDrv, (void *)&gMSDC_Handle, NULL},
        ...
        #if defined(__FS_CARD2_SUPPORT__) // Dual T-Card Support
            #if !defined(__MSDC_NOT_SUPPORT_HOT_PLUG__)
                {FS_DEVICE_FDISK, 0, FS_DEVICE_REMOVABLE , &FS_SdDrv, (void *)&MSDC_Blk[1], NULL},
		        ...
		        {FS_DEVICE_FDISK, 0, FS_DEVICE_REMOVABLE, &FS_SdDrv, (void *)&MSDC2_Blk[0], NULL},
#endif
#endif
  ...
  /* The number of supported OTG devices is not configurable */
	   ...
	   {FS_DEVICE_FDISK, 0, FS_DEVICE_NOR_FLASH, &FileDiskNorDriver, (void *)&file_disk_info[NOR_DISK], NULL},
    #endif

    #ifdef NAND_SUPPORT
	   {FS_DEVICE_FDISK, 0, FS_DEVICE_NAND_FLASH, &FileDiskNandDriver, (void *)&file_disk_info[NAND_DISK], NULL},
    #if ( defined(__NANDFDM_MULTI_INSTANCE__) && defined(__NAND_FDM_50__) )
    #if (NAND_DISK_NUM > 1)
	   {FS_DEVICE_FDISK, 0, FS_DEVICE_NAND_FLASH, &FileDiskNandDriver1, (void *)&file_disk_info[NAND_DISK1], NULL},
    #endif
    #endif
    #endif

    #if (defined __MSDC_MS__) || (defined __MSDC_SD_MMC__) || (defined __MSDC_MSPRO__)
	   {FS_DEVICE_FDISK, 0, FS_DEVICE_REMOVABLE, &FileDiskCardDriver, (void *)&file_disk_info[CARD_DISK], NULL},
       #ifdef __FS_CARD2_SUPPORT__ // Dual T-Card Support
       {FS_DEVICE_FDISK, 0, FS_DEVICE_REMOVABLE, &FileDiskCardDriver, (void *)&file_disk_info[CARD_DISK1], NULL},
       #endif
    #endif
#endif
...
FS_Buffer          gFS_BufferTable[MAX_BUFFERS];
    ...
    #if   defined(__UBL__)
    #define __fs_cache_align(x)   __attribute__ ((aligned(x)), (section("EXT_BOOTLOADER_ZI")))
    #elif defined(__DYNAMIC_SWITCH_CACHEABILITY_)
        #if defined(__FS_CACHE_SUPPORT__) && !defined(__FS_CARD_DOWNLOAD__)
        #define __fs_cache_align(x)   __attribute__ ((aligned(x)))
        #else
    #define __fs_cache_align(x)   __attribute__ ((aligned(x)), (section("NONCACHEDZI")))
        #endif
    #else
    #define __fs_cache_align(x)   __attribute__ ((aligned(x)))
    #endif
...
// File System Cache Pool (Dynamic-Swtich-Cacheable)
#if defined(__FS_CACHE_SUPPORT__) && !defined(__FS_CARD_DOWNLOAD__)
__fs_cache_align(32) BYTE gFS_CachePoolMem[CACHE_POOL_SIZE];
#endif
...
// File System Cache Pool (Non-Cacheable)
#if !defined(__FS_CACHE_SUPPORT__) || defined(__FS_CARD_DOWNLOAD__)
