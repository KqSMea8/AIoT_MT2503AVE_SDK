...
mmi_ret mmi_charbat_wearable_device_proc(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#ifdef __DISPLAY_LEVEL_GDI_ONLY__
#if defined(__MMI_MAINLCD_240X240__) || defined(__MMI_MAINLCD_128X128__)
extern const U8 charging_logo_img[];
extern const U8 charging_done_img[];
static GDI_HANDLE charging_anim_handle = GDI_NULL_HANDLE;
#endif
void StopChargingCFScr()
{
#if defined(__MMI_MAINLCD_240X240__) || defined(__MMI_MAINLCD_128X128__)
    if (GDI_NULL_HANDLE != charging_anim_handle)
    {
        gdi_anim_stop(charging_anim_handle);
    }
#endif
}

void ShowChargingCFScr()
{
#if defined(__MMI_MAINLCD_240X240__) || defined(__MMI_MAINLCD_128X128__)
    GDI_HANDLE base_layer;
    S32 w, h;
    S32 img_size;
    img_size = sizeof(charging_logo_img) - 8;
    gdi_layer_get_base_handle(&base_layer);
    gdi_layer_push_and_set_active(base_layer);
    gdi_layer_clear(GDI_COLOR_BLACK);
    gdi_image_get_dimension_mem(GDI_IMAGE_TYPE_GIF, (U8*)charging_logo_img + 8, img_size, &w, &h);
    gdi_anim_draw_mem(LCD_WIDTH / 2 - w / 2, LCD_HEIGHT / 2 - h / 2, (U8*)charging_logo_img + 8, GDI_IMAGE_TYPE_GIF, img_size, &charging_anim_handle);
    gdi_layer_pop_and_restore_active();
    gdi_layer_blt(base_layer, 0, 0, 0, 0, 0, LCD_WIDTH, LCD_HEIGHT);
#endif
}

MMI_RET tracker_charbat_notify_evt_hdlr(mmi_event_struct *param)
{
    ...
    switch (evt->status)
    {
        case PMIC_CHARGE_COMPLETE:
        {
            ...
            S32 img_size;
            S32 w, h;
            img_size = sizeof(charging_done_img) - 8;
            StopChargingCFScr();
            gdi_layer_get_base_handle(&base_layer);            
            gdi_layer_push_and_set_active(base_layer);
            gdi_layer_clear(GDI_COLOR_BLACK);
            gdi_image_get_dimension_mem(GDI_IMAGE_TYPE_JPG, (U8*)charging_done_img + 8, img_size, &w, &h);
            gdi_image_draw_mem(LCD_WIDTH / 2 - w / 2, LCD_HEIGHT / 2 - h / 2, (U8*)charging_done_img + 8, GDI_IMAGE_TYPE_JPG, img_size);
            gdi_layer_pop_and_restore_active();
            gdi_layer_blt(base_layer, 0, 0, 0, 0, 0, LCD_WIDTH, LCD_HEIGHT);
        #endif
        }
...
mmi_ret mmi_charbat_main_evt_hdlr(mmi_event_struct* para)
{
    ...
    switch (p->status)
    {
        ...
        {
            #if defined(__BAT_7_LEVEL__)
            g_charbat_context.chargerLevel = BATTERY_LEVEL_7;
            #else			
...
void BatteryStatusIndication(U8 battery_voltage)
{
    ...
    switch (battery_voltage)
    {
...
#if defined(__BAT_7_LEVEL__)
        case BATTERY_LEVEL_4:	/* pass */
        case BATTERY_LEVEL_5:	/* pass */
        case BATTERY_LEVEL_6:	/* pass */
        case BATTERY_LEVEL_7:	/* pass */
#endif			
        {
            MMI_TRACE(MMI_COMMON_TRC_G8_DEV, TRC_MMI_GPIO_PMIC_BATTERY_LEVEL, battery_voltage);
            battery_grid_percentage = BATTERY_LEVEL_PERCENTAGE * (battery_voltage - BATTERY_LEVEL_0);
            break;
        }
        ...
        default:
        {
            ...
            battery_grid_percentage = BATTERY_LEVEL_PERCENTAGE; // 1 grid
            break;
        }
    }
