...
#if defined(__MMI_BTD_BOX_UI_STYLE__) || defined(__MMI_WEARABLE_DEVICE__)
...
#endif
#if (defined(__GPS_ADAPTOR_SUPPORT__) || defined(__NTPD_SUPPORT__))&&!defined(__MMI_NITZ__)
U8 srv_get_apply_nitz_type(void)
{
	return 0;
}
#endif
...
#if defined(__GPS_ADAPTOR_SUPPORT__) || defined(__NTPD_SUPPORT__) || defined(__MMI_NITZ__)
	if(srv_dt_set_get_gps_time_sync_mode() || srv_ntp_availability_get() || srv_get_apply_nitz_type() != 0)
...
mmi_ret mmi_display_phnset_menu_handler(cui_menu_event_struct *menu_evt, mmi_id parent_id)
 {
	 /*----------------------------------------------------------------*/
	 /* Local Variables 											   */
	 /*----------------------------------------------------------------*/
	 ...
	 if (evt->highlighted_menu_id == MENU_PHNSET_BACKLIGHT_DISP &&
...
mmi_phnset_menu_entry_hdlr phnset_menu_entry_array[]={
	...
	#ifdef __MMI_WEARABLE_DEVICE__
       ,mmi_display_phnset_menu_handler
    #else
...
#endif
#ifdef __MMI_WEARABLE_DEVICE__
	cui_menu_set_item_hidden(main_menu_cui_id, MENU_IDLE_SCR_DISP, MMI_TRUE);
	cui_menu_set_item_hidden(main_menu_cui_id, MENU_ID_PHNSET_GPIO_SETTING, MMI_TRUE);
#endif
#ifdef __NTPD_SUPPORT__
