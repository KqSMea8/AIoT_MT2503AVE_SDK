...
#endif
#ifdef __NTPD_SUPPORT__
extern void srv_ntp_auto_sync_timer_proc(void);
#endif
 ...
 *  void
 *****************************************************************************/
void mmi_phnset_set_date_time_picker_save(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    switch (header->type)
    {
		...
		case GUI_PICKER_TYPE_DATE:
        {
            ...
            g_phnset_cntx_p->settime.prev_time.nMonth = picker_year_month_and_day->month;
            g_phnset_cntx_p->settime.prev_time.nDay = picker_year_month_and_day->day;
			g_phnset_cntx_p->settime.prev_time.nYear = picker_year_month_and_day->current_year;
            break;
        }
        case GUI_PICKER_TYPE_TIME:
        {
			...
			if(PhnsetGetTimeFormat())  //add define in the header.
			{
				g_phnset_cntx_p->settime.prev_time.nHour = picker_time->hour;
				g_phnset_cntx_p->settime.prev_time.nMin = picker_time->minute;
			}
			...
			{	
				if(picker_time->am_pm == 0)
				{
					...
					{
						g_phnset_cntx_p->settime.prev_time.nHour = 0;
					}
					else
					{
						g_phnset_cntx_p->settime.prev_time.nHour = picker_time->hour;
				...
				else
				{
					...
					{
						g_phnset_cntx_p->settime.prev_time.nHour = picker_time->hour + 12;
					}
					g_phnset_cntx_p->settime.prev_time.nMin = picker_time->minute;
				}
...
void PhnsetSendSetTimeReqMessage(const MYTIME *time)
{
    ...
    #endif
    /*set_time.nSec = 0;*/
	update_datetime_mode = MMI_UPDATE_DATETIME_NONE;
...
#endif            
#ifdef __MMI_NITZ__
		if(mmi_get_update_status_string_from_nitz() == STR_GLOBAL_ON)
#endif
			{
		#ifdef __NTPD_SUPPORT__
				StartTimer(NTP_SYNC_TIMER, 1000, srv_ntp_auto_sync_timer_proc);
		#endif
			}
    }
    else
...
/* under construction !*/
	...
	{
		cui_menu_set_item_disabled(time_and_date_menu_gid, MENU9143_SETTIMEANDDATE, MMI_TRUE);
	}
	else
	{
		cui_menu_set_item_disabled(time_and_date_menu_gid, MENU9143_SETTIMEANDDATE, MMI_FALSE);
	}
#endif
