...
#include "mmi_rp_app_profiles_def.h" // SCR_KEY_TONE_VOLUME
#ifdef __MMI_BLE_PXP_SUPPORT__
#include "mmi_rp_app_pxpapp_def.h" // GRP_ID_PXPAPP_MAIN
#endif /* __MMI_BLE_PXP_SUPPORT__ */
#endif /* __MMI_WEARABLE_DEVICE__ */

#ifdef __MMI_ONE_KEY_SOS_SUPPORT__
#include "OneKeySOSGprot.h" // mmi_oksos_launch
...
void mmi_ucm_release_ind(srv_ucm_release_ind_struct *ind)
{
    ...
    mmi_ucs2ncpy((CHAR*)call_end_message, (CHAR*)ind->remote_info.disp_name, MMI_UCM_END_TEXT_LEN);
#endif /* __MMI_WEARABLE_DEVICE__ */
    ...
    #if defined(__AGPS_SUPPORT__) && defined(__SUPL_SUPPORT__) && !defined(__GPS_ADAPTOR_SUPPORT__)
...
#ifdef __MMI_WEARABLE_DEVICE__
    if (IsMyTimerExist(UCM_VOL_BAR_SHOW_TIMER))
    {
        StopTimer(UCM_VOL_BAR_SHOW_TIMER);
        SetInCallVolInfoContentShow(MMI_FALSE);
        ShowInCallVolBar(MDI_AUD_VOL_NUM, MDI_AUD_VOL_NUM, MMI_FALSE);
    }
#endif /* __MMI_WEARABLE_DEVICE__ */
...
void mmi_ucm_enter_processing_screen(MMI_ID SG_id)
{
    ...
    #endif /* __MMI_VIDEO_TELEPHONY__ */
    #ifdef __MMI_WEARABLE_DEVICE__
        else if (mmi_frm_group_is_present(GRP_ID_UCM_HW_CNF))
        {
            inserted_SG_id = GRP_ID_UCM_HW_CNF;
        }
    #endif /* __MMI_WEARABLE_DEVICE__ */
        else
        {
...
void mmi_ucm_go_back_screen_check(void)
{
    ...
    if (MMI_FALSE == srv_ucm_is_pending_action())
    {
        ...
        else
        {
            ...
            #endif /* __MMI_BT_CALL_SUPPORT__ */
            #ifdef __MMI_WEARABLE_DEVICE__
                mmi_frm_group_close(GRP_ID_UCM_HW_CNF);
            #endif /* __MMI_WEARABLE_DEVICE__ */
            #if defined(__MMI_BACKGROUND_CALL__) && !defined(__MMI_BACKGROUND_CALL_WITH_NCENTER__)
                mmi_frm_group_close(GRP_ID_UCM_INT_MARKER);
            #endif /* defined(__MMI_BACKGROUND_CALL__) && !defined(__MMI_BACKGROUND_CALL_WITH_NCENTER__) */
...
static void mmi_ucm_call_select_main_menu_entry(MMI_ID cui_gid)
{
    ...
    if (srv_ucm_bt_is_any_call() && !mmi_ucm_get_hide_state_bt())
    {
        cui_menu_set_item_disabled(cui_gid, MENU_ID_UCM_FROM_LOCAL_CALL, MMI_TRUE);
    }
    else if (srv_sim_ctrl_is_inserted(MMI_SIM1) && srv_mode_switch_is_network_service_available())
    {
    ...
    if (srv_bt_cm_is_profile_connected(SRV_BT_CM_HF_CONNECTION))
    {
        ...
        cui_menu_set_item_hidden(cui_gid, MENU_ID_UCM_FROM_BT_CALL, MMI_TRUE);
 ...
 *  void
 *****************************************************************************/
void mmi_ucm_vol_set(U8 volume)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_prompt_trace(MOD_MMI, "[UCM] mmi_ucm_vol_set, volume:%d", volume);

    if (mmi_vol_get_speech_vol() != volume)
    {
 ...
 *  void
 *****************************************************************************/
void mmi_ucm_vol_pen_down_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (mmi_frm_group_get_active_id() == GRP_ID_UCM_INCALL && mmi_frm_scrn_get_active_id() == SCR_ID_UCM_IN_CALL)
    {
        g_ucm_p->vol_info.valid_down = MMI_TRUE;
        g_ucm_p->vol_info.pen_down_vol = mmi_vol_get_speech_vol();
    }
    else if (g_ucm_p->vol_info.valid_down)
    {
        memset(&g_ucm_p->vol_info, 0, sizeof(mmi_ucm_vol_change_info_struct));
        kal_prompt_trace(MOD_MMI, "[UCM] mmi_ucm_pre_touch_hdlr, valid_down:%d", g_ucm_p->vol_info.valid_down);
    }
}

/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_vol_pen_move_hdlr
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  void
 *****************************************************************************/
void mmi_ucm_vol_pen_move_hdlr(mmi_touch_pos_struct* down_pos, mmi_touch_pos_struct* currpos)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(g_ucm_p->vol_info.valid_down)
    {
        if(mmi_frm_group_get_active_id() == GRP_ID_UCM_INCALL
         && mmi_frm_scrn_get_active_id() == SCR_ID_UCM_IN_CALL)
        {
            mmi_ucm_vol_ret_enum vol_ret;

            // already show
            if (g_ucm_p->vol_info.valid_move)
            {
                U8 diff_level, UI_level;
                vol_ret = mmi_ucm_vol_is_valid_swipe(down_pos, currpos, &diff_level);

                if (g_ucm_p->vol_info.pen_down_vol >= 1)
                {
                    UI_level = g_ucm_p->vol_info.pen_down_vol-1;
                }
                else
                {
                    UI_level = 0;
                ...
                if (vol_ret > MMI_UCM_VOL_RET_NONE)
                {
                    if (MMI_UCM_VOL_RET_INCREASE == vol_ret)
                    {
                        if (UI_level < MMI_UCM_VOL_MAX_UI_VALUE)
                        {
                            if ((UI_level + diff_level) < MMI_UCM_VOL_MAX_UI_VALUE)
                            {
                                UI_level += diff_level;
                                mmi_ucm_vol_set(g_ucm_p->vol_info.pen_down_vol+diff_level);
                            }
                            else
                            {
                                UI_level = MMI_UCM_VOL_MAX_UI_VALUE;
                                mmi_ucm_vol_set(MMI_UCM_VOL_MAX);
                            }
                            UpdateInCallVolBar(MMI_UCM_VOL_MAX_UI_VALUE, UI_level);
                        }
                        else if (MMI_UCM_VOL_MAX_UI_VALUE == UI_level)
                        {
                            UpdateInCallVolBar(MMI_UCM_VOL_MAX_UI_VALUE, UI_level);
                        }
                    }
                    else if (MMI_UCM_VOL_RET_DECREASE == vol_ret)
                    {
                        if (UI_level > MMI_UCM_VOL_MIN_UI_VALUE)
                        {
                            ...
                            {
                                UI_level = MMI_UCM_VOL_MIN_UI_VALUE;
                                mmi_ucm_vol_set(MMI_UCM_VOL_MIN);
                            }
                            UpdateInCallVolBar(MMI_UCM_VOL_MAX_UI_VALUE, UI_level);
                        }
                        else if (MMI_UCM_VOL_MIN_UI_VALUE == UI_level)
                        {
                            UpdateInCallVolBar(MMI_UCM_VOL_MAX_UI_VALUE, UI_level);
                        }
                    }
            ...
            {
                vol_ret = mmi_ucm_vol_is_valid_swipe(down_pos, currpos, NULL);

                if (vol_ret > MMI_UCM_VOL_RET_NONE)
                {
                    U8 UI_level = g_ucm_p->vol_info.pen_down_vol - 1;

                    g_ucm_p->vol_info.valid_move = MMI_TRUE;
                    // TODO: show volume bar fisrt.
                    ShowInCallVolBar(MMI_UCM_VOL_MAX_UI_VALUE, UI_level, MMI_TRUE);
                }
            }
        }                 
        StartTimer(UCM_VOL_BAR_SHOW_TIMER, MMI_UCM_VOL_HIDE_BAR_TIMER, mmi_ucm_vol_timeout_hdlr);
    }
}

/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_vol_pen_up_hdlr
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  void
 *****************************************************************************/
void mmi_ucm_vol_pen_up_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  MMI_RET_OK
 *****************************************************************************/
void mmi_ucm_vol_str_timeout_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  MMI_RET_OK
 *****************************************************************************/
void mmi_ucm_vol_timeout_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  MMI_RET_OK
 *****************************************************************************/
U8 mmi_ucm_vol_get_level(S32 diff_pos_y)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (diff_pos_y >= 0)
    {
        level = diff_pos_y / MMI_UCM_VOL_PER_LEVEL_Y;
    }

    kal_prompt_trace(MOD_MMI, "[UCM] mmi_ucm_vol_get_level, level:%d", level);
    return level;
}

/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_vol_is_valid_swipe
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 * RETURNS
 *  mmi_ucm_vol_ret_enum
 *****************************************************************************/
mmi_ucm_vol_ret_enum mmi_ucm_vol_is_valid_swipe(mmi_touch_pos_struct* down_pos, mmi_touch_pos_struct* up_pos, U8* level)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_ucm_vol_ret_enum result = MMI_UCM_VOL_RET_NONE;
    U8 diff_level = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (down_pos && up_pos)
    {
        ...
        if (up_pos->pos.y >= down_pos->pos.y)
        {
            ...
            if (diff > MMI_UCM_VOL_SWIPE_THRESHOLD_Y || level != NULL)
            {
                result = MMI_UCM_VOL_RET_DECREASE;
            }
        }
        else
        {
            ...
            if (diff > MMI_UCM_VOL_SWIPE_THRESHOLD_Y || level != NULL)
            {
                result = MMI_UCM_VOL_RET_INCREASE;
            }
        }

        if (result > MMI_UCM_VOL_RET_NONE)
        {
            // save update level
            diff_level = mmi_ucm_vol_get_level(diff);
            ...
            if (diff >= MMI_UCM_VOL_SWIPE_THRESHOLD_X)
            {
                result = MMI_UCM_VOL_RET_NONE;
            }
        }
    }

    if (level != NULL && result > MMI_UCM_VOL_RET_NONE)
    {
        *level = diff_level;
    }

    kal_prompt_trace(MOD_MMI, "[UCM] pen[0].x:%d, pen[1].x:%d, pen[0].y:%d, pen[1].y:%d", down_pos->pos.x, up_pos->pos.x, down_pos->pos.y, up_pos->pos.y);
    kal_prompt_trace(MOD_MMI, "[UCM] mmi_ucm_vol_is_valid_swipe, result:%d, diff_level:%d", result, diff_level);
    return result;
}
 ...
 * 
 *****************************************************************************/
static void mmi_ucm_hw_end_call(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (mmi_frm_group_is_present(GRP_ID_UCM_HW_CNF))
    {
        mmi_frm_group_close(GRP_ID_UCM_HW_CNF);
    }
    // mmi_ucm_enter_processing_screen(GRP_ID_UCM_HW_CNF);
}


/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_move_close_hd_sg_proc
 * DESCRIPTION
 * 
 * PARAMETERS
 * 
 ...
 * 
 *****************************************************************************/
mmi_ret mmi_ucm_move_close_hd_sg_proc(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_exit_hw_key_cnf_screen
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  
 *****************************************************************************/
void mmi_ucm_exit_hw_cnf_screen(mmi_scrn_essential_struct* scr_info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_entry_hw_key_cnf_screen
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  
 *****************************************************************************/
void mmi_ucm_entry_hw_cnf_screen(mmi_scrn_essential_struct* scr_info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (!mmi_frm_scrn_enter(
                    scr_info->group_id,
                    SCR_ID_UCM_HW_CNF,
                    (FuncPtr)mmi_ucm_exit_hw_cnf_screen,
                    (FuncPtr)mmi_ucm_entry_hw_cnf_screen,
                    MMI_FRM_FULL_SCRN))
    {
        return;
    }

    if (srv_ucm_query_call_count(SRV_UCM_INCOMING_STATE,SRV_UCM_CALL_TYPE_NO_DATA_CSD,NULL) > 0)
    {
        main_str_id = STR_ID_UCM_BT_REJECT_THE_CALL;
    }
    else
    {
        main_str_id = STR_ID_UCM_BT_END_THE_CALL;
    }

    if (!mmi_is_redrawing_bk_screens())
    {
        ClearKeyEvents();
        if(mmi_frm_group_is_present(GRP_ID_UCM_MT))
        {
    ...
    wgui_showcategory6333_screen(NULL,
                                ...
                                (UI_string_type)GetString(main_str_id),
                                NULL);

    SetLeftSoftkeyFunction(mmi_ucm_hw_end_call, KEY_EVENT_UP);
    SetCenterSoftkeyFunction(mmi_ucm_hw_end_call, KEY_EVENT_UP);
 ...
 *  
 *****************************************************************************/
void mmi_ucm_enter_hw_key_cnf(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    MMI_BOOL is_dising = mmi_ucm_is_any_disconnecting_call();
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_prompt_trace(MOD_MMI, "[UCM] mmi_ucm_enter_hw_key_cnf, disconnecting:%d", is_dising);
    ...
    {
        srv_bt_cm_connect_default_headset(NULL);
        return;
    }
#endif /* __MMI_HFP_SUPPORT__ */

    if (srv_ucm_is_any_call() && !is_dising)
    {
        if (mmi_frm_group_is_present(GRP_ID_UCM_HW_CNF))
        {
            mmi_frm_group_close(GRP_ID_UCM_HW_CNF);
        ...
        mmi_ucm_sg_create(GRP_ID_ROOT, GRP_ID_UCM_HW_CNF);
        ...
        mmi_frm_scrn_first_enter(
            GRP_ID_UCM_HW_CNF, SCR_ID_UCM_HW_CNF, 
            (FuncPtr)mmi_ucm_entry_hw_cnf_screen, 
            (void*)NULL);
    }
}
...
void mmi_ucm_lsk_mute_action(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_HFP_SUPPORT__
    if (srv_bt_cm_get_bqb_test_flag() && srv_bt_cm_is_profile_connected(SRV_BT_CM_HFP_CONNECTION))
    {
    ...
    {
        srv_speech_unmute_phone();
        SetInCallMuteState(0);
        // lsk_str = STR_GLOBAL_MUTE;
    }
    else
    {
        srv_speech_mute_phone();
        SetInCallMuteState(1);
        // lsk_str = STR_ID_UCM_UNMUTE;
    }
...
#ifdef __MMI_WEARABLE_DEVICE__
    SetDialingKeypadCallHandler_SW(mmi_ucm_make_voice_call_from_dtmf_editor_icon);
    SetKeyDownHandler(mmi_ucm_enter_hw_key_cnf, KEY_POWER);
#endif /* __MMI_WEARABLE_DEVICE__ */
...
void mmi_ucm_entry_incoming_call(mmi_scrn_essential_struct* scr_info)
{
    ...
    ShowCategory17Screen_ext(
            ...
            rsk_str,
            notification_str_id,
            (U8*)call_info->remote_info.disp_name,
    ...
    if (SRV_UCM_RESULT_OK == mmi_ucm_answer_option(MMI_UCM_PERMIT_ONLY))
    {
        SetInComingAccepthandler(mmi_ucm_incoming_call_sendkey);
    }
    else if (SRV_UCM_RESULT_OK == mmi_ucm_replace_option(MMI_UCM_PERMIT_ONLY))
    {
        SetInComingAccepthandler(mmi_ucm_replace);
    }
    else
    {
        SetInComingAccepthandler(mmi_ucm_incoming_call_sendkey);
    ...
    SetInComingRejecthandler(mmi_ucm_incoming_call_endkey);
    SetKeyDownHandler(mmi_ucm_enter_hw_key_cnf, KEY_POWER);
    ...
    // add by chenjing auto answer call
	gui_start_timer(200, mmi_ucm_incoming_call_sendkey);
    OslMfree(call_info);
}
...
void mmi_ucm_entry_outgoing_call(mmi_scrn_essential_struct* scr_info)
{
    ...
    ShowCategory17Screen_ext(
            ...
            STR_ID_UCM_CALLING,
            (PU8)display_name,
            IMG_ID_UCM_BT_OUTGOING,
            WGUI_CATE_MOMT_OUTGOING_CALL,
            guiBuffer);

    SetKeyDownHandler(mmi_ucm_enter_hw_key_cnf, KEY_POWER);
    SetOutGoingEndhandler(mmi_ucm_outgoing_call_endkey);
#endif /* __MMI_WEARABLE_DEVICE__ */
    SetKeyDownHandler(mmi_ucm_set_sidekey, KEY_VOL_UP);
    SetKeyDownHandler(mmi_ucm_set_sidekey, KEY_VOL_DOWN);
#ifndef __MMI_WEARABLE_DEVICE__
...
void mmi_ucm_enter_in_call(void)
{
    ...
    else
    {
        ...
        StartTimer(UCM_VOL_STR_SHOW_TIMER, MMI_UCM_VOL_HIDE_STR_TIMER, mmi_ucm_vol_str_timeout_hdlr);
    }
#endif /* __MMI_WEARABLE_DEVICE__ */
...
void mmi_ucm_entry_in_call(mmi_scrn_essential_struct* scr_info)
{
...
#endif /* __MMI_FTE_SUPPORT__ */
#ifdef __MMI_WEARABLE_DEVICE__
    wgui_cat9023_call_info_struct cm_call_info[2];
    S32 num_calls;
#endif /* __MMI_WEARABLE_DEVICE__ */
#ifdef __MMI_UCM_PLUTO_BW__
  ...
  if (group_info.call_state == SRV_UCM_ACTIVE_STATE)
...
#else /* __MMI_WEARABLE_DEVICE__ */
    num_group = srv_ucm_query_group_count(SRV_UCM_CALL_STATE_CONNECTED, SRV_UCM_CALL_TYPE_NO_CSD, MMI_FALSE, group_index_array);

    memset(cm_call_info, 0, 2*sizeof(wgui_cat9023_call_info_struct));
    cm_call_info[0].call_flag = WGUI_CAT9023_NO_CALL;
    cm_call_info[1].call_flag = WGUI_CAT9023_NO_CALL;
    ...
    for (i = 0; i < num_group; i++)
    {
        ...
        /* get state and call type */
        calls_of_group = srv_ucm_query_group_data(group_index_array[i], &group_info);
        if (calls_of_group <= 0)
        {
             MMI_ASSERT(0);
        }
        ...
        {
            srv_ucm_index_info_struct call_idx;

            if (group_info.call_state == SRV_UCM_ACTIVE_STATE)
            {
                cm_call_info[0].call_flag = WGUI_CAT9023_ACTIVE_CALL;
                if (calls_of_group > 1)
                {
                    cm_call_info[0].active_caller = (PU8)get_string(STR_ID_UCM_BT_CONF_CALL);
                }
                else if (calls_of_group == 1)
                {
                    call_idx.group_index = group_index_array[i];
                    call_idx.call_index = 0; // because there is only one call in this group.
                    srv_ucm_query_call_data(call_idx, call_info);
                    cm_call_info[0].active_caller = (PU8) call_info->remote_info.disp_name;
                    cm_call_info[0].active_call_duration = &call_info->start_time;
                }
            }
            else if (group_info.call_state == SRV_UCM_HOLD_STATE)
            {
                cm_call_info[1].call_flag = WGUI_CAT9023_HELD_CALL;
                cm_call_info[1].held_call_icon = IMG_ID_UCM_BT_CALL_STATE_HOLD;
                cm_call_info[1].held_call_str = (PU8) get_string(STR_ID_UCM_BT_STATE_HELD);

                if (calls_of_group > 1)
                {
                    cm_call_info[1].held_caller = (PU8)get_string(STR_ID_UCM_BT_CONF_CALL);
                }
                else if (calls_of_group == 1)
                {
                    call_idx.call_index = group_index_array[i];
                    call_idx.call_index = 0; // because there is only one call in this group.
                    srv_ucm_query_call_data(call_idx, call_info_hold);
                    cm_call_info[1].held_caller = (PU8) call_info_hold->remote_info.disp_name;
                    cm_call_info[1].active_call_duration = &call_info_hold->start_time;
                }
            }
        }
    }

    if (num_group == 1 && group_info.call_state == SRV_UCM_HOLD_STATE)
    {
        SetInCallMuteState(2);
    }
    else
    {
        if (srv_speech_is_phone_mute())
    ...
    if (num_group == 1 && group_info.call_state == SRV_UCM_HOLD_STATE)
    {
        SetInCallMutehandler(mmi_ucm_retrieve_action);
    }
    else
    {
        SetInCallMutehandler(mmi_ucm_lsk_mute_action);
    }

    SetInCallEndhandler(mmi_ucm_in_call_endkey);

    if (num_group > 1)
    {
        SetInCallSwaphandler(mmi_ucm_swap_action);
    }
...
void mmi_ucm_entry_call_end(mmi_scrn_essential_struct* scr_info)
{
    ...
    ShowCategory17Screen_ext(
            ...
            (U8*)call_end_p->disp_msg,
            IMG_ID_UCM_BT_IN_CALL,
            WGUI_CATE_MOMT_END_CALL,
            NULL);

    if (srv_ucm_query_call_count(SRV_UCM_CALL_STATE_ALL, SRV_UCM_CALL_TYPE_NO_DATA_CSD, NULL) > 0)
    {
        mmi_frm_set_key_handler(mmi_frm_scrn_close_active_id, KEY_POWER, KEY_EVENT_DOWN);
    }
#endif /* __MMI_WEARABLE_DEVICE__ */
...
#endif /* __MMI_VIDEO_TELEPHONY__ */
#ifdef __MMI_WEARABLE_DEVICE__
    mmi_frm_scrn_close(GRP_ID_UCM_HW_CNF, SCR_ID_UCM_PROCESSING);
#endif /* __MMI_WEARABLE_DEVICE__ */
}
...
mmi_ret mmi_ucm_bk_call_key_proc(mmi_event_struct *evt)
{
    ...
    if (srv_ucm_query_call_count(SRV_UCM_CALL_STATE_ALL, SRV_UCM_CALL_TYPE_ALL, NULL) > 0)
    {
        ...
        if (MMI_FALSE == mmi_ucm_is_ucm_sg(curr_act_sg_id))
        {
         ...
         #ifdef __MMI_WEARABLE_DEVICE__
            else if (key_code == KEY_POWER && key_type == KEY_EVENT_DOWN)
            {
                mmi_ucm_enter_hw_key_cnf();
                return MMI_RET_STOP_KEY_HANDLE;
            }
...
mmi_ret mmi_ucm_mo_sg_proc(mmi_event_struct *evt)
{
    ...
    {
    #ifdef __MMI_WEARABLE_DEVICE__
        case EVT_ID_TOUCHSCREEN_MOVE_LEFT:
...
mmi_ret mmi_ucm_incall_sg_proc(mmi_event_struct *evt)
{
    ...
    switch(evt->evt_id)
    {
        ...
        case EVT_ID_TOUCHSCREEN_MOVE_LEFT:
        {
            if (SCR_ID_UCM_DTMF_EDITOR != mmi_frm_scrn_get_active_id()
           && !mmi_frm_scrn_is_present(GRP_ID_UCM_INCALL, SCR_ID_UCM_DTMF_EDITOR,MMI_FRM_NODE_ALL_FLAG))
            {
                mmi_ucm_pre_entry_dtmf_editor();
                return MMI_RET_ERR;
        ...
        case EVT_ID_TOUCHSCREEN_MOVE_RIGHT:
        {
            if (srv_bt_cm_get_bqb_test_flag() && SCR_KEY_TONE_VOLUME != mmi_frm_scrn_get_active_id()
              && SCR_ID_UCM_DTMF_EDITOR != mmi_frm_scrn_get_active_id())
            {
                SetDefaultVolumeKeyHandlers();
                EntryScrSetKeyPadVolumeLevel();
                return MMI_RET_ERR;
            }
            else if (SCR_ID_UCM_DTMF_EDITOR != mmi_frm_scrn_get_active_id()
              && !mmi_frm_scrn_is_present(GRP_ID_UCM_INCALL, SCR_ID_UCM_DTMF_EDITOR,MMI_FRM_NODE_ALL_FLAG))
            {
                return MMI_RET_ERR;
            }
...
MMI_ID mmi_ucm_sg_create(MMI_ID parent_id, MMI_ID group_id)
{
    ...
    if (MMI_FALSE == mmi_frm_group_is_present(group_id))
    {
        switch (group_id)
        {
            ...
            break;
        #ifdef __MMI_WEARABLE_DEVICE__
            case GRP_ID_UCM_HW_CNF:
            {
                flag = MMI_FRM_NODE_SMART_CLOSE_FLAG;
                proc = mmi_ucm_move_close_hd_sg_proc;
            }
            break;
        #endif /* __MMI_WEARABLE_DEVICE__ */
        #endif /* __MMI_BT_CALL_SUPPORT__ */
...
void mmi_ucm_delete_up_to_sg_id(MMI_ID sg_id, MMI_BOOL inc_this_sg, MMI_BOOL inc_active_sg)
{
    ...
    if (MMI_RET_OK ==mmi_frm_group_query_children_list(GRP_ID_ROOT, sg_table, &total_sg_num))
    {
        ...
        for (i = start_index; i < total_sg_num; i++)
        {
            ...
            {
            #if defined(__MMI_WEARABLE_DEVICE__) && defined(__MMI_BLE_PXP_SUPPORT__)
                if (GRP_ID_PXPAPP_MAIN != sg_table[i])
            #endif /* __MMI_WEARABLE_DEVICE__ && __MMI_BLE_FMP_SUPPORT__ */
                {
