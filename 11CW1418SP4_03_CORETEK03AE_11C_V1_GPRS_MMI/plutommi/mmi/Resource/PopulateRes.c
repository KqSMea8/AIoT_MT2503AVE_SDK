...
#if defined(__FOTA_ENABLE__) || defined(__CARD_DOWNLOAD__) || defined(__ZIMAGE_SUPPORT__) || defined(__FAST_LOGO__) || defined(__FUNET_ENABLE__) || defined(__FUDIFFNET_ENABLE__)||defined(__ADUPS_FOTA_DIFF__) 
#include "lcd_sw_rnd.h"
#include "ABMLoader.h"
#include "gui_resource_type.h"
#endif /* defined(__FOTA_ENABLE__) || defined(__CARD_DOWNLOAD__) || defined(__ZIMAGE_SUPPORT__) || defined(__FAST_LOGO__) || defined(__FUNET_ENABLE__) || defined(__FUDIFFNET_ENABLE__) */
...
#if defined(__CARD_DOWNLOAD__)||defined(__ADUPS_FOTA_DIFF__)
...
static void PopulateChargerData()
{
#if defined(__MMI_MAINLCD_240X240__) || defined(__MMI_MAINLCD_128X128__)
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    FILE *fp = NULL, *fp_in = NULL;
    char filename_in[] = CUST_IMG_PATH"\\MainLCD\\UIElement\\Charger\\Charging.gif";
    char output_name[] = "charging_logo_img";

    char filename_in2[] = CUST_IMG_PATH"\\MainLCD\\UIElement\\Charger\\Charge_Done.jpg";
    char output_name2[] = "charging_done_img";
    
    char filename_out[] = "..\\..\\Customer\\CustResource\\CustChargingImgData.h";
    char buff[256];
    int ret, n_frames, i, count;
    U32 width, height, file_size;
    U8 image_identifier;
    U32 width_height;
    int out_index = 0;
    ...
    do
    {
        ...
        n_frames = Image_Get_Dimensions(filename_in, &width, &height);
        width_height = ((width & 0XFFF) << 12) | (height & 0XFFF);
        if (n_frames <= 0)
        {
            RESPOP_LOG_V("\n[CHARGING] Get dimensions failed:\t%s", filename_in);
            break;
        }
        ...
        if (fp_in == NULL)
        {
            RESPOP_LOG_V("[CHARGING] Read %s failed !!\n", filename_in);
            break;
        }
        ...
        if (fp == NULL)
        {
            RESPOP_LOG_V("[CHARGING] Output file failed !!");
            break;
        }
    ...
    do
    {
        ...
        n_frames = Image_Get_Dimensions(filename_in2, &width, &height);
        width_height = ((width & 0XFFF) << 12) | (height & 0XFFF);
        if (n_frames <= 0)
        {
            RESPOP_LOG_V("\n[CHARGING] Get dimensions failed:\t%s", filename_in2);
            break;
        }
        
        /* decode image */       
        fp_in = fopen(filename_in2, "rb");        
        if (fp_in == NULL)
        {
            RESPOP_LOG_V("[CHARGING] Read %s failed !!\n", filename_in2);
            break;
...
static void PopulatePowerOnOffAnimationData()
{
#if defined(__MMI_MAINLCD_240X240__) || defined(__MMI_MAINLCD_128X128__)
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    FILE *fp = NULL, *fp_in = NULL;
    char filename_in[] = CUST_IMG_PATH"\\MainLCD\\Active\\Poweronoff\\POON.gif";
    char output_name[] = "power_on_img";
    char filename_out[] = "..\\..\\Customer\\CustResource\\CustPowerOnImgData.h";

    char filename_in2[] = CUST_IMG_PATH"\\MainLCD\\Active\\Poweronoff\\POOFF.gif";
    char output_name2[] = "power_off_img";
    char filename_out2[] = "..\\..\\Customer\\CustResource\\CustPowerOffImgData.h";
    ...
    do
    {
        ...
        if (n_frames <= 0)
        {
            RESPOP_LOG_V("\n[POWERON] Get dimensions failed:\t%s", filename_in);
            break;
        }
        ...
        if (fp_in == NULL)
        {
            RESPOP_LOG_V("[POWERON] Read %s failed !!\n", filename_in);
            break;
        }
        ...
        if (fp == NULL)
        {
            RESPOP_LOG_V("[POWERON] Output file failed !!");
            break;
        }
    ...
    do
    {
        ...
        if (n_frames <= 0)
        {
            RESPOP_LOG_V("\n[POWEROFF] Get dimensions failed:\t%s", filename_in2);
            break;
        }
        ...
        if (fp_in == NULL)
        {
            RESPOP_LOG_V("[POWEROFF] Read %s failed !!\n", filename_in2);
            break;
        }
        ...
        /* output data */
        fp = fopen(filename_out2, "w+");
        if (fp == NULL)
        {
            RESPOP_LOG_V("[POWEROFF] Output file failed !!");
            break;
        }
...
//    PopulateMainMenuRes();
#if defined (__DISPLAY_LEVEL_GDI_ONLY__) && !defined(__POPULATE_NO_IMAGE_DATA__)
    PopulateChargerData();
    PopulatePowerOnOffAnimationData();
...
#if defined (__BG_IMAGE__) || defined (__FUNET_ENABLE__) || defined (__FUDIFFNET_ENABLE__)||defined(__ADUPS_FOTA_DIFF__)
    RESPOP_LOG_V("Populating BL Resource");	
    PopulateBLData();
#endif /* defined (__BG_IMAGE__)  */
...
#if defined (__BG_IMAGE__) || defined(__FUNET_ENABLE__) || defined (__FUDIFFNET_ENABLE__) || defined (__DISPLAY_LEVEL_GDI_ONLY__)||defined(__ADUPS_FOTA_DIFF__)
/*****************************************************************************
 * FUNCTION
 *  PopulateFOTAImage
 * DESCRIPTION
