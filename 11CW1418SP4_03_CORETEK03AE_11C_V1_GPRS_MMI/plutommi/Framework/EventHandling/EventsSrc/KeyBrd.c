...
#ifdef __CKPD_VIRTUAL_KEY__
#define NEW_DEVICE_KEY_BEGIN	150
#define NEW_DEVICE_KEY_END		199
#endif
...
#define DELAYED_POWER_OFF_TIME_COUNT		5
#endif

#ifdef __MMI_WRISTBAND_SUPPORT__
#define CONTINUE_CLICK_THRESHOLD				300
#define SINGLE_CLICK_THRESHOLD					1000
#endif
/***************************************************************************** 
* Typedef 
*****************************************************************************/
...
typedef struct
{
	key_click_status_enum click_status;
	U32 click_up_time;
	kal_bool has_double_click;
}key_click_info_struct;
#endif
...
#ifdef __MMI_WEARABLE_DEVICE__
static U32 power_key_repeat_count;
#endif

#ifdef __MMI_WRISTBAND_SUPPORT__
static key_click_info_struct g_key_click;
static U32 current_key_down_time;
static U32 current_key_up_time;
static MMI_BOOL is_long_pressed = MMI_FALSE;
#endif
/*****************************************************************************
* Global Variable
*****************************************************************************/
...
void mmi_frm_init_key_event(void)
{
...
#ifdef __MMI_WRISTBAND_SUPPORT__
    memset(&g_key_click, 0, sizeof(g_key_click));
		current_key_down_time = 0;
		current_key_up_time = 0;
#endif
    ...
    mmi_frm_qwerty_init();
#endif /* __MMI_QWERTY_KEYPAD_SUPPORT__ */
    mmi_frm_set_single_protocol_event_handler(MSG_ID_MMI_EQ_KEYPAD_DETECT_IND, (PsIntFuncPtr)mmi_frm_key_handle);
}
...
#ifdef __MMI_WEARABLE_DEVICE__
static void delayed_power_off(void)
{
...
static mmi_ret mmi_sys_key_proc(mmi_frm_key_evt_struct *evt)
{
    ...
    {
        #ifdef __MMI_WEARABLE_DEVICE__
        if ((evt->key_code == KEY_END || evt->key_code == KEY_POWER) && evt->key_type == KEY_EVENT_DOWN)
	...
	#if defined(__MMI_TAB_BAR_SUPPORT_TAB_PAGE__)
    if (cui_tab_is_in_tab_screen() == MMI_TRUE)
    {
    ...
    {
    	  if (evt->key_code == KEY_RSK && evt->key_type == KEY_LONG_PRESS)
        {
            if (IS_IN_CALL())
            {
                mmi_ucm_end_key();
 ...
 *  key_click_continue
 * DESCRIPTION
 *  Timer callback for single click
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
static void key_click_continue(void)
{
	mmi_event_struct key_click_evt;
	if (g_key_click.click_status == CLICK_STATUS_SINGLE)
	{
		if (g_key_click.has_double_click != KAL_TRUE)
		{
			if (is_long_pressed == MMI_FALSE)
			{
				kal_prompt_trace(MOD_MMI, "[WRISTBAND_KEY]emit EVT_ID_KEY_CLICK_SINGLE");
				MMI_FRM_INIT_EVENT(&key_click_evt, EVT_ID_KEY_CLICK_SINGLE);
				MMI_FRM_CB_EMIT_EVENT(&key_click_evt);
			}
		}
		memset(&g_key_click, 0, sizeof(g_key_click));
	}	
...
static void mmi_key_handle(mmi_key_evt_struct *mmi_evt_p)
{
    ...
    for (i = 0; i < evt_num; i++)
    {
        ...
        if (MMI_TRUE == mmi_frm_is_input_engine_free(MMI_DEVICE_KEY))
        {
            ...
            MMI_TRACE(
	          ...
	          mmi_frm_input_event_handle_input(
				    exec_evt[i].cvt_code,exec_evt[i].mmi_key_type);			
			  #else/*__MMI_FRM_INPUT_EVT__*/

		#ifdef __MMI_WEARABLE_DEVICE__
			if (exec_evt[i].cvt_code == KEY_POWER)
			{
				if (exec_evt[i].mmi_key_type == KEY_EVENT_DOWN)
				{
					power_key_repeat_count = 0;
				}
				else if (exec_evt[i].mmi_key_type == KEY_REPEAT)
				{
					power_key_repeat_count++;
					if (power_key_repeat_count == DELAYED_POWER_OFF_TIME_COUNT)
					{
						delayed_power_off();
					}
				}
		...
		#ifdef __MMI_WRISTBAND_SUPPORT__	
			if (key_event.key_type == KEY_EVENT_DOWN)
			{
				is_long_pressed = MMI_FALSE;
				kal_get_time(&system_time);
				current_key_down_time = kal_ticks_to_milli_secs(system_time);
				kal_prompt_trace(MOD_MMI, "[WRISTBAND_KEY]current_key_down_time = %d", current_key_down_time);
				kal_prompt_trace(MOD_MMI, "[WRISTBAND_KEY]click_up_time = %d", g_key_click.click_up_time);
				kal_prompt_trace(MOD_MMI, "[WRISTBAND_KEY]click_status = %d", g_key_click.click_status);
				g_key_click.has_double_click = KAL_FALSE;
				if (current_key_down_time - g_key_click.click_up_time > CONTINUE_CLICK_THRESHOLD)
				{
					memset(&g_key_click, 0, sizeof(g_key_click));
				}
				else
				{
					if (IsMyTimerExist(KEY_CONTINUE_CLICK_TIMER) == MMI_TRUE)
					{
						StopTimer(KEY_CONTINUE_CLICK_TIMER);
					}

					if (g_key_click.click_status == CLICK_STATUS_SINGLE)						
					{
						kal_prompt_trace(MOD_MMI, "[WRISTBAND_KEY]emit EVT_ID_KEY_CLICK_DOUBLE");
						MMI_FRM_INIT_EVENT(&key_click_evt, EVT_ID_KEY_CLICK_DOUBLE);
						MMI_FRM_CB_EMIT_EVENT(&key_click_evt);
						memset(&g_key_click, 0, sizeof(g_key_click));
						g_key_click.has_double_click = KAL_TRUE;
					}
				}
			}
			else if (key_event.key_type == KEY_LONG_PRESS)
			{
				is_long_pressed = MMI_TRUE;
			}
			else
			{
				if (key_event.key_type == KEY_EVENT_UP)
				{
					kal_get_time(&system_time);
					current_key_up_time = kal_ticks_to_milli_secs(system_time);
					kal_prompt_trace(MOD_MMI, "[WRISTBAND_KEY]current_key_up_time = %d", current_key_up_time);
					kal_prompt_trace(MOD_MMI, "[WRISTBAND_KEY]current_key_down_time = %d", current_key_down_time);
					kal_prompt_trace(MOD_MMI, "[WRISTBAND_KEY]click_status = %d", g_key_click.click_status);
					if (current_key_up_time - current_key_down_time > SINGLE_CLICK_THRESHOLD)
					{
						memset(&g_key_click, 0, sizeof(g_key_click));
					}
					else
					{
						g_key_click.click_up_time = current_key_up_time;
						if (g_key_click.click_status == CLICK_STATUS_NONE)
						{
							if (is_long_pressed == MMI_FALSE)
							{
								g_key_click.click_status = CLICK_STATUS_SINGLE;
								StartNonAlignTimer(KEY_CONTINUE_CLICK_TIMER, CONTINUE_CLICK_THRESHOLD, key_click_continue);
							}
						}
					}
				}
...
static void get_mmi_key_events(dev_key_evt_struct *dev_evt_p, mmi_key_evt_struct *mmi_evt_p)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __CKPD_VIRTUAL_KEY__
    if (dev_evt_p->device_key_code >= NEW_DEVICE_KEY_BEGIN  && dev_evt_p->device_key_code <= NEW_DEVICE_KEY_END)
		{
				mmi_key_code = dev_evt_p->device_key_code;
    		mmi_evt_p->cvt_code = dev_evt_p->device_key_code;
		}
		else
    {
#endif
...
void SetKeyDownHandler(FuncPtr funcPtr, U16 keyCode)
{
    mmi_frm_set_key_handler(funcPtr, keyCode, KEY_EVENT_DOWN);
}
void SetKeyUpHandler(FuncPtr funcPtr, U16 keyCode)
{
    mmi_frm_set_key_handler(funcPtr, keyCode, KEY_EVENT_UP);
}
void SetKeyRepeatHandler(FuncPtr funcPtr, U16 keyCode)
{
    mmi_frm_set_key_handler(funcPtr, keyCode, KEY_REPEAT);
}
void SetKeyLongpressHandler(FuncPtr funcPtr, U16 keyCode)
{
    mmi_frm_set_key_handler(funcPtr, keyCode, KEY_LONG_PRESS);
}
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 +
