...
#if defined(__MED_IN_ASM__) && defined(__UPDATE_BINARY_FILE__)
...
#if (defined(__UPDATE_BINARY_FILE__) && !defined(__NVRAM_SUPPORT_CUSTPACK__))
extern void MMICacheDefaultInit(kal_uint8 *bRstMMICacheDefault);
#endif 

#if defined(__MRE_PACKAGE_NONE__)
int vm_appcomm_queue_is_empty(void){return 1;}
S32 srv_mre_appmgr_init(mmi_event_struct *evt){return 1;}
...
MMI_BOOL vm_frm_is_active(void){return 0;}
U32 srv_mre_get_ua_string(char *buff, U32 size){return 0;}
void _vm_kernel_free(void* ptr){}
mmi_ret vm_ivr_handler(mmi_event_struct *param){return 0;}
MMI_STR_ID mmi_am_get_error_info(S32 error_result, S32 *popup_type){return 0;}
S32 mmi_am_is_using_media_memory(void){return 0;}
unsigned int vm_get_mre_version(void){return 0;}
int vm_get_system_info(char info[], unsigned int* size){return 0;}
MMI_BOOL srv_mre_appmgr_is_bg_running(mmi_app_package_char *app_name){return 0;}
int vm_pmng_get_app_handle(void){return 0;}
#else
...
extern void mmi_frm_send_log_by_bt();

#if defined(__MMI_SCREEN_SWITCH_EFFECT__)
extern void gui_sse_init(void);
#endif

#if defined(__COMMAND_PORT_SUPPORT__)
extern void* (*rmmi_cmd_processor_alloc_mem)(U32 size, asm_alloc_flag alloc_flag, CHAR* file, U32 line);
extern void (*rmmi_cmd_processor_free_mem)(void *ptr, CHAR* file, U32 line);
...
#if defined(__UPDATE_BINARY_FILE__) && defined(__XYSSL_SUPPORT__)
extern kal_uint32 (*xyssl_app_getcurrtime)(void);
extern kal_uint32 (*xyssl_app_gettime)(kal_uint32 *t_loc);
#endif

#if (defined (__WIFI_SUPPORT__) && defined (__CUSTOM_WIFI_FEATURES_SWITCH__) && defined (__UPDATE_BINARY_FILE__) && defined(__MTK_TARGET__))
extern kal_uint32 (*dhcp_app_getcurrtime)(void);
#endif
...
#if defined (__GPS_ADAPTOR_SUPPORT__) && defined(__CUSTOM_GPS_FEATURE_SWITCH__) && !defined(__GPS_EPO_DOWNLOAD__)
#include "app_md5.h"
extern int epodownload_function_ptr;
void epodownload_function_init(void){
	extern kal_uint32 app_gettime(kal_uint32 *t_loc);
	...
	*(int *)epodownload_function_ptr=(int)app_gettime;
	epodownload_function_ptr+=sizeof(int *);
	*(int *)epodownload_function_ptr=(int)applib_md5_init;
	epodownload_function_ptr+=sizeof(int *);
	*(int *)epodownload_function_ptr=(int)applib_md5_update;
	epodownload_function_ptr+=sizeof(int *);
	*(int *)epodownload_function_ptr=(int)applib_md5_final;
	epodownload_function_ptr+=sizeof(int *);
#ifdef __TCPIP__
	*(int *)epodownload_function_ptr=(int)srv_mode_switch_is_network_service_available;
#endif
	epodownload_function_ptr+=sizeof(int *);
}
#if defined(__MTK_TARGET__) && defined(__UPDATE_BINARY_FILE__) && !defined(__UPDATE_PKG_BT_ONLY__) && !defined(__UPDATE_PKG_SENSOR_ONLY__)
extern kal_uint32 UBIN_DummyReference();
...
MMI_BOOL MMI_Init(task_indx_type task_indx)
{
    ...
    #if defined (__GPS_ADAPTOR_SUPPORT__) && (__CUSTOM_GPS_FEATURE_SWITCH__) && !defined(__GPS_EPO_DOWNLOAD__)
	  epodownload_function_init();
	  #endif
    
#if defined(__COMMAND_PORT_SUPPORT__)
    rmmi_cmd_processor_alloc_mem = mmi_frm_asm_alloc_anonymous_int;
    rmmi_cmd_processor_free_mem = mmi_frm_asm_free_anonymous_int;
#endif /*__COMMAND_PORT_SUPPORT__*/
#if defined(__MED_IN_ASM__) && defined(__UPDATE_BINARY_FILE__)
	mmi_frm_asm_alloc_int_p = mmi_frm_asm_alloc_int;
	mmi_frm_asm_free_int_p = mmi_frm_asm_free_int;
#endif
    InitFileSystem();
    applib_mime_init();
    ...
    #if (defined (__WIFI_SUPPORT__) && defined (__CUSTOM_WIFI_FEATURES_SWITCH__) && defined (__UPDATE_BINARY_FILE__) && defined(__MTK_TARGET__))    
    dhcp_app_getcurrtime = app_getcurrtime;
    #endif    
    
    #if defined(__UPDATE_BINARY_FILE__) && defined(__XYSSL_SUPPORT__)
      xyssl_app_getcurrtime = app_getcurrtime;
      xyssl_app_gettime=   app_gettime;
    #endif
    #if defined(__MTK_TARGET__) && defined(__UPDATE_BINARY_FILE__) && !defined(__UPDATE_PKG_BT_ONLY__) && !defined(__UPDATE_PKG_SENSOR_ONLY__)
	UBIN_DummyReference();
...
void MMI_task(oslEntryType *entry_param)
{
    ...
    mmi_ext_qid = qid;
    #if (defined(__UPDATE_BINARY_FILE__) && !defined(__NVRAM_SUPPORT_CUSTPACK__))
    MMICacheDefaultInit(&bRstMMICacheDefault);
    #endif
...
/* under construction !*/
                                        ...
                                        /* disk check */
                                #if defined(__FLIGHT_MODE_SUPPORT__) && defined(__MMI_TELEPHONY_SUPPORT__) && !defined(__IOT__)
...
#ifdef __COMMAND_PORT_SUPPORT__
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_CUSTOMIZED_COMMAND_PASS_IND, (PsFuncPtr)app_frm_at_command_process, KAL_FALSE);
#endif /*__COMMAND_PORT_SUPPORT__*/
#ifdef __MMI_USB_SUPPORT__
    SetProtocolEventHandler(srv_usb_detect_ind_hdlr, MSG_ID_MMI_EQ_USBDETECT_IND);
#endif 
#ifdef __HEART_RATE_ENABLE__
    SetProtocolEventHandler(srv_sensor_hr_data_hdlr, MSG_ID_MMI_EQ_HR_SENSOR_DATA_IND);
#endif
