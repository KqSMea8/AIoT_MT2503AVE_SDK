...
void wgui_cat9015_post_display_callback(void *item, void *common_item_data, S32 x, S32 y)
{
    ...
    S32 item_index;
#if defined(__MMI_MAINMENU_SPREAD_SUPPORT__) && (defined(__MMI_MAINLCD_128X160__) || defined(__MMI_MAINLCD_240X320__))
...
#if defined(__MMI_MAINMENU_SPREAD_SUPPORT__) && (defined(__MMI_MAINLCD_128X160__) || defined(__MMI_MAINLCD_240X320__))
	if ( (item_index == MMI_fixed_matrix_menu.highlighted_item) && current_MMI_theme->matrix_main_menu_highlight_image)
	{
		...
		if (m->icon_x <= 6)
		{
			gui_draw_filled_area(x + m->icon_x, y, x + m->width, y + m->height, &background_filler);
			gdi_image_draw_resized(x + m->icon_x + 2, y + m->icon_y + 2, item_w + 6, item_h + 6, mi->item_icon);
		}
		else
	#if defined(__MMI_MAINLCD_128X160__)
		{
			gui_push_and_set_clip(x + 6, y, x + m->width, y + m->height);
			gui_draw_filled_area(x + 6, y, x + m->width, y + m->height, &background_filler);
			gdi_image_draw(x + m->icon_x, y + m->icon_y, mi->item_icon);
			gui_pop_clip();
		}
	#else /* defined(__MMI_MAINLCD_240X320__) */
		{
			gui_push_and_set_clip(x + 10, y, x + m->width, y + m->height);
			gui_draw_filled_area(x + 10, y, x + m->width, y + m->height, &background_filler);
			gdi_image_draw(x + m->icon_x, y + m->icon_y, mi->item_icon);
			gui_pop_clip();
		}
	#endif
    ...
    if (wgui_cat9015_app_package_info[item_index].image.type ==  MMI_IMAGE_SRC_TYPE_PATH)
    {
        ...
        if ((image_w > item_w) || (image_h > item_h))
        {
            ...
            gdi_image_draw_resized_file(
                ...
                item_w - 1, 
                item_h - 1, 
                (S8*)wgui_cat9015_app_package_info[item_index].image.data.path);
        }
        else
        {
            gui_set_clip(
        ...
        U8 *history_buffer)
{
    ...
    wgui_fixed_matrix_create_menu(
        ...
        fixed_icontext_menuitem_apply_theme(current_UI_theme->menuitem_theme);
		...
		ixsize = (MMI_CONTENT_WIDTH - current_fixed_matrix_menu_theme->vbar_width)/ WUGI_MM_MATRIX_ITEM_NUM_X;		
	#else
...
/* under construction !*/
        ...
        SetKeyDownHandler(fixed_matrix_goto_previous_item, KEY_VOL_UP);
        SetKeyDownHandler(fixed_matrix_goto_next_item, KEY_VOL_DOWN);
        SetKeyRepeatHandler(fixed_matrix_goto_previous_item, KEY_VOL_UP);
        SetKeyRepeatHandler(fixed_matrix_goto_next_item, KEY_VOL_DOWN);
...
/* under construction !*/
        ...
        #if 0//def __MMI_MAINLCD_240X320__
             #ifdef __MMI_MAINMENU_PAGE_SUPPORT__
/* under construction !*/
        #endif 
        #endif 
        ...
        #ifdef __MMI_WALLPAPER_ON_BOTTOM__
            #if 0//def __MMI_MAINMENU_PAGE_SUPPORT__
/* under construction !*/
            #endif
            ...
            SetKeyDownHandler(fixed_list_goto_previous_item, KEY_LEFT_ARROW);
            SetKeyDownHandler(fixed_list_goto_next_item, KEY_RIGHT_ARROW);
            SetKeyRepeatHandler(fixed_list_goto_previous_item, KEY_LEFT_ARROW);
            SetKeyRepeatHandler(fixed_list_goto_next_item, KEY_RIGHT_ARROW);            
		#endif
            break;
...
#endif
        /*for Pluto QVGA show 3*3 */
        #if defined(__PLUTO_MMI_PACKAGE__) && defined(__MMI_MAINLCD_240X320__)          
            iysize = (iysize < 80)? 80 : iysize;
        #endif
            

#if defined(__MMI_BT_DIALER_SUPPORT__) 
    #if defined(__MMI_MAINLCD_320X480__)
            ixsize = (MMI_fixed_matrix_menu.width - MMI_fixed_matrix_menu.vbar.width) >> 2;
            iysize = (MMI_fixed_matrix_menu.height - MMI_fixed_matrix_menu.hbar.height) >> 2;
            configure_fixed_matrix(ixsize, iysize, 4, 0);  // 16 grid for HVGA
    #elif defined(__MMI_MAINLCD_240X320__) || defined(__MMI_MAINLCD_320X240__) || defined(__MMI_MAINLCD_176X220__)|| defined(__MMI_MAINLCD_128X160__) || defined(__MMI_MAINLCD_128X128__)
            ixsize = (MMI_fixed_matrix_menu.width - MMI_fixed_matrix_menu.vbar.width) >> 1;
            iysize = (MMI_fixed_matrix_menu.height - MMI_fixed_matrix_menu.hbar.height) >> 1;
            configure_fixed_matrix(ixsize, iysize, 2, 0);  // 4 grid
    #else
            configure_fixed_matrix(ixsize, iysize, 3, 0);  // 9 grid , ex. 240x400...
    #endif
#else
    #ifdef __MMI_MAINLCD_320X240__
        ...
        U8 *history_buffer)
{
    ...
    SetKeyDownHandler(wgui_cat9014_up_arrow_key_hdlr, KEY_UP_ARROW);
    SetKeyDownHandler(wgui_cat9014_down_arrow_key_hdlr, KEY_DOWN_ARROW);
...
void ExitCategory14Screen(void)
{
    ...
    #if 0//defined(__MMI_MAINLCD_240X320__) || defined(__MMI_MAINLCD_240X400__) || defined(__MMI_MAINLCD_320X480__)
/* under construction !*/
    #endif 
    #if 0
    #if(TITLE_TRANSITION_STYLE_NON_BLOCKING)
