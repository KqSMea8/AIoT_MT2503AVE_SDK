...
cui_pwd_basic_cntx_struct *cui_pwd_basic_get_cntx(mmi_id instance_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void cui_pwd_basic_pen_long_tap_hdlr(mmi_pen_point_struct pos)
{
    if(pos.y <= MMI_DIALING_KEYPAD_Y)
		cui_pwd_basic_on_rsk_pressed();
...
static void cui_pwd_basic_entry(mmi_scrn_essential_struct *es_data)
{
 ...
 #else
    /*for set ok button handle function*/
 	SetDialingPinOkHandler(cui_pwd_basic_on_user_confirmed);
 #if defined(__MMI_TOUCH_SCREEN__)
    wgui_register_pen_long_tap_handler(cui_pwd_basic_pen_long_tap_hdlr);
#endif
 #endif
