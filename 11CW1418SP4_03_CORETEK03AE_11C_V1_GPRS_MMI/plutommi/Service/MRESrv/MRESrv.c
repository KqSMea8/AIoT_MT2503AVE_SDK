...
extern VMUINT vm_init_default_game(VMUINT* appcount, VMUINT* smcount, VMUINT* socount, VMUINT* vtpcount, VMUINT* vxpcount);
...
#ifdef __IOT__
extern VMINT g_pmng_httpd_launch_ret;
#endif
...
srv_mre_load_core_result_enum srv_mre_load_core(srv_mre_load_core_cb_t load_cb, srv_mre_load_core_caller_enum caller)
{
    ...
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_9, load_ret);
    return load_ret;
}
...
static void srv_mre_launch_app_cb(U16 *app_path, srv_mre_launch_app_evt_enum evt)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_APP_CALLBACK_RESULT, evt);
...
srv_mre_launch_app_result_enum srv_mre_launch_app_check_cert(U16 *app_path)
{
    ...
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_SRV_LAUNCH_CHECK,ce_check_result,ret);
...
srv_mre_launch_app_result_enum srv_mre_launch_app(U16 *app_path, srv_mre_launch_app_cb_t app_cb)
{
    ...
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_APP_RESULT_2,result);
...
srv_mre_launch_app_result_enum srv_mre_launch_app_ex(U16 *app_path, srv_mre_launch_app_cb_t app_cb)
{
    ...
    //MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G10, "[MRE][Service] srv_mre_launch_app <app_path:%s>\n", file_path_log_out);
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_7,file_path_log_out);

    /* Check parameter */
    if (app_path == NULL)
    {
        MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_8);
        MRE_SWLA_STOP("M11");
        return SRV_MRE_LAUNCH_APP_BAD_PARAM;
    }
    ...
    if (!srv_mre_is_core_loaded())
    {
        ...
        {
        	MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_12);
            MRE_SWLA_STOP("M11");
    ...
    vm_get_vm_tag((short*)app_path, VM_CE_INFO_NO_SCREEN, &is_no_screen, &bufSize);

    if (srv_mre_is_app_bg_running(app_path) > 0)
    {
        MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_13);
        if ( is_no_screen || (ret = (S32) vm_pmng_set_fg((VMWSTR) app_path)) >= 0)
        {
            if (app_cb)
            {
        ...
        {
        	MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_14,ret);
            if (app_cb)
            {
        ...
        MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_17, start_ret);
    }
    else
    {
		...
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_18, ret);
        ret += 16;
	...
	#ifdef __IOT__
	if(SRV_MRE_LAUNCH_APP_SUCCESS!=start_ret)
	{
		temp_result = 0;
	}
	if(parameter_of_start!=NULL&&temp_result==0&&g_pmng_httpd_launch_ret==0)
	{
		#ifdef __INET_HTTPD__
		inet_httpd_cgi_launch_ret(((cgi_launch_param *)parameter_of_start)->hc_handle,temp_result);
		#endif
	}
	_vm_kernel_free(parameter_of_start);
	parameter_of_start = NULL;
	g_pmng_httpd_launch_ret = 0;
    #endif
...
/* under construction !*/
...
#ifdef __MMI_MRE_AUTH_SUPPORT__
    {
        ...
        srv_mre_appmgr_generate_identification(app_path, app_name_tmp);
        ...
        {
            if ((pcb = pcb_table[i]) != NULL)
            {
                if (0 == kal_wstrcmp(pcb->fullFileName, app_path))
                {
                    pcb->user_auth = user_defined_auth;
                    kal_prompt_trace(MOD_MRE, "[MRE_AUTH]set_auth_when_launch_app, write to pcb, auth = %d", pcb->user_auth);
                    break;
                }
            }
        }
...
MMI_BOOL srv_mre_free_app_image_ex(U8 *p_img)
{
    ...
    res_buf = p_img - g_mresrv_offset - 10;
    vm_free(res_buf);
#if 0
/* under construction !*/
#endif
...
/*****************************************************************************
* FUNCTION
*  srv_mre_get_app_authorization
* DESCRIPTION
*  Get resolution
* PARAMETERS
*  app_path : [IN]  MRE application full path.
*  app_auth : [OUT] authorization info.
* RETURN VALUES
*  MMI_TRUE : If successful.
*  MMI_FALSE : If failed.
*****************************************************************************/
MMI_BOOL srv_mre_get_app_authorization(U16 *app_path, U32 *app_auth)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (vm_get_vm_tag((PS16) app_path, VM_CE_INFO_AUTH, (void*)&value, &size))
    {
        *app_auth = value;
        return MMI_TRUE;
    }
...
MMI_BOOL srv_mre_is_app_bg_running(U16 *app_path)
{
    ...
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_SERVICE_IS_APP_BG_RUNNING, ret);
...
U32 srv_mre_get_buildin_app_list(U16 **app_path_list, U32 *app_num)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U32 appcount, smcount, socount, vtpcount, vxpcount;
    S8 rom_app_path[20];
...
/* under construction !*/
    #endif
    //MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G10, "[MRE][Service] srv_mre_is_app_valid <rescode:%d>\n", rescode);
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_SRV_LAUNCH_APP_IS_VALID,rescode);
...
#define MRE_SRV_ATCI_RET_OK                  "\r\n+EMREAT:OK\r\n"
#define MRE_SRV_ATCI_RET_ERROR               "\r\n+EMREAT:ERROR\r\n"


void srv_mre_send_atci_data_cnf(srv_mre_data_req_struct *cnf)
{
...
kal_uint8 srv_mre_atci_data_translate(kal_uint8 *data, kal_uint16 length, kal_uint8 *path, kal_uint32 *para1, kal_uint32 *para2 )
{
		kal_uint16 index = 0; 
		kal_uint16 data_index = 0;
		kal_uint8 command[20]; 
		//kal_uint8 path[128];
		kal_uint8 para_buff[5];
		//kal_uint32 para1, para2;

		//AT command: AT+EMREDRV="path",para1,para2

		while ((data[data_index] != '=') && (index < 19) && (data_index < length))  //Get command  
		{
			command[index++] = data[data_index++] ;
		}
		if(index == 19 || data_index == length)
		{
			return 1;
		}
		command[index] = '\0';
		if(strcmp(command, "AT+EMREAT"))  //If valid AT command
		{
			return 1;
		}
		...
		while ((data[data_index] != ',') && (index < 127) && (data_index < length)) //Get parh
		{
			path[index++] = data[data_index++] ;
		}
		if(index == 127 || data_index == length)
		{
			return 1;
		}
		path[index] = '\0';   
		...
		data_index++;  //skip ","
		index = 0;
		while ((data[data_index] != ',') && (index < 4) && (data_index < length))  //Get para1
		{
			para_buff[index++] = data[data_index++] ;
		}

		if(index == 4 || data_index == length)
		{
			return 1;
		}

		para_buff[index] = '\0';
		*para1 = atoi(para_buff);

		data_index++; //skip ","
		index = 0;
		while ((data[data_index] != '\r') && (data[data_index] != '\n') && (index < 4) && (data_index < length)) //Get para2
		{
			para_buff[index++] = data[data_index++] ;
		}
		if(index == 4 || data_index == length)
...
static void srv_mre_atci_data_launch_app_cb(U16 *app_path, srv_mre_launch_app_evt_enum evt)
{
	return;
}


VMINT srv_mre_atci_data_rsp(VM_P_HANDLE sender, VMUINT msg_id, VMINT wparam, VMINT lparam)
{
    if (VM_MSG_ATCI_RSP != msg_id)
    {
        return 0;
    }
    else
    {
        srv_mre_data_req_struct *cnf;
        
        cnf = construct_local_para(sizeof(srv_mre_data_req_struct), TD_RESET);
        cnf->src_id = (kal_uint8)lparam;
        strcpy((kal_char*)cnf->data, (kal_char*)wparam);
        cnf->size = strlen((kal_char*)wparam);
        cnf->data_type = GAS_OUTPUT_NORMAL;

        srv_mre_send_atci_data_cnf(cnf);
    }
}

void srv_mre_atci_init(void)
{
    vm_appcomm_reg_msg_proc(0,srv_mre_atci_data_rsp);
}

void srv_mre_atci_data_ind_hdlr(srv_mre_data_ind_struct *ind)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    kal_uint8 path[128] = {0};
    kal_uint32 para1, para2;
    kal_wchar path_ucs2[128] = {0};

    kal_uint8 translate_ret = 0;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

    translate_ret = srv_mre_atci_data_translate((kal_uint8*)ind->data, ind->size, path, &para1, &para2);
    app_asc_str_to_ucs2_wcs((kal_wchar*) path_ucs2, (kal_int8*)path);
    
    if (para1 == 0 && para2 == 0 && translate_ret == 0)
    {
        srv_mre_close_app((VMUWSTR)path_ucs2);
        ret = 0;
    }
    else if (para1 == 0 && para2 == 1&& translate_ret == 0)
    {
        srv_mre_launch_app_result_enum ret_tem;
        
        ret_tem = srv_mre_launch_app((VMUWSTR)path_ucs2, srv_mre_atci_data_launch_app_cb);
        
        if (ret_tem == SRV_MRE_LAUNCH_APP_SUCCESS)
        {
            ret = 0;
        }
        else
    ...
    else if (para1 == 0 && para2 == 2 && translate_ret == 0)
    {
        if(FS_Delete(path_ucs2) == 0)
        {
            ret = 0;
        }
        else
    ...
    else if (para1 == 1 && translate_ret == 0)
    {
        VMINT handle = vm_pmng_get_handle((VMWSTR)path_ucs2);
        vm_appcomm_post_msg(handle, VM_MSG_ATCI_IND, (VMINT)para2, (VMINT)ind->src_id);
        ret = 2;
    }
    else
    {
    ...
    {
        cnf = construct_local_para(sizeof(srv_mre_data_req_struct), TD_RESET);
        cnf->src_id = ind->src_id;
        strcpy((kal_char*)cnf->data, MRE_SRV_ATCI_RET_OK);
        cnf->size = strlen(MRE_SRV_ATCI_RET_OK);
        cnf->data_type = GAS_OUTPUT_NORMAL;
    ...
    else if (ret == 1)
    {
        cnf = construct_local_para(sizeof(srv_mre_data_req_struct), TD_RESET);
        cnf->src_id = ind->src_id;
        strcpy((kal_char*)cnf->data, MRE_SRV_ATCI_RET_ERROR);
        cnf->size = strlen(MRE_SRV_ATCI_RET_ERROR);
        cnf->data_type = GAS_OUTPUT_NORMAL;
