...
kal_prompt_trace(MOD_ENG,"%s() %d ind->LocalRole=%d",__func__,__LINE__,ind->LocalRole);
    if (!ind || !ind->app_ctx)
    {
        return;
    }
...
void gatts_init_handle_connected_ind(void)
{
...
void gatts_deinit_handle_connected_ind(void)
{
    ClearProtocolEventHandler(MSG_ID_BT_GATTS_CONNECT_CNF);
    ClearProtocolEventHandler(MSG_ID_BT_GATTS_CONNECTED_IND);
}
#endif
/*****************************************************************************
 * FUNCTION
