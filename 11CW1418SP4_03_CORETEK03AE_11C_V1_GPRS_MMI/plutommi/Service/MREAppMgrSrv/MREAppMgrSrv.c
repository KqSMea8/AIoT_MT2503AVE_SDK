...
#if defined(__MMI_MRE_PREINSTALL_)||defined(__IOT__)||defined(__MMI_MRE_AUTO_START__)
typedef enum 
{  
    StartState,    
    SectionLabelState,  
...
MMI_BOOL srv_mre_appmgr_get_file_path(mmi_app_package_char *app_name, U16 *filepath)
{
    ...
    if (srv_mre_appmgr_get_installed_info(app_name_w, installed_info))
    {
        if ((installed_info->app_type & SRV_MRE_APPMGR_APP_TYPE_ROM) &&
            (installed_info->updated_app_path[0] != 0) && (installed_info->updated_app_path[1] != 0))
        {
            MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE][APPMGR], get installed app path, app_name = %s", app_name);
            mmi_wcscpy(filepath, installed_info->updated_app_path);
        }
        else
        {
            MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE][APPMGR], get build-in app path, app_name = %s", app_name);
...
static S32 srv_mre_appmgr_install_app_internal(
    U16 *app_path,
    srv_mre_appmgr_app_type_enum app_type,
    mmi_app_package_char *app_name,
    MMI_BOOL install_flag,MMI_BOOL r_flag)   //0:install; 1:update install ; r_flag:0 return bool, 1 return S32
{
    S32 ret = MMI_FALSE;//ok
    ...
    U32 developer_size;
    #endif
    MMI_BOOL is_already_installed = MMI_FALSE;
    MMI_BOOL is_full_auth = MMI_FALSE;
    ...
    {
    	if(r_flag)
    	    return -1;//-1 app_path is NULL
    	else
          return MMI_FALSE;
    }
    
    if (app_type & SRV_MRE_APPMGR_APP_TYPE_CTA_FULL)
    {
        app_type &= ~SRV_MRE_APPMGR_APP_TYPE_CTA_FULL;
        is_full_auth = MMI_TRUE;
    }
    ...
    if(install_flag==0)
    {
        ...
        if (able_to_install_ret==MMI_FALSE)
        {
            ...
            if(r_flag)
    	        return -2;//-2 reach max installed number.
    	      else
            return MMI_FALSE;
        }
    }
    if (!srv_mre_appmgr_generate_identification(app_path, temp_app_name))
    {
        ...
        if(r_flag)
    	      return -3;//-3 get app name tag error
    	  else
    ...
    if (installed_info == NULL)
    {
        ...
        MMI_TRACE(MMI_MRE_TRC_G2_APPMGR_SRV, MMI_MRE_TRC_G2_APPMGR_INSTALL_E1);
        if(r_flag)
    	      return -4;// -4 alloc memory from engine fail
    	  else
            return MMI_FALSE; 
    }
    ...
    else
    {
        ...
        if (is_shortcut)
        {
            if (srv_mre_appmgr_get_installed_info(temp_app_name, installed_info))
            {
                if (installed_info->app_size > 0)
                {
                    vm_free(installed_info);
                    if(r_flag)
    	                 return -5;//-5 shortcut already installed
    	              else
                       return MMI_FALSE;
                }
            }
            else
            {
                vm_free(installed_info);
                if(r_flag)
    	              return -6;//-6 get shortcut installed info file fail,fs error
    	          else
                    return MMI_FALSE;
            }
        ...
        else
        {
            ...
            MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV, "[MRE][install] check if already installed.file_hdl = %d", file_hdl);
            ...
            if (file_hdl >= FS_NO_ERROR)
            {
                ...
                FS_Close(file_hdl);
                is_already_installed = MMI_TRUE;

                file_hdl = FS_Open(info_path, FS_OPEN_NO_DIR | FS_READ_WRITE);
                MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE][install] already proload in ROM, get install info.file_hdl = %d", file_hdl);
                if (file_hdl >= FS_NO_ERROR)
                {
                    FS_Read(file_hdl,installed_info,sizeof(srv_mre_appmgr_installed_info_struct),&size);
                    FS_Close(file_hdl);
                }
                srv_mre_close_app(installed_info->app_path);
    ...
    if(get_img_ret==MMI_FALSE || get_name_ret==MMI_FALSE)
    {
      ...
      if(r_flag)
    	    return -8;//-8: get tag for icon or app name fail
    	else
    	    return MMI_FALSE;
    }
    ...
    else if (is_already_installed == MMI_FALSE)
    {
        if(!is_shortcut)
        {
...
//#endif
    ...
    /* Get app authorization info */
    if (MMI_TRUE == srv_mre_get_app_authorization(app_path,&installed_info->app_auth))
    {
    	  MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV, "[MRE_AUTH]Install app, success to set auth: %x, app_type = %d, is_full_auth = %d", installed_info->app_auth, app_type, is_full_auth);
        if ((is_full_auth == MMI_TRUE) || (app_type & SRV_MRE_APPMGR_APP_TYPE_ROM))
        {
            // full authorizations app
            installed_info->app_auth |= SRV_MRE_APPMGR_APP_AUTH_ALL;
        }
    }
    else
    {
        installed_info->app_auth = 0;
        MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]Install app, fail to get vxp authotization.");
    }
    installed_info->user_auth = installed_info->app_auth;     
#endif /* __MMI_MRE_AUTH_SUPPORT__ */
        ...
        if (info_hdl >= FS_NO_ERROR)
        {
            if(SRV_MRE_APPMGR_APP_TYPE_ROM==app_type)
            {
                ...
                else
                {
                    ...
                    //srv_mre_appmgr_write_maptable();
                    if(r_flag)
    	                 ret = -9;//-9 write installed info for build-in app fail
    	              else
                       ret = MMI_FALSE; 
                }
            #if defined(__MMI_WEARABLE_DEVICE__)||defined(__IOT__)
            ...
            else
            {
                ...
                {               
                    ...
                    if(r_flag)
                       ...
                       ret  = MMI_FALSE;//-11 save installed info fail for vxp install
                }


            }
        }
        if(ret !=MMI_TRUE && !is_shortcut)
        {
            //if(!is_shortcut)
            //{
            FS_Delete(info_path);
...
//#endif
    ...
    { 
        ...
        if (app_name != NULL)
        {
            ...
            if(install_flag==1 || is_already_installed)
            {
    ...
    return ret;// 0 unknow error 1 success
}

/*****************************************************************************
* FUNCTION
*  srv_mre_appmgr_install_app_ex
* DESCRIPTION
...
*  other: If failed.
*****************************************************************************/
S32 srv_mre_appmgr_install_app_ex(
                                    U16 *app_path,
                                    srv_mre_appmgr_app_type_enum app_type,
                                    mmi_app_package_char *app_name)
{//MMI_TRUE means need return the exactly value why install fail
    return srv_mre_appmgr_install_app_internal(app_path,app_type,app_name,MMI_FALSE,MMI_TRUE);   //0:install; 1:update install
}

/*****************************************************************************
* FUNCTION
*  srv_mre_appmgr_install_and_auto_launch_app
* DESCRIPTION
*  Install an application and launch it if it has auto launch tag.
* PARAMETERS
*  app_path : [IN] the application path.
*  app_type : [IN] the application type.
*  app_name : [OUT] the application identification.
...
*  2: install and autolaunch
*****************************************************************************/
S32 srv_mre_appmgr_install_and_auto_launch_app(
                                    U16 *app_path,
                                    srv_mre_appmgr_app_type_enum app_type,
                                    mmi_app_package_char *app_name)
{
	S32 ret = 0;
	mmi_app_package_char temp_app_name[MMI_APP_NAME_MAX_LEN];
	mmi_app_package_char* temp_p;
	if (app_name != NULL)
		temp_p = app_name;
	else
		temp_p = temp_app_name;
	MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "srv_mre_appmgr_install_and_auto_launch_app start");
    if (srv_mre_appmgr_install_app(app_path, app_type, temp_p))
	{
		ret = 1;
		MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "srv_mre_appmgr_install_and_auto_launch_app install success");
		if (srv_mre_appmgr_get_autostart(temp_p) > 0)
		{			
			srv_mre_launch_app_result_enum launch_ret;
			MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "srv_mre_appmgr_install_and_auto_launch_app auto launch start");	
			launch_ret = srv_mre_launch_app_ex(app_path, NULL);

			if(launch_ret == SRV_MRE_LAUNCH_APP_SUCCESS)
			{
			    ret = 2;	
			}			
		}
	...
	MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "srv_mre_appmgr_install_and_auto_launch_app ret = %d", ret);
...
MMI_BOOL srv_mre_appmgr_install_app(
                                    ...
                                    mmi_app_package_char *app_name)
{
    return srv_mre_appmgr_install_app_internal(app_path,app_type,app_name,MMI_FALSE,MMI_FALSE);   //0:install; 1:update install
}
...
/*****************************************************************************
* FUNCTION
*  srv_mre_appmgr_set_user_defined_authorization
* DESCRIPTION
*  set the user defined authorization infomation of a MRE application.
* PARAMETERS
*  app_name  : [IN] the application identification.
*  auth      : [IN] user defined authorization.
* RETURN VALUES
*  MMI_TRUE : If successful.
*  MMI_FALSE : If failed.
*****************************************************************************/
MMI_BOOL srv_mre_appmgr_set_user_defined_authorization(mmi_app_package_char *app_name, U32 auth)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]set_user_defined_auth,");
    if (!app_name)
    {
        MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]set_user_defined_auth, app_name = NULL");
        return MMI_FALSE;
    }
    ...
    if (!srv_mre_appmgr_is_mre_app(app_name_w))
    {
        MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]set_user_defined_auth, this is not a mre app.");
        return MMI_FALSE;
    }
    ...
    if (srv_mre_appmgr_get_installed_info(app_name_w, &installed_info))
    {
        ...
        file_hdl = FS_Open(info_path, FS_CREATE_ALWAYS | FS_READ_WRITE);
        if (file_hdl >= FS_NO_ERROR)
        {
            _vm_pcb_t* pcb = NULL;
            
            installed_info.user_auth = auth;
            FS_Write(file_hdl,&installed_info,sizeof(srv_mre_appmgr_installed_info_struct),&info_size_written);
            FS_Close(file_hdl);
            MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]set_user_defined_auth, success, auth = %x", installed_info.user_auth);
            ...
            for (i = VM_PMNG_FOREGROUND; i < VM_PMNG_MAX_NUM; i++)
            {
                ...
                while ( pcb != NULL)
                {
                    if (0 == vm_wstrcmp_ignore_case(pcb->fullFileName, (VMWSTR)installed_info.app_path))
                    {
                        pcb->user_auth = auth;
                        MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]set_user_defined_auth, write to pcb, auth = %x", pcb->user_auth);
                        break;
                    }
                    pcb = pcb->next_pcb;
                }
...
/*****************************************************************************
* FUNCTION
*  srv_mre_appmgr_get_authorization
* DESCRIPTION
*  Get the authorization infomation of a MRE application.
* PARAMETERS
*  app_name          : [IN] the application identification.
*  app_defined_auth  : [OUT] application defined authorization.
*  user_defined_auth : [OUT] user defined authorization.
* RETURN VALUES
*  MMI_TRUE : If successful.
*  MMI_FALSE : If failed.
*****************************************************************************/
MMI_BOOL srv_mre_appmgr_get_authorization(mmi_app_package_char *app_name, U32 *app_defined_auth, U32 *user_defined_auth)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (!app_defined_auth || !user_defined_auth || !app_name)
    {
        MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]get_auth, parameter error.");
        return MMI_FALSE;
    }
    ...
    if (!srv_mre_appmgr_is_mre_app(app_name_w))
    {
        MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]get_auth, this is not a mre app.");
        return MMI_FALSE;
    }

    if (srv_mre_appmgr_get_installed_info(app_name_w, &installed_info))
    {
        *app_defined_auth = installed_info.app_auth;
        *user_defined_auth = installed_info.user_auth;
        MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]get_auth, success, app_auth = %x, user_auth = %x", installed_info.app_auth, installed_info.user_auth);
        return MMI_TRUE;
    }
    else
    {
        MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]get_auth, fail.");
        return MMI_FALSE;
    }
}
...
/*****************************************************************************
* FUNCTION
*  srv_mre_appmgr_check_user_defined_authorization
* DESCRIPTION
*  Check the authorization infomation of a MRE application.
* PARAMETERS
*  app_name          : [IN] the application identification.
*  app_defined_auth  : [OUT] application defined authorization.
*  user_defined_auth : [OUT] user defined authorization.
...
*  MMI_FALSE : If failed.
*****************************************************************************/
MMI_BOOL srv_mre_appmgr_check_user_defined_authorization(mmi_app_package_char *app_name, srv_mre_appmgr_app_authorization_enum auth)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (srv_mre_appmgr_get_installed_info(app_name_w, &installed_info))
    {
        if (installed_info.user_auth & SRV_MRE_APPMGR_APP_AUTH_ALL)
        {
            return MMI_TRUE;
        } 

        if ((installed_info.user_auth & auth) == auth)
        {
            return MMI_TRUE;
        } 
    }
...
/*****************************************************************************
* FUNCTION
*  srv_mre_appmgr_check_current_defined_authorization
* DESCRIPTION
*  Check the authorization infomation of a MRE application.
* PARAMETERS
...
*  MMI_FALSE : If failed.
*****************************************************************************/
MMI_BOOL srv_mre_appmgr_check_current_defined_authorization(srv_mre_appmgr_app_authorization_enum auth)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    user_auth = vm_pmng_get_current_authorization();

    MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]check_auth, user_auth = %x, check_auth = %x", user_auth, auth);
    
    if (user_auth & SRV_MRE_APPMGR_APP_AUTH_ALL)
    {
        return MMI_TRUE;
    } 
    ...
    MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV,  "[MRE_AUTH]check_auth, fail.");
    return MMI_FALSE;
...
#endif
MMI_BOOL srv_mre_appmgr_app_uninstall_internal(mmi_app_package_char *app_name,U32 flag)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
// #ifndef __MRE_SLIM__
 ...
 //#endif
        ...
        if(flag==1)
		{
			...
			FS_Delete(installed_info.app_path);
		}
		...
		srv_mre_appmgr_app_delete_bt_file(app_name_w);
		#if defined(__MMI_WEARABLE_DEVICE__)||defined(__IOT__)
...
//#endif
    ...
    else
    {
        ...
        U32 index;
        srv_mre_close_app(installed_info.app_path);
        srv_mre_close_app(installed_info.updated_app_path);
        if(flag==1)
        {
            FS_Delete(installed_info.app_path);
            FS_Delete(installed_info.updated_app_path);
        }
        index = installed_info.app_unique_id - SRV_MRE_APPMGR_ID_BASE;
        srv_mre_appmgr_get_image_full_path(index,app_name_w,image_path);
        FS_Delete(image_path);
...
name_array = vm_malloc(sizeof(mmi_app_package_name_struct)*clock_app_num);
srv_mre_appmgr_get_app_list(SRV_APPMGR_INSTALLED_CLOCK_APP_FLAG_TYPE,name_array,clock_app_num);
for(i=0;i<clock_app_num;i++)
{
srv_mre_appmgr_get_file_path(name_array[i], filepath);
//get file path can do something.
}
*/
            ...
            if((g_mre_appmgr_maptable[index].app_install_type == SRV_MRE_APPMGR_APP_TYPE_ROM )||(g_mre_appmgr_maptable[index].app_install_type&SRV_MRE_APPMGR_APP_TYPE_ROM_VPP))
            {
               ...
               // srv_mre_load_core_result_enum load_mre_core_ret;
                ...
                if(resoffset)
                {
                    ...
                    /* Check does it have app icon 4bytes-align  */
                    if (vm_get_vm_tag((PS16) app_path, 0x36, &tag_buf, &tag_size) != GET_TAG_TRUE)
                    {
                        tag_buf[0] = 0;
                    }
                    MMI_PRINT(MOD_MRE,MMI_MRE_TRC_G2_APPMGR_SRV,"tag_buf[0]=%d",tag_buf[0]);
                    image->type = MMI_IMAGE_SRC_TYPE_MEM;
                    if(tag_buf[0] == 0)
                    {
                        image->data.image.image_ptr = ptr+resoffset+13+10+header_size;
                    
                        img_size = *(ptr+resoffset + 13 + 6) +
                            ((*(ptr+resoffset+ 13 + 7)) << 8) + 
                            ((*(ptr+resoffset + 13 + 8)) << 16) + 
                            ((*(ptr+resoffset + 13 + 9)) << 24);
                    }
                    else
                    {
                        image->data.image.image_ptr = ptr+resoffset+24+header_size;
                    
                        img_size = *(ptr+resoffset + 13 + 7) +
                            ((*(ptr+resoffset+ 13 + 8)) << 8) + 
                            ((*(ptr+resoffset + 13 + 9)) << 16) + 
                            ((*(ptr+resoffset + 13 + 10)) << 24);
                    }
                                        ...
                                        U32 param_size)
{
    ...
    /*----------------------------------------------------------------*/
    //U16 app_name_w[MMI_APP_NAME_MAX_LEN];
     U32 app_defined_auth, user_defined_auth;
...
#ifdef __MMI_MRE_AUTH_SUPPORT__
    //srv_mre_appmgr_get_authorization(app_name, &app_defined_auth, &user_defined_auth);
    //srv_mre_appmgr_set_user_defined_authorization(app_name, user_defined_auth);
#endif
    return 0;
}
...
static void srv_mre_appmgr_preinstall_t_card(FS_HANDLE t_preintall_hdl,char *key_value,WCHAR *app_name_w,U32 offset,U32 start_offset,MMI_BOOL flag,MMI_BOOL save_flag)
{
    ...
    if(flag==MMI_FALSE)
    {
        ...
        {
            MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_APPMGR_SRV, "[MRE_AUTH]Preinstall app, set all auth.");
            install_ret = srv_mre_appmgr_install_app(app_name_w, SRV_MRE_APPMGR_APP_TYPE_LOCAL_INSTALL | SRV_MRE_APPMGR_APP_TYPE_CTA_FULL, NULL);   
        }
        else
        {
...
{  
    ...
    U32 size = 0;
    CHAR *buffer = NULL;
    kal_wsprintf(t_preinstall_file, "%c", SRV_MRE_APPMGR_PREINSTALL_DRV);
    ...
    kal_wsprintf(app_name_w, "%c", SRV_MRE_APPMGR_PREINSTALL_DRV);
...
#if defined(__IOT__)||defined(__MMI_MRE_AUTO_START__)
void srv_mre_appmgr_launch_app_iot_cb(U16 *app_path, srv_mre_launch_app_evt_enum evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    case SRV_MRE_LAUNCH_APP_ASM_READY:
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_35);
        srv_mre_launch_app_ex(app_path, srv_mre_appmgr_launch_app_iot_cb);
        break;
...
#if defined(__LINKIT_DEV_KIT__) && defined(__MTK_TARGET__)
MMI_BOOL vm_bootup_blink_flag = MMI_FALSE;
DCL_HANDLE rx_handle, tx_handle;

static U32 blink_count = 0; ;

static void srv_mre_appmgr_blink_rx_tx(void)
{	
	StopTimer(MRE_APPMGR_RX_TX_TIMER);
	...
	if (vm_bootup_blink_flag)
	{
		DCL_Control(rx_handle,GPIO_CMD_WRITE_LOW, NULL); 
		DCL_Control(tx_handle,GPIO_CMD_WRITE_HIGH, NULL);
		vm_bootup_blink_flag = MMI_FALSE;
	}
	else
	{
		DCL_Control(rx_handle,GPIO_CMD_WRITE_HIGH, NULL); 
		DCL_Control(tx_handle,GPIO_CMD_WRITE_LOW, NULL); 
		vm_bootup_blink_flag = MMI_TRUE;
	}
	StartTimer(MRE_APPMGR_RX_TX_TIMER, 1000, srv_mre_appmgr_blink_rx_tx);
}

mmi_ret srv_mre_appmgr_handle_bootup_early_init(mmi_event_struct *event)
{
#if defined(__TINY_SYS__)
	if(srv_bootup_is_hibernation_mode())
	{
		return MMI_RET_OK;
	}
#endif  
...
extern VMBOOL vm_nvram_set_port(VMINT port);
extern void srv_mre_appmgr_auto_start_mre_app();

mmi_ret srv_mre_appmgr_bootup_iot_autostart_hdlr(mmi_event_struct *evt)
{
...
/* under construction !*/
#endif
		//srv_mre_appmgr_auto_start_mre_app();
...
/* under construction !*/
	...
	{
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_78);
		//if save mode, RX/TX blink		
		rx_handle = DCL_Open(DCL_GPIO, 10);
		tx_handle = DCL_Open(DCL_GPIO, 11);
		DCL_Control(rx_handle,GPIO_CMD_SET_MODE_0,NULL);
		DCL_Control(tx_handle,GPIO_CMD_SET_MODE_0,NULL);
		DCL_Control(rx_handle,GPIO_CMD_SET_DIR_OUT, NULL);
		DCL_Control(tx_handle,GPIO_CMD_SET_DIR_OUT, NULL);
...
static void srv_mre_appmgr_auto_launch_iot(FS_HANDLE t_preintall_hdl,char *key_value,WCHAR *app_name_w,U32 offset,U32 start_offset,MMI_BOOL flag,MMI_BOOL save_flag)
{
    U32 install_ret = 0;
    //U32 app_name_w_len = mmi_wcslen(app_name_w);
    U32 write_size = 0;
	U8  log[200];
	memset(log,0,200);
    mmi_chset_convert(MMI_CHSET_UTF8, MMI_CHSET_UCS2, (char *)key_value, (char *)app_name_w,  SRV_MRE_APPMGR_FILEPATH_LEN*2);
    //srv_mre_launch_app_result_enum result = srv_mre_launch_app_check_cert(app_name_w);
   ...
   // {
    ...
    //{
       // install_ret = MMI_FALSE;
   // }
	install_ret = srv_mre_launch_app_ex(app_name_w, srv_mre_appmgr_launch_app_iot_cb);
    if(save_flag)
    {
        FS_Write(t_preintall_hdl,key_value,offset - start_offset,&write_size);
        //if(install_ret == MMI_TRUE)
        //{
		sprintf(log, "        :  launch ret:%d\r\n",install_ret);
        FS_Write(t_preintall_hdl,log,strlen(log),&write_size);
        //}
       // else
        //{
       // / //   FS_Write(t_preintall_hdl,"        :  Install Fail\r\n",sizeof("        :  Install Fail\r\n"),&write_size);
...
MMI_BOOL srv_mre_appmgr_auto_start_iot(U32 flag) //1:phone fs, 0: t-card fs
{  
    U32 offset = 0;  
    U32 start_offset = 0;
    mmi_mre_appmgr_parse_state parse_state = StartState;
	...
	if(flag==0)
	{
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_1,SRV_MRE_APPMGR_PRELOAD_DRV);
		kal_wsprintf(t_preinstall_file, "%c", SRV_MRE_APPMGR_PRELOAD_DRV);
		//mmi_wcscat(t_preinstall_file, SRV_MRE_APPMGR_AUTO_START_FOLDER);
		mmi_wcscat(t_preinstall_file, SRV_MRE_APPMGR_AUTO_START_FILE);
	}
	if(flag==1)
	{  
//#if  defined(__MMI_WEARABLE_DEVICE__)
       // char c =  FS_GetDrive(FS_DRIVE_V_NORMAL, 2, FS_DRIVE_V_NORMAL);
//#elif  defined(__MMI_WEARABLE_DEVICE__)
   // char c =  FS_GetDrive(FS_DRIVE_I_SYSTEM, 1, FS_DRIVE_I_SYSTEM | FS_DRIVE_V_NORMAL);
//#else
		char c =  FS_GetDrive(FS_DRIVE_V_NORMAL, 2, FS_DRIVE_V_NORMAL);
//#endif
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_2,c);
		kal_wsprintf(t_preinstall_file, "%c", c);
		//mmi_wcscat(t_preinstall_file, SRV_MRE_APPMGR_AUTO_START_FOLDER);
		mmi_wcscat(t_preinstall_file, SRV_MRE_APPMGR_AUTO_START_FILE);
	}
    if(flag==3)
    ...
    if(t_preintall_hdl>=FS_NO_ERROR)
    {
		...
		S32 fs_ret = 0;
        FS_GetFileSize(t_preintall_hdl,&size);
        buffer = (CHAR *)_vm_kernel_calloc(size+1);
        if(buffer==NULL)
        {
            MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_4,size+1);
            return MMI_FALSE;
		}
		fs_ret = FS_Read(t_preintall_hdl,buffer,size,&read_size);
        if(fs_ret <FS_NO_ERROR)
        {
        	MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_5,fs_ret);
            FS_Close(t_preintall_hdl);
            return MMI_FALSE;
        }
       // if(flag==MMI_FALSE)
    ...
    {
        MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_3);
        return  MMI_FALSE;
    }
    app_name_w = (WCHAR *)_vm_kernel_malloc(SRV_MRE_APPMGR_FILEPATH_LEN * sizeof(WCHAR));
    if(app_name_w==NULL)
    	{
    		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_6);
        return MMI_FALSE;
      }
    ////if(flag==0)
	//{
       //kal_wsprintf(app_name_w, "%c", SRV_MRE_APPMGR_PRELOAD_DRV);
       //mmi_wcscat(app_name_w, SRV_MRE_APPMGR_AUTO_START_FOLDER);
	//}
	//if(flag==1)
	//{
		//kal_wsprintf(app_name_w, "%c", SRV_MRE_APPMGR_PHONE_DRV);
        //mmi_wcscat(app_name_w, SRV_MRE_APPMGR_AUTO_START_FOLDER);
	//}
    ...
    if(flag==0)
	{
		kal_wsprintf(t_preinstall_file, "%c", SRV_MRE_APPMGR_PRELOAD_DRV);
		//mmi_wcscat(t_preinstall_file, SRV_MRE_APPMGR_AUTO_START_FOLDER);
		mmi_wcscat(t_preinstall_file, SRV_MRE_APPMGR_AUTO_FILE_RESULT);
	}
	if(flag==1)
	{
		kal_wsprintf(t_preinstall_file, "%c", SRV_MRE_APPMGR_PHONE_DRV);
		//mmi_wcscat(t_preinstall_file, SRV_MRE_APPMGR_AUTO_START_FOLDER);
		mmi_wcscat(t_preinstall_file, SRV_MRE_APPMGR_AUTO_FILE_RESULT);
	}
    if(flag==3)
    {
        char d =  FS_GetDrive(FS_DRIVE_I_SYSTEM, 1, FS_DRIVE_I_SYSTEM | FS_DRIVE_V_NORMAL);
        kal_wsprintf(t_preinstall_file, "%c", d);
        mmi_wcscat(t_preinstall_file, SRV_MRE_APPMGR_AUTO_FILE_RESULT);
    }
    FS_Delete(t_preinstall_file);
	t_preintall_hdl = FS_Open(t_preinstall_file, FS_CREATE_ALWAYS | FS_READ_WRITE);
        ...
        case ValueState: 
            {
                ...
                {  
                    ...
                    if(buffer[offset-1]=='\r')
                        buffer[offset-1]='\0';
                    memcpy(key_value,&(buffer[start_offset]),offset - start_offset);
                    srv_mre_appmgr_auto_launch_iot(t_preintall_hdl,key_value,app_name_w,offset,start_offset,flag,save_flag);
...
srv_mre_appmgr_auto_start_phone()
{
	srv_mre_appmgr_auto_start_iot(1);
}

srv_mre_appmgr_auto_start_tcard()
{
	srv_mre_appmgr_auto_start_iot(0);
	//WCHAR auto_folder[SRV_MRE_APPMGR_PATH_SIZE];
	//FS_HANDLE auto_hdl = -1;
	//memset(auto_folder,0,sizeof(WCHAR)*SRV_MRE_APPMGR_PATH_SIZE)
	//kal_wsprintf(auto_folder, "%c", SRV_MRE_APPMGR_PRELOAD_DRV);
   // mmi_wcscat(auto_folder, SRV_MRE_APPMGR_AUTO_START_FOLDER);
    //auto_hdl = FS_Open(mre_path, FS_OPEN_DIR | FS_READ_ONLY);
}
void srv_mre_appmgr_auto_start_mre_app()
{
	srv_mre_appmgr_auto_start_phone();//phone driver
	srv_mre_appmgr_auto_start_tcard();//t card
    srv_mre_appmgr_auto_start_iot(3);//system driver
...
{
    return srv_mre_appmgr_install_app_internal(app_path,(srv_mre_appmgr_app_type_enum)0,NULL,MMI_TRUE,MMI_FALSE);   //0:install; 1:update install 
}
...
#endif
#else
int srv_mre_appmgr_get_app_list(int flag, char *name_array, unsigned int num)
{
	return 0;
}
unsigned int srv_mre_appmgr_get_app_num(int flag)
{
	return 0;
}
unsigned char srv_mre_appmgr_get_file_path(char *app_name, unsigned short*filepath)
{
	return 0;
}
unsigned char srv_mre_appmgr_is_app_installed_by_path(unsigned short *app_path)
{
	 return 0;
}
int vm_get_vm_tag(short* filename,int tag_num, void* buf ,int* buf_size)
{
    return 0;
}
unsigned char srv_mre_appmgr_generate_identification(unsigned short *app_path, unsigned short *app_name){}
void srv_mre_close_app(unsigned short *app_path){}
unsigned char srv_mre_get_app_state(unsigned short *app_path){}
#endif
