 ...
 *  srv_prof_is_in_call
 * DESCRIPTION
 *  Is there bt call or local call ?
 *  
 * PARAMETERS
 *  
 *  
 ...
 *  void
 *****************************************************************************/
MMI_BOOL srv_prof_is_in_call()
{
    MMI_BOOL in_bt_call = MMI_FALSE;
    MMI_BOOL in_local_call = MMI_FALSE;
    MMI_BOOL in_call = MMI_FALSE;
    
    /* check in call*/
    in_local_call = (MMI_BOOL)srv_ucm_query_call_count(SRV_UCM_CALL_STATE_ALL, SRV_UCM_CALL_TYPE_NO_DATA_CSD, NULL);
#ifdef __MMI_HF_SUPPORT__
    in_bt_call = srv_bt_dialer_is_any_call();
#endif

    if(in_local_call || in_bt_call)
    {
        in_call = MMI_TRUE;
    }
    return in_call;
...
void srv_prof_init(void)
{
    ...
    g_srv_prof.cur_prof = SRV_PROF_SILENT_MODE_NUM;
    g_srv_prof.last_prof = SRV_PROF_SILENT_MODE_NUM;
...
kal_int8 volume_status = -1;
void bkss_set_volume_test(kal_uint8 status)
{
	volume_status = status;
}
kal_uint8 bkss_get_volume_test(void)
{
...
static void srv_prof_play_audio_info_init(srv_prof_play_audio_struct* play_info, srv_prof_tone_enum tone_type)
{
    ...
    if(bkss_get_volume_test() >= 0)
    {
        if(bkss_get_volume_test() == 0)
        {
            play_info->volume = SRV_PROF_VOL_LEVEL_6;
        }
        else
        {
            play_info->volume = SRV_PROF_VOL_LEVEL_1;
        }
    }
    else if((SRV_PROF_TONE_POWER_OFF == tone_type)||(SRV_PROF_TONE_POWER_ON == tone_type)||(SRV_PROF_TONE_ALARM == tone_type))
    {
        if((curr_time.nHour >= 7)&&(curr_time.nHour < 22))
        {
            play_info->volume = SRV_PROF_VOL_LEVEL_6;
        }
        else
...
/* under construction !*/
    ...
    else
    {
        ...
        if(!audio_data)
        {
            srv_prof_play_audio_callback(MDI_AUDIO_END_OF_FILE, NULL);
            return;
        }
