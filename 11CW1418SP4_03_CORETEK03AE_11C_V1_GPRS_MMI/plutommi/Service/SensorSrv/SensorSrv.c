...
*****************************************************************************/
#ifdef MOTION_SENSOR_SUPPORT
SRV_SENSOR_ACC_DATA_CB acc_data_cb = NULL;
#ifdef __HEART_RATE_ENABLE__
SRV_SENSOR_HR_DATA_CB  hr_data_cb = NULL; 
U32 hr_data = 0;
#endif
...
#ifdef __MTK_TARGET__
#ifdef __HEART_RATE_ENABLE__
extern void PowerControl_Cardio(kal_bool enable);
#endif
#endif
/***************************************************************************** 
* Local Function
...
static MMI_BOOL sensor_srv_motion_read_switch()
{
	...
	kal_prompt_trace(MOD_MMI,"[MIN]g_motion_sensor_able = %d",g_motion_sensor_able);
...
static void srv_sensor_motion_data_hdlr(void *msg_ptr)
{
	...
	if ((msg_data_ptr->data_state == MOTION_DATA_EXIST) || (msg_data_ptr->data_state == MOTION_DATA_FULL))
	{
		...
		if (is_has_data == MMI_TRUE)
		{
			...
			kal_prompt_trace(MMI_MEDIA_TRC_G10_MMI_PRINT,"[acc]acc_data_cb start");
            if(acc_data_cb)
            {
	            (*acc_data_cb)((void *)&g_srv_sensor_motion_cntx.detail_info.acc_x,
							(void *)&g_srv_sensor_motion_cntx.detail_info.acc_y,
							(void *)&g_srv_sensor_motion_cntx.detail_info.acc_z);
				kal_prompt_trace(MMI_MEDIA_TRC_G10_MMI_PRINT,"[acc]acc_data_cb!=NUll ,g_srv_sensor_motion_cntx.detail_info.acc_x = %d,g_srv_sensor_motion_cntx.detail_info.acc_y = %d,g_srv_sensor_motion_cntx.detail_info.acc_z = %d",g_srv_sensor_motion_cntx.detail_info.acc_x,g_srv_sensor_motion_cntx.detail_info.acc_y,g_srv_sensor_motion_cntx.detail_info.acc_z);
            }
			kal_prompt_trace(MMI_MEDIA_TRC_G10_MMI_PRINT,"[acc]acc_data_cb end");
...
#ifdef __HEART_RATE_ENABLE__
void srv_sensor_hr_data_hdlr(void *info)
{
    mmi_sensor_hr_data_ind_struct *p = (mmi_sensor_hr_data_ind_struct*) info;
	kal_prompt_trace(MMI_MEDIA_TRC_G10_MMI_PRINT,"[HR]srv_sensor_hr_data_hdlr");
	kal_prompt_trace(MMI_MEDIA_TRC_G10_MMI_PRINT,"[HR]srv_sensor_hr_data_hdlr CALLBACK p->hr_data = %d", p->hr_data);
    if(hr_data_cb)
    {
    	hr_data = p->hr_data;
        (*hr_data_cb)((void *)&hr_data);
		kal_prompt_trace(MMI_MEDIA_TRC_G10_MMI_PRINT,"[HR]srv_sensor_hr_data_hdlr CALLBACK hr_data = %d", hr_data);
    }

}
void srv_sensor_hr_data_rigister_cb(SRV_SENSOR_HR_DATA_CB event_hdlr)
{
    hr_data_cb = event_hdlr;
	kal_prompt_trace(MMI_MEDIA_TRC_G10_MMI_PRINT,"[HR]srv_sensor_hr_data_rigister_cb");

}
#endif

void srv_sensor_acc_data_rigister_cb(SRV_SENSOR_ACC_DATA_CB event_hdlr)
{
     acc_data_cb = event_hdlr;
	 kal_prompt_trace(MMI_MEDIA_TRC_G10_MMI_PRINT,"[acc]srv_sensor_acc_data_rigister_cb");
