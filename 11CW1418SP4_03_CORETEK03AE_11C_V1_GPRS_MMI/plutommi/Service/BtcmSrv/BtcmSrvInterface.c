...
srv_bt_cm_hdlr_struct srv_bt_cm_handle[SRV_BT_CM_MAX_INSTANCE_NUM];
	...
	U32 g_srv_bt_cm_iot_flag_by_flight_mode;
#endif


static void srv_bt_cm_nvram_read_info(void);
#ifdef __MMI_BT_TEMP_VISIBILITY__
static void srv_bt_cm_set_temp_visibility_timeout(void);
#endif 
...
void srv_bt_cm_set_bqb_test_flag(MMI_BOOL bqb_flag)
{
	...
	#ifdef __MMI_BT_APPLICATION_BQB__
	{
		...
		MMI_TRACE(MMI_CONN_TRC_G7_BT,SRV_BT_CM_SET_BQB_TEST_FLAG,(kal_int8)use_custom_cod);
		Custom_Config_Set_BT_COD(cod, use_custom_cod);
		Custom_Config_Set_BQB_COD(KAL_TRUE);
	}
	#endif
}
...
S32 srv_bt_cm_get_visibility(void)
{
    ...
    if (SRV_BT_CM_GET_FLAG(SRV_BT_CM_MASK_VIS)
        #ifdef __MMI_BT_DIALER_SUPPORT__
            && MMI_FALSE == srv_bt_cm_get_prevent_connect_flag()
        #endif
        )
    {
...
static void srv_bt_cm_set_temp_visibility_timeout(void)
{
	...
	#ifdef __BT_LE_ONLY__
	srv_bt_cm_write_scanenable_mode_req_hdler(BTBM_ADP_P_OFF_I_OFF);
	#else
...
static void srv_bt_cm_set_limit_visibility_timeout_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (i < SRV_BT_CM_MAX_DEVICE_LIST)
    {
        for (j = 0; j < SRV_BT_CM_MAX_CONNECTION; ++j)
        {
                ...
                (g_srv_bt_cm_cntx.existed_conn[j].active))
            {
	            MMI_TRACE(
			        MMI_CONN_TRC_G7_BT,
			        SRV_BT_CM_IS_DEV_CONN_ACTIVE_1,
			        g_srv_bt_cm_cntx.existed_conn[j].conn_id,
			        g_srv_bt_cm_cntx.existed_conn[j].index_of_dev_list,
			        g_srv_bt_cm_cntx.existed_conn[j].active,
			        g_srv_bt_cm_cntx.existed_conn[j].profile_id);
                result = MMI_TRUE;
                break;
            }
        }
    ...
    switch (dev_type)
    {
        ...
        case SRV_BT_CM_DEFAULT_DEV_TYPE_HID:
        {
			...
			if(i < SRV_BT_CM_MAX_DEVICE_LIST)
			{
				...
				MMI_TRACE(MMI_CONN_TRC_G7_BT,SRV_BT_CM_SET_DEFAULT_DEV_2,g_srv_bt_cm_cntx.dev_list[i].dev_info.attribute);
...
S32 srv_bt_cm_start_conn(MMI_BOOL in_out, U32 profile_id, srv_bt_cm_bt_addr* dev_addr, CHAR* dev_name)
{
	...
	)
	{
		...
		{
			MMI_TRACE(MMI_CONN_TRC_G7_BT,SRV_BT_CM_START_CONN_3);
			return -1;
		}
	}
#endif
    ...
    else                        // incomming connection
    {
		...
		#ifdef __MMI_BT_DIALER_SUPPORT__
		if (bth_state == SRV_BT_CM_STATE_CONNECTING)
		{
			if(profile_id == SRV_BT_CM_ADVANCED_AUDIO_PROFILE_UUID ||
				profile_id == SRV_BT_CM_HF_PROFILE_UUID ||
				profile_id == SRV_BT_CM_SPP_PROFILE_UUID)
			{
				conn_id = srv_bt_cm_allocate_conn_id_int(dev_addr, profile_id);
				if (conn_id > 0)
				{
					g_srv_bt_cm_cntx.existed_conn[conn_id-1].is_excluded = MMI_TRUE;
				}
				MMI_TRACE(
			        MMI_CONN_TRC_G7_BT,
			        SRV_BT_CM_START_CONN_2,
			        conn_id,
			        g_srv_bt_cm_cntx.state,
			        profile_id);
				return conn_id;
...
void srv_bt_cm_connect_ind(U32 conn_id)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	MMI_BOOL is_excluded = MMI_FALSE;
    U32 j;
	...
	#ifdef __MMI_BT_DIALER_SUPPORT__
		is_excluded = g_srv_bt_cm_cntx.existed_conn[conn_id-1].is_excluded;
            g_srv_bt_cm_cntx.existed_conn[conn_id-1].is_excluded = MMI_FALSE;
	#endif
...
void srv_bt_cm_stop_conn(U32 conn_id)
{
    ...
    MMI_TRACE(
        ...
        SRV_BT_CM_STOP_CONN,
        conn_id,
        g_srv_bt_cm_cntx.existed_conn[conn_id - 1].profile_id);
...
void ATA_le_get_bd_addr(BD_ADDR *dst,const srv_bt_cm_bt_addr *src)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void srv_bt_cm_init_le_addr(char * buffer)
{
	bt_bm_change_le_address_req_struct *res_msg;

	res_msg = (bt_bm_change_le_address_req_struct*) OslConstructDataPtr(sizeof(bt_bm_change_le_address_req_struct));

	res_msg->addr[0] = buffer[0];
	res_msg->addr[1] = buffer[1];
	res_msg->addr[2] = buffer[2];
	res_msg->addr[3] = buffer[3];
	res_msg->addr[4] = buffer[4];
	res_msg->addr[5] = buffer[5];
 ...
 *  void
 *****************************************************************************/
 unsigned char addrbuffer[BD_ADDR_SIZE];
 void srv_bt_cm_init(void)
{
	...
	BD_ADDR bd_addr = {0};
	BD_ADDR bd_addr1 = {0};
	BD_ADDR bd_addrbuffer = {0};
	nvram_ef_btradio_mtk_bt_chip_struct btradio_mtk_bt_cntx;
	//add by wpf end
	
	kal_prompt_trace(MOD_ENG,"BLE=init");
    MMI_TRACE(
        MMI_CONN_TRC_G7_BT,
        SRV_BT_CM_INIT);
    memset(&g_srv_bt_cm_cntx, 0, sizeof(srv_bt_cm_cntx_struct));
    memset(srv_bt_cm_handle, 0, (sizeof(srv_bt_cm_hdlr_struct) * SRV_BT_CM_MAX_INSTANCE_NUM));
#ifdef __MMI_BT_DIALER_SUPPORT__
	srv_bt_cm_reset_prevent_connect_flag();
#endif
	//add by wpf start	
	ReadRecordSlim(NVRAM_EF_BTRADIO_MTK_BT_CHIP_LID, 1,&btradio_mtk_bt_cntx, sizeof(nvram_ef_btradio_mtk_bt_chip_struct));
	if(btradio_mtk_bt_cntx.BDAddr[0]==0)
	{
	
	}
	else
	{
		srv_bt_cm_init_le_addr(btradio_mtk_bt_cntx.BDAddr);
		
		kal_prompt_trace(MOD_ENG,"BLE srv_bt_cm_init_le_addr init");
...
#if defined(__IOT__) && defined(__MMI_TELEPHONY_SUPPORT__)
	ReadValueSlim(SRV_BT_IOT_POWER_STATUS, &g_srv_bt_cm_iot_flag_by_flight_mode, DS_BYTE);
#endif
//add by wpf start
ATA_le_get_bd_addr(&bd_addr,&g_srv_bt_cm_cntx.host_dev.le_bd_addr);
kal_prompt_trace(MOD_ENG,"BLE bd_addr1=%d,%d,%d,%d,%d,%d",bd_addr.addr[0],bd_addr.addr[1],bd_addr.addr[2],bd_addr.addr[3],bd_addr.addr[4],bd_addr.addr[5]);
ReadRecordSlim(NVRAM_EF_BTRADIO_MTK_BT_CHIP_LID, 1,&btradio_mtk_bt_cntx, sizeof(nvram_ef_btradio_mtk_bt_chip_struct));
if(btradio_mtk_bt_cntx.BDAddr[0]==0)
{
	memset(btradio_mtk_bt_cntx.BDAddr, 0, sizeof(btradio_mtk_bt_cntx.BDAddr));	
	memcpy(btradio_mtk_bt_cntx.BDAddr, bd_addr.addr, sizeof(bd_addr.addr));
	WriteRecordSlim(NVRAM_EF_BTRADIO_MTK_BT_CHIP_LID, 1, &btradio_mtk_bt_cntx, sizeof(nvram_ef_btradio_mtk_bt_chip_struct));
	kal_prompt_trace(MOD_ENG,"BLE2=%d,%d,%d,%d,%d,%d",bd_addr.addr[0],bd_addr.addr[1],bd_addr.addr[2],bd_addr.addr[3],bd_addr.addr[4],bd_addr.addr[5]);
	kal_prompt_trace(MOD_ENG,"BLE3=%d,%d,%d,%d,%d,%d",btradio_mtk_bt_cntx.BDAddr[0],btradio_mtk_bt_cntx.BDAddr[1],btradio_mtk_bt_cntx.BDAddr[2],btradio_mtk_bt_cntx.BDAddr[3],btradio_mtk_bt_cntx.BDAddr[4],btradio_mtk_bt_cntx.BDAddr[5]);
...
g_srv_bt_cm_cntx.host_dev.bd_addr.lap = g_srv_bt_cm_cntx.host_dev.le_bd_addr.lap;
g_srv_bt_cm_cntx.host_dev.bd_addr.uap= g_srv_bt_cm_cntx.host_dev.le_bd_addr.uap;
g_srv_bt_cm_cntx.host_dev.bd_addr.nap= g_srv_bt_cm_cntx.host_dev.le_bd_addr.nap;


//add by wpf end 
ATA_le_get_bd_addr(&bd_addr1,&g_srv_bt_cm_cntx.host_dev.bd_addr);
kal_prompt_trace(MOD_ENG,"BLE5 bd_addr1=%d,%d,%d,%d,%d,%d",bd_addr1.addr[0],bd_addr1.addr[1],bd_addr1.addr[2],bd_addr1.addr[3],bd_addr1.addr[4],bd_addr1.addr[5]);
memset(addrbuffer, 0, sizeof(addrbuffer));	
memcpy(addrbuffer, bd_addr1.addr, sizeof(bd_addr1.addr));
 ...
 *****************************************************************************/
S32 srv_bt_cm_get_existed_conn_num_by_addr(srv_bt_cm_bt_addr* dev_addr)
{
	S32 dev_index = srv_bt_cm_get_dev_index(dev_addr, SRV_BT_CM_ALL_DEV);
	S32 i, count;
	
	MMI_BT_ASSERT(dev_addr != NULL);
	MMI_TRACE(MMI_CONN_TRC_G7_BT, SRV_BT_CM_GET_EXISTED_CONN_NUM_BY_ADDR, 
    ...
    for (i = 0, count = 0; i < SRV_BT_CM_MAX_CONNECTION; ++i) 
    {
        if ((g_srv_bt_cm_cntx.existed_conn[i].index_of_dev_list == dev_index)&&
			(g_srv_bt_cm_cntx.existed_conn[i].conn_id > 0) &&
            (g_srv_bt_cm_cntx.existed_conn[i].active))
        {
            ++count;
	...
	MMI_TRACE(MMI_CONN_TRC_G7_BT, SRV_BT_CM_GET_EXISTED_CONN_NUM_BY_ADDR_END, count); 
	return count;
}
...
static void srv_bt_cm_release_all_conn_timeout_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  srv_bt_cm_result_codes_enum
 *****************************************************************************/
static S32 srv_bt_cm_release_conn(U32 conn_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
S32 srv_bt_cm_switch_on_ex(U8 power_on_mode)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    switch (g_srv_bt_cm_cntx.state)
    {
        case SRV_BT_CM_STATE_INIT_ON:
            #ifdef __TINY_SYS__
                g_srv_bt_cm_cntx.tiny_switch_on_mode = power_on_mode;
            #endif
...
{
    srv_bt_cm_switch_on_ex(0);
}
static void srv_bt_cm_switch_off_timeout_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
S32 srv_bt_cm_get_link_type(void)
{
	...
	if(srv_ble_cm_get_connected_dev_num()>0)
	{
		linked_type |= SRV_BT_CM_LINK_TYPE_LE;
	}
	MMI_TRACE(MMI_CONN_TRC_G7_BT,SRV_BT_CM_GET_LINK_TYPE,linked_type);
	return linked_type;
}
...
S32 srv_bt_cm_get_dev_link_type(srv_bt_cm_bt_addr* dev_addr)
{
	...
	if(srv_ble_cm_get_connected_list_inx(dev_addr)!= -1)
	{
		linked_type |= SRV_BT_CM_LINK_TYPE_LE;
	}
	MMI_TRACE(MMI_CONN_TRC_G7_BT,SRV_BT_CM_GET_DEV_LINK_TYPE,linked_type);
...
S32 srv_bt_cm_get_link_num(srv_bt_cm_link_type_enum link_type)
{
	...
	{
		 link_num += srv_ble_cm_get_connected_dev_num();
	}
	MMI_TRACE(MMI_CONN_TRC_G7_BT,SRV_BT_CM_GET_LINK_NUM,link_type,link_num);

	return link_num;
}
#endif

/* For IoT project, remove native app */
#if defined(__IOT__) && defined(__MMI_TELEPHONY_SUPPORT__)
mmi_ret srv_bt_switch_flight_mode(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                  */
    /*----------------------------------------------------------------*/
    ...
    if (EVT_ID_SRV_MODE_SWITCH_SWITCH_OTHER_NOTIFY == evt->evt_id)
    {
        ...
        {            
            switch (pwstatus)
            {
                case SRV_BT_CM_POWER_ON:
                    g_srv_bt_cm_iot_flag_by_flight_mode |= SRV_BT_CM_IOT_MASK_OFF_BY_FLIGHT_MODE;
                    srv_bt_cm_switch_off();
                    break;
                ...
                case SRV_BT_CM_POWER_SWITCHING_OFF:
                    g_srv_bt_cm_iot_flag_by_flight_mode |= SRV_BT_CM_IOT_MASK_JUST_NOTIFY_FLIGHT_MODE;
                    break;
            }
        ...
        else if (mode_switch->select_flight_mode == SRV_MODE_SWITCH_OFF)
        {
            ...
            {
            case SRV_BT_CM_POWER_ON:
                g_srv_bt_cm_iot_flag_by_flight_mode |= SRV_BT_CM_IOT_MASK_ON_BY_FLIGHT_MODE;
               // srv_bt_cm_activate_profile();
                srv_bt_cm_active_complete(MMI_TRUE);
                break;

            case SRV_BT_CM_POWER_OFF:
                if (g_srv_bt_cm_iot_flag_by_flight_mode & SRV_BT_CM_IOT_MASK_ON)
                {
                    g_srv_bt_cm_iot_flag_by_flight_mode |= SRV_BT_CM_IOT_MASK_ON_BY_FLIGHT_MODE;
                    srv_bt_cm_switch_on();
                }
	...
	MMI_TRACE(
		MMI_CONN_TRC_G7_BT,
		SRV_BT_SWITCH_FLIGHT_MODE_2,
		g_srv_bt_cm_iot_flag_by_flight_mode);

    return MMI_RET_OK;
}

MMI_RET srv_bt_cm_init_power(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void srv_bt_cm_active_complete(MMI_BOOL result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	MMI_TRACE(
		MMI_CONN_TRC_G7_BT,
		SRV_BT_CM_ACTIVE_COMPLETE,
		g_srv_bt_cm_iot_flag_by_flight_mode,
		result);

	if (g_srv_bt_cm_iot_flag_by_flight_mode & SRV_BT_CM_IOT_MASK_OFF_BY_FLIGHT_MODE)
	{
	   srv_bt_cm_switch_off();
	   g_srv_bt_cm_iot_flag_by_flight_mode |= SRV_BT_CM_IOT_MASK_ON;
	}
	else if (g_srv_bt_cm_iot_flag_by_flight_mode & SRV_BT_CM_IOT_MASK_ON_BY_FLIGHT_MODE)
	{
	   g_srv_bt_cm_iot_flag_by_flight_mode &= ~SRV_BT_CM_IOT_MASK_ON_BY_FLIGHT_MODE;
	   srv_mode_switch_update_cb_func();
	   srv_bt_cm_activate_profile();
	...
	else
	{
	   ...
	   if (g_srv_bt_cm_iot_flag_by_flight_mode & SRV_BT_CM_IOT_MASK_JUST_NOTIFY_FLIGHT_MODE)
	   {
		   g_srv_bt_cm_iot_flag_by_flight_mode &= SRV_BT_CM_IOT_MASK_JUST_NOTIFY_FLIGHT_MODE;
		   srv_mode_switch_update_cb_func();
	   }
	}
	WriteValueSlim(SRV_BT_IOT_POWER_STATUS, &g_srv_bt_cm_iot_flag_by_flight_mode, DS_BYTE);

}

void srv_bt_cm_deactive_complete(void)
{
	/*----------------------------------------------------------------*/
   /* Local Variables												 */
   /*----------------------------------------------------------------*/
