...
static int epb_get_varint32_bits(const uint8_t *data, int len, uint32_t *value);
 ...
 static int sizeof_tag(uint16_t tag)
{
...
static int epb_get_tag_value_offset(const uint8_t *data, int len, uint16_t tag)
{
    ...
    while (offset < len) {
        ...
        wire_type = epb_get_wire_type(p_tag);
        ret = epb_get_value_length(data+offset, len-offset, wire_type);
        if (ret < 0) {
            offset++;
            continue;
        }
...
static int epb_pack_buf_remain(Epb *e)
{
