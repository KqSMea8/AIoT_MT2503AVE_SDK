...
BaseResponse *epb_unpack_base_response(const uint8_t *buf, int buf_len)
{
	...
	if (!epb_has_tag(&epb, TAG_BaseResponse_ErrCode)) {
		...
		BaseResponse *response = (BaseResponse *)get_ctrl_buffer(sizeof(BaseResponse));
...
{
	Epb epb;
	BasePush *push = (BasePush *)get_ctrl_buffer(sizeof(BasePush));
	epb_unpack_init(&epb, buf, buf_len);
...
AuthResponse *epb_unpack_auth_response(const uint8_t *buf, int buf_len)
{
	...
	if (!epb_has_tag(&epb, TAG_AuthResponse_AesSessionKey)) {
		...
		AuthResponse *response = (AuthResponse *)get_ctrl_buffer(sizeof(AuthResponse));
...
InitResponse *epb_unpack_init_response(const uint8_t *buf, int buf_len)
{
	...
	if (!epb_has_tag(&epb, TAG_InitResponse_UserIdLow)) {
		...
		InitResponse *response = (InitResponse *)get_ctrl_buffer(sizeof(InitResponse));
...
void epb_unpack_send_data_response(SendDataResponse *response,const uint8_t *buf, int buf_len)
{
	Epb epb;
	const uint8_t *tmp;
	int tmp_len,i;
	...
	if (!epb_has_tag(&epb, TAG_SendDataResponse_BaseResponse)) {
		...
		if (epb_has_tag(&epb, TAG_SendDataResponse_Data)) {
			...
			for(i=0;i<response->data.len;i++)
			{
				bkss_dbg_print(MOD_ENG,"%s() %d %d",__func__,__LINE__,response->data.data[i]);
			}
...
RecvDataPush *epb_unpack_recv_data_push(const uint8_t *buf, int buf_len)
{
	...
	{
	RecvDataPush *push = (RecvDataPush *)get_ctrl_buffer(sizeof(RecvDataPush));
...
SwitchViewPush *epb_unpack_switch_view_push(const uint8_t *buf, int buf_len)
{
	...
	{
	SwitchViewPush *push = (SwitchViewPush *)get_ctrl_buffer(sizeof(SwitchViewPush));
...
SwitchBackgroudPush *epb_unpack_switch_backgroud_push(const uint8_t *buf, int buf_len)
{
	...
	{
	SwitchBackgroudPush *push = (SwitchBackgroudPush *)get_ctrl_buffer(sizeof(SwitchBackgroudPush));
