...
MD5String append_padding_bits ( char * argv )
{
	...
	MD5String string;
	string.str = (char *)get_ctrl_buffer(msg_length + bit_length + APP_SIZE / BITS);
...
//	uint8_t r[16];
	...
	uint32_t j;
	uint8_t roundIdx,i;
	str = append_padding_bits ( argv );
	chain[0] = A;
	chain[1] = B;
	chain[2] = C;
	chain[3] = D;
	for (j = 0; j < str.len; j += BLOCK_SIZE / BITS)
	{
		memmove ( (char *)w, str.str + j, BLOCK_SIZE / BITS );
