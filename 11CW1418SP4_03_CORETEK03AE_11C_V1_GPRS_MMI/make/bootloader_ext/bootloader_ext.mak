...
SRC_LIST =  \
            ...
            hal\system\bootloader\src\bl_FUDIFFNET.c \
...
ifneq ($(strip $(ADUPS_FOTA_GPS_ENABLE)),TRUE)
#GPS fota
SRC_LIST += \
    ...
    #Timer wait api		  
   SRC_LIST += hal\system\pll\src\pll.c 
endif
endif
endif

ifdef FUDIFFNET_ENABLE
ifneq ($(strip $(FUDIFFNET_ENABLE)),NONE)
...
ifneq ($(strip $(ROCK_FOTA_SUPPORT)),TRUE)
	...
	# RedBend's IPL
    #SRC_LIST += vendor\redbend_fuediff\src\fudiffnet_rb_porting_api.c
...
ifneq ($(strip $(ADUPS_FOTA_DIFF_LIB)),TRUE)
	SRC_LIST += adups\bl\src\bspatch.c \
			adups\bl\src\bzlib.c \
			adups\bl\src\crctable.c \
			adups\bl\src\decompress.c \
			adups\bl\src\huffman.c \
			adups\bl\src\randtable.c \
			adups\bl\src\vmempool.c \
			adups\bl\src\adups_sha1.c \
			adups\bl\src\crc.c
endif


   SRC_LIST +=adups\hal\src\adups_bl_main.c \
			adups\hal\src\adups_mcu.c \
	 		adups\hal\src\adups_flash.c

   SRC_LIST += adups\hal\src\adups_file.c
...
#GPS fota
SRC_LIST += \
            adups\gps_fota\src\brom_base.c \
            adups\gps_fota\src\brom_mt3301.c \
            adups\gps_fota\src\da_cmd.c \
            adups\gps_fota\src\flashtool.c \
            adups\gps_fota\src\gps_uart.c
endif

  # For LCD
ifeq ($(strip $(FUNET_ENABLE)),NONE)
  SRC_LIST += adups\hal\src\lcd_fota.c \
              hal\display\lcd_if_manager\src\lcd_if_manager.c \
              hal\display\common\src\lcm_proxy.c
endif

ifeq ($(strip $(LCD_MODULE)),COMBO_LCM)
...
ifeq ($(strip $(ROCK_FOTA_SUPPORT)),TRUE)
	INC_DIR += vendor\rock_fota\include
else
	INC_DIR += vendor\redbend_fuediff\include
endif
endif
endif
ifeq ($(strip $(ADUPS_FOTA_DIFF_ENABLE)),TRUE)
...
ifneq ($(strip $(ADUPS_FOTA_DIFF_LIB)),TRUE)
	INC_DIR += adups\bl\include
endif
ifeq ($(strip $(FUNET_ENABLE)),NONE)
ifeq ($(strip $(ADUPS_FOTA_GPS_ENABLE)),TRUE)
	INC_DIR += adups\gps_fota\include
endif
endif
endif
...
ifeq ($(filter __LINKIT_DEV_KIT__, $(strip $(MODULE_DEFS))),)
...
ifdef FUNET_ENABLE
  ifneq ($(strip $(FUNET_ENABLE)),NONE)
    ...
    ifneq ($(filter __LCD_SUPPORT__,$(strip $(MODULE_DEFS))),)
       COMP_DEFS +=  __LCD_DRIVER_IN_BL__
    endif
  endif
endif
...
ifeq ($(strip $(ADUPS_FOTA_DIFF_ENABLE)),TRUE)
COMP_DEFS += __UBL_NOR_FULL_DRIVER__
endif
