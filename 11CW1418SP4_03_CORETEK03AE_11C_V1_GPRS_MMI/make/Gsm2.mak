...
ifndef MBIN_WITH_ALL_LIBS
  MBIN_WITH_ALL_LIBS := TRUE
endif
...
ifndef DO_CLEAN_MODULE
  ...
  ifeq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
    ifeq ($(strip $(CLEAN_MODS)),ALL)
      CLEAN_MODS := $(UBIN_COMPLIST)
    endif
  endif
...
ifeq ($(strip $(ENABLE_PARTIAL_TRACE)),TRUE)
  ...
  MBIN_CUS_REL_MTK_COMP := $(CUS_REL_MTK_COMP)
  ifeq ($(strip $(NEED_BUILD_TINY_SYSTEM)),TRUE)
    MBIN_CUS_REL_MTK_COMP := $(filter-out $(TS_COMPLIST),$(MBIN_CUS_REL_MTK_COMP))
  endif
  ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
    ifneq ($(strip $(MBIN_WITH_ALL_LIBS)),TRUE)
      MBIN_CUS_REL_MTK_COMP := $(filter-out $(UBIN_LINKLIST),$(MBIN_CUS_REL_MTK_COMP))
    endif
  endif
  MBIN_MTK_LIBS += $(foreach COMP,$(filter-out $(PARTIAL_TRACE_LIB),$(MBIN_CUS_REL_MTK_COMP)),$(CUS_MTK_LIB)\$(COMP).lib)
  MBIN_MTK_LIBS += $(foreach COMP,$(filter $(PARTIAL_TRACE_LIB),$(MBIN_CUS_REL_MTK_COMP)),$(CUS_MTK_LIB_TRACE)\$(COMP).lib)
else
  ...
  ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
    ...
    MBIN_MTK_LIBS += $(foreach COMP,$(MBIN_CUS_REL_MTK_COMP),$(CUS_MTK_LIB)\$(COMP).lib)
  endif
  MBIN_MTK_LIBS += $(foreach COMP,$(MBIN_CUS_REL_MTK_COMP),$(CUS_MTK_LIB)\$(COMP).lib)
...
ifeq ($(strip $(DO_UPDATE_MODULE)),TRUE)
  ...
  ifneq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
    ifeq ($(filter verno,$(UPDATE_MODS)),)
      UPDATE_MODS := $(UPDATE_MODS) verno
    endif
  endif
  ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
    ifeq ($(filter system_ubin,$(UPDATE_MODS)),)
      UPDATE_MODS := $(UPDATE_MODS) system_ubin
    endif
    ifeq ($(filter ubinverno,$(UPDATE_MODS)),)
      UPDATE_MODS := $(UPDATE_MODS) ubinverno
    endif
  endif
  endif
  ...
  ifeq ($(strip $(DO_REMAKE_MODULE)),TRUE)
    ...
    ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
      ifeq ($(filter system_ubin,$(REMAKE_MODS)),)
        REMAKE_MODS := $(REMAKE_MODS) system_ubin
      endif
      ifeq ($(filter ubinverno,$(REMAKE_MODS)),)
        REMAKE_MODS := $(REMAKE_MODS) ubinverno
      endif
    endif
    endif
    ifneq ($(strip $(CUSTOM_RELEASE)),TRUE)
    ...
    ifeq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
      COMPLIBLIST = $(call sort,$(foreach comp,$(UBIN_COMPLIST),$(comp).lib))
    else
...
ifeq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
  ifeq ($(strip $(DO_CLEAN_MODULE)),TRUE)
    CHK_COMPLIST := $(filter-out $(foreach comp,$(call lc,$(UBIN_COMPLIST)),$(comp)),$(call lc,$(CLEAN_MODS)))
    ifneq ($(words $(CHK_COMPLIST)),0)
      ifeq ($(strip $(CUSTOM_RELEASE)),TRUE)
        $(error $(foreach comp,$(CHK_COMPLIST),"$(comp)") NOT in CUS_REL_SRC_COMP or UBIN_COMPLIST)
      else
        $(error $(foreach comp,$(CHK_COMPLIST),"$(comp)") NOT in UBIN_COMPLIST)
      endif
    endif
  else
    CHK_COMPLIST = $(filter-out $(foreach comp,$(call lc,$(UBIN_COMPLIST)),$(comp).lib),$(call lc,$(COMPLIBLIST)))
    ifneq ($(words $(CHK_COMPLIST)),0)
      ifeq ($(strip $(CUSTOM_RELEASE)),TRUE)
        $(error $(foreach comp,$(CHK_COMPLIST),"$(comp)") NOT in CUS_REL_SRC_COMP or UBIN_COMPLIST)
      else
        $(error $(foreach comp,$(CHK_COMPLIST),"$(subst .lib,,$(comp))") NOT in UBIN_COMPLIST)
      endif
    endif
  endif
else
...
ifeq ($(strip $(MODIS_CONFIG)),FALSE)
  ifeq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
    MODULELIS := $(foreach comp,$(sort $(UBIN_COMPLIST)),$(comp).lis)
  else
...
# *************************************************************************
#  Settings for UPDATE BIN
# *************************************************************************
UBIN_PREFIX   := $(strip $(CUSTOMER))_UBIN_$(strip $(PLATFORM))
UBIN_BIN_FILE := $(strip $(UBIN_PREFIX))_$(strip $(PURE_VERNO)).bin
...
ifeq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
  ...
  ifneq ($(filter $(MAKECMDGOALS),remake cgen tiny_system ts_new ts_bm_new),)
    AUTO_CHECK_CGEN := TRUE
  else
...
define CheckRemakeNeedDependTarget
-include $$(strip $$(RULESDIR_TARGET))\$(2)\.\$(1).det
$(1): $$(strip $$(RULESDIR_TARGET))\$(2)\$(1).det
$$(strip $$(RULESDIR_TARGET))\$(2)\$(1).det: $$(NEED_CHECK_DEPEND_LIST)
endef
...
# *************************************************************************
ifeq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
new : backup ubin_checklist ubin_cleanall genlog codegen resgen update
else
...
new : backup cleanall genlog cleancodegen asngen umts_gen ss_lcs_gen codegen codegen_check asnregen cleanbin mcddll_update resgen nvram_auto_gen bootloader tiny_system remake
      else
new : backup cleanall genlog cleancodegen asngen codegen codegen_check asnregen cleanbin mcddll_update resgen nvram_auto_gen bootloader tiny_system remake
      endif    
    else
      ifneq ($(filter UMTS HSPA TDD128 TDD128DPA TDD128HSPA,$(strip $(call Upper,$(PROJECT)))),)
new : backup cleanall genlog cleancodegen asngen umts_gen ss_lcs_gen codegen codegen_check asnregen cleanbin mcddll_update resgen nvram_auto_gen tiny_system remake
      else
new : backup cleanall genlog cleancodegen asngen codegen codegen_check asnregen cleanbin mcddll_update resgen nvram_auto_gen tiny_system remake
      endif    
    endif
  else
    ifneq ($(filter UMTS HSPA TDD128 TDD128DPA TDD128HSPA,$(strip $(call Upper,$(PROJECT)))),)
...
new : backup cleanall genlog cleancodegen asngen codegen codegen_check asnregen cleanbin mcddll_update resgen nvram_auto_gen tiny_system remake
  ...
  ifeq ($(strip $(NEED_BUILD_BOOTLOADER)),TRUE)
    ifneq ($(filter UMTS HSPA TDD128 TDD128DPA TDD128HSPA,$(strip $(call Upper,$(PROJECT)))),)
new : backup cleanall genlog sysgen cleancodegen asngen umts_gen ss_lcs_gen codegen codegen_check asnregen resgen bootloader tiny_system update
    else
new : backup cleanall genlog sysgen cleancodegen asngen codegen codegen_check asnregen resgen bootloader tiny_system update  
    endif
  else
    ifneq ($(filter UMTS HSPA TDD128 TDD128DPA TDD128HSPA,$(strip $(call Upper,$(PROJECT)))),)
new : backup cleanall genlog sysgen cleancodegen asngen umts_gen ss_lcs_gen codegen codegen_check asnregen tiny_system update
    else
new : backup cleanall genlog sysgen cleancodegen asngen codegen codegen_check asnregen tiny_system update
    endif
  endif
endif
endif # end of BUILD_UBIN_ONLY
...
ifeq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
  ifeq ($(strip $(CUSTOM_RELEASE)),TRUE)
update : backup ubin_checklist genlog codegen resgen cksysdrv_slim remake
  else
update : backup ubin_checklist genlog codegen resgen mmi_obj_check cksysdrv_slim remake
  endif
else
...
ifeq ($(strip $(MODIS_CONFIG)),FALSE)
  ifeq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
remake : backup ubin_checklist mcp_check genlog genubinverno libs ubin cmmgen cfggen catgen done
  else
    ifneq ($(strip $(LINT)),TRUE)
...
ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
	@echo $(MBIN_LNKOPT)>> make\~libs.tmp
endif
...
ifeq ($(strip $(MBIN_WITH_ALL_LIBS)),TRUE)
	@for /d %%i in ($(COMPLIST)) do \
		@if /I "%%i" NEQ "rompatch" \
			(echo $(COMPLIBDIR)\%%i.lib >> make\~libs.tmp)
else
	@for /d %%i in ($(filter-out $(UBIN_COMPLIST),$(COMPLIST))) do \
		@if /I "%%i" NEQ "rompatch" \
			(echo $(COMPLIBDIR)\%%i.lib >> make\~libs.tmp)
endif
...
ifneq ($(strip $(MBIN_MTK_LIBS)),)
	@tools\strcmpex.exe abc abc e make\~libs_2.tmp $(MBIN_MTK_LIBS)
...
ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
  ifeq ($(strip $(MBIN_WITH_ALL_LIBS)),TRUE)
	@echo $(UBIN_LNKOPT)>> make\~libs.tmp
  else
	@if exist $(strip $(COMPLOGDIR))\keep_ubin_sym_list.log (del /q $(strip $(COMPLOGDIR))\keep_ubin_sym_list.log)
	@if exist $(strip $(MBIN_KEEP_UBIN_SYMBOL_LIST)) \
		perl tools\keep_ubin_symbol.pl $(strip $(MBIN_KEEP_UBIN_SYMBOL_LIST)) $(strip $(COMPLOGDIR))\keep_ubin_sym_list.log $(INFOMAKELOG) $(COMPLIBDIR) $(CUS_MTK_LIB) $(CUSTOM_RELEASE)> $(strip $(COMPLOGDIR))\keep_ubin_symbol.log 2>&1 & \
		(if ERRORLEVEL 1 (echo Error in tools\keep_ubin_symbol.pl. Please check $(strip $(COMPLOGDIR))\keep_ubin_symbol.log & exit 1))
	@if exist $(strip $(COMPLOGDIR))\keep_ubin_sym_list.log (type $(strip $(COMPLOGDIR))\keep_ubin_sym_list.log >> make\~libs.tmp)
  endif
endif
...
ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
$(BIN_FILE): LINK POSTBUILD_CHECK ubin
else
...
ifdef UPDATE_BINARY_FILE
ifneq ($(strip $(UPDATE_BINARY_FILE)),TRUE)
  ifeq ($(strip $(CUSTOM_RELEASE)),TRUE)
    ifeq ($(strip $(MTK_SUBSIDIARY)),TRUE)
POSTBUILD_CHECK: irc
    endif
...
ifeq ($(strip $(DCM_COMPRESSION_MAUI_INIT)),TRUE)
	@perl tools\MAUIINIT_IN_DCM_post_parsing.pl $(strip $(TARGDIR))\$(LIS_FILE) $(strip $(TARGDIR))\$(TARGNAME).txt > $(strip $(COMPLOGDIR))\MAUIINIT_in_DCM_post_parsing.log 2>&1 &\
	(if ERRORLEVEL 1 (echo Error: Failed in tools\MAUIINIT_IN_DCM_post_parsing.pl. Please check $(strip $(COMPLOGDIR))\MAUIINIT_in_DCM_post_parsing.log & exit 1))
endif

ifeq ($(strip $(NEED_BUILD_TINY_SYSTEM)),TRUE)
	@perl tools\chkCSCI.pl $(strip $(TARGDIR))\$(LIS_FILE) $(strip $(TARGDIR))\$(TS_PREFIX).lis $(PLATFORM)> $(strip $(COMPLOGDIR))\chkCSCI.log 2>&1 & \
...
ifneq ($(filter $(strip $(PLATFORM)),$(SV5_PLATFORM)),)
	@if not exist $(strip $(TARGDIR))\$(strip $(BIN_FILE))\ROM \
		...
		(perl tools\gfh_process.pl make\~gfh_files.tmp make\~gfh_cfg.tmp $(strip $(THE_MF)) $(strip $(GFH_PROCESS_TYPE)) > $(strip $(COMPLOGDIR))\gfh_process.log 2>&1) & \
...
ifeq ($(strip $(NEED_BUILD_TINY_SYSTEM)),TRUE)
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE))\ROM \
		...
		(if exist $(strip $(TARGDIR))\$(strip $(TS_BIN_FILE))\TS_ROM3 \
			copy /y $(strip $(TARGDIR))\$(strip $(TS_BIN_FILE))\TS_ROM3 $(strip $(TARGDIR))\$(strip $(BIN_FILE))\TS_ROM3 >NUL) \
	else \
		(if exist $(strip $(TARGDIR))\$(strip $(TS_BIN_FILE))\TS_ROM1 \
		...
		(if exist $(strip $(TARGDIR))\$(strip $(TS_BIN_FILE))\TS_ROM3 \
			copy /y $(strip $(TARGDIR))\$(strip $(TS_BIN_FILE))\TS_ROM3 $(strip $(TARGDIR))\TS_ROM3 >NUL)
endif
...
ifneq ($(filter $(strip $(SV5_PLATFORM)),$(PLATFORM)),)
	...
	@if exist $(strip $(TARGDIR))\$(FOTA_BIN_FILE) \
		...
		(perl tools\gfh_process.pl make\~gfh_files.tmp make\~gfh_cfg.tmp $(strip $(THE_MF)) $(strip $(GFH_PROCESS_TYPE)) > $(strip $(COMPLOGDIR))\gfh_process_fota.log 2>&1) & \
...
ifneq ($(filter $(strip $(PLATFORM)),$(SV5_PLATFORM)),)
	...
	@perl tools\gfh_process.pl make\~gfh_files.tmp make\~gfh_cfg.tmp $(strip $(THE_MF)) $(strip $(GFH_PROCESS_TYPE)) > $(strip $(COMPLOGDIR))\gfh_process_bl.log 2>&1 & \
...
ifneq ($(filter $(strip $(PLATFORM)),$(SV5_PLATFORM)),)
	...
	@perl tools\gfh_process.pl make\~gfh_files.tmp make\~gfh_cfg.tmp $(strip $(THE_MF)) $(strip $(GFH_PROCESS_TYPE)) > $(strip $(COMPLOGDIR))\gfh_process_extbl.log 2>&1 & \
...
# *************************************************************************
# Generate UBIN VersionNo
# *************************************************************************
genubinverno:
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_S,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))

	@if not exist $(VERNODIR) md $(VERNODIR)
	@if exist $(strip $(VERNODIR))\ubin_verno.c (del $(strip $(VERNODIR))\ubin_verno.c)
	
	@echo #include "kal_release.h" > $(strip $(VERNODIR))\ubin_verno.c
	@echo kal_char* ubin_customer_name(void) >> $(strip $(VERNODIR))\ubin_verno.c
	@echo { >> $(strip $(VERNODIR))\ubin_verno.c
	@echo    static kal_char ubin_customer_name[] = "$(strip $(FULL_PRJ_NAME))"; >> $(strip $(VERNODIR))\ubin_verno.c
	@echo    return ubin_customer_name; >> $(strip $(VERNODIR))\ubin_verno.c
	@echo } >> $(strip $(VERNODIR))\ubin_verno.c
	
	@echo kal_char* ubin_release_verno(void) >> $(strip $(VERNODIR))\ubin_verno.c
	@echo { >> $(strip $(VERNODIR))\ubin_verno.c
	@echo    static kal_char ubin_release_verno[] = "$(strip $(BRANCH))"; >> $(strip $(VERNODIR))\ubin_verno.c
	@echo    return ubin_release_verno; >> $(strip $(VERNODIR))\ubin_verno.c
	@echo } >> $(strip $(VERNODIR))\ubin_verno.c
	
	@echo kal_char* ubin_build_date_time(void) >> $(strip $(VERNODIR))\ubin_verno.c
	@echo { >> $(strip $(VERNODIR))\ubin_verno.c
	@echo    static kal_char ubin_build_date_time[] = "$(strip $(DBG_BUILD_DATE_TIME))"; >> $(strip $(VERNODIR))\ubin_verno.c
	@echo    return ubin_build_date_time; >> $(strip $(VERNODIR))\ubin_verno.c
	@echo } >> $(strip $(VERNODIR))\ubin_verno.c
	
	@echo #define UBIN_BUILD_DATE_TIME_STR "$(strip $(DBG_BUILD_DATE_TIME))" > $(strip $(VERNODIR))\ubin_verno.h
...
ifneq ($(strip $(NEED_BUILD_TINY_SYSTEM)),TRUE)
	@if exist tools\CMMAutoGen.pl  \
	(perl tools\CMMAutoGen.pl 0 $(FIXPATH)\$(strip $(TARGNAME))_nocode.cmm $(strip $(TARGDIR))\$(TARGNAME).elf $(strip $(THE_MF)) $(strip $(BIN_FILE)) ~lis_temp "$(CC)" "$(VIA)" make\~customIncDef.tmp $(strip $(INSIDE_MTK)) $(strip $(TARGDIR))\$(UBIN_PREFIX).elf > $(strip $(COMPLOGDIR))\cmmgen_nocode.log) & \
	(if ERRORLEVEL 1 echo Error: generate CMM file Failed. Please check $(strip $(COMPLOGDIR))\cmmgen_nocode.log & exit 1)
else
	@if exist tools\CMMAutoGen.pl  \
	(perl tools\CMMAutoGen.pl 0 $(FIXPATH)\$(strip $(TARGNAME))_nocode.cmm $(strip $(TARGDIR))\$(TARGNAME).elf $(strip $(THE_MF)) $(strip $(BIN_FILE)) ~lis_temp "$(CC)" "$(VIA)"  make\~customIncDef.tmp $(strip $(INSIDE_MTK)) $(strip $(TARGDIR))\$(UBIN_PREFIX).elf $(strip $(TARGDIR))\$(TS_PREFIX).elf > $(strip $(COMPLOGDIR))\cmmgen_nocode.log) & \
	(if ERRORLEVEL 1 echo Error: generate CMM file Failed. Please check $(strip $(COMPLOGDIR))\cmmgen_nocode.log & exit 1)
endif
ifeq ($(strip $(RUN_CMMGEN_HQ)),TRUE)
	@if exist tools\CMMAutoGen.pl  \
	(perl tools\CMMAutoGen.pl 1 $(FIXPATH)\$(strip $(TARGNAME))_HQ_Only.cmm $(strip $(TARGDIR))\$(TARGNAME).elf $(strip $(THE_MF)) $(strip $(BIN_FILE)) ~lis_temp "$(CC)" "$(VIA)" make\~customIncDef.tmp $(strip $(INSIDE_MTK)) $(strip $(TARGDIR))\$(UBIN_PREFIX).elf > $(strip $(COMPLOGDIR))\cmmgen_hqonly.log) & \
  ...
  ifdef UPDATE_BINARY_FILE
    ifeq ($(strip $(PARTIAL_SOURCE)),TRUE)
		@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE)) \
			(perl $(FIXPATH)\tools\cfgGen.pl 1 $(strip $(TARGDIR)) $(strip $(BIN_FILE)) $(strip $(BTLD_BIN_FILE)) $(strip $(FOTA_BIN_FILE)) $(strip $(UBIN_BIN_FILE)) $(strip $(THE_MF)) $(FIXPATH)\custom\system\$(strip $(BOARD_VER)) $(SCATTERFILE) "$(CC)" "$(VIA)" make\~customIncDef.tmp "$(strip $(NET_PATH))" -sign> $(strip $(COMPLOGDIR))\cfggen.log)
    else
		@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE)) \
			(perl $(FIXPATH)\tools\cfgGen.pl 1 $(strip $(TARGDIR)) $(strip $(BIN_FILE)) $(strip $(BTLD_BIN_FILE)) $(strip $(FOTA_BIN_FILE)) $(strip $(UBIN_BIN_FILE)) $(strip $(THE_MF)) $(FIXPATH)\custom\system\$(strip $(BOARD_VER)) $(SCATTERFILE) "$(CC)" "$(VIA)" make\~customIncDef.tmp "$(strip $(NET_PATH))" > $(strip $(COMPLOGDIR))\cfggen.log)
    endif
  else
    ifeq ($(strip $(PARTIAL_SOURCE)),TRUE)
		@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE)) \
...
ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
	@echo NEED_BUILD_UPDATE_BINARY = $(strip $(NEED_BUILD_UPDATE_BINARY)) >>$(INFOMAKELOG)
	@echo UBIN_COMPLIST = $(strip $(UBIN_COMPLIST)) >>$(INFOMAKELOG)
	@echo BUILD_UBIN_ONLY = $(BUILD_UBIN_ONLY) >> $(INFOMAKELOG)
endif
...
ifeq ($(strip $(ACTION)),new)
  ifeq ($(strip $(CUSTOM_RELEASE)),TRUE)
done: gen_modlibtbl end adups_verno send
  else
done: end adups_verno send
  endif
else
done: end adups_verno send
endif
...
ifeq ($(strip $(NEED_BUILD_TINY_SYSTEM)),TRUE)
	@if not exist $(strip $(TARGDIR))\$(strip $(TS_BIN_FILE)) \
		(echo $(strip $(TARGDIR))\$(strip $(TS_BIN_FILE)) does not exist. Please check it. & \
		(if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE))\NUL \
		(rd /S /Q $(strip $(TARGDIR))\$(strip $(BIN_FILE)))) & \
		(if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE)) \
		(del /Q /F $(strip $(TARGDIR))\$(strip $(BIN_FILE)))) & \
...
BUILD_DATE_TIME:=$(BUILD_DATE_TIME2_TMP)
	

ADUPS_SW_RELEASE_VERNOS:= ADUPS_$(strip $(ADUPS_FOTA_SERVICE_MODELS))_$(strip $(BUILD_DATE_TIME))

ADUPS_INFO_FILE:=$(strip $(TARGDIR))\$(ADUPS_SW_RELEASE_VERNOS)\adups_info.txt

adups_verno:
	@echo --------ADUPS BUILD VERNO START---------
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE)) \
		@if not exist $(strip $(TARGDIR))\$(ADUPS_SW_RELEASE_VERNOS) (md  $(strip $(TARGDIR))\$(ADUPS_SW_RELEASE_VERNOS) )& \
			(copy /y $(strip $(TARGDIR))\$(strip $(BIN_FILE))  $(strip $(TARGDIR))\$(ADUPS_SW_RELEASE_VERNOS)\adups.bin > nul) & \
			
	@if not exist  ADUPS_INFO_FILE (@perl -e "open F,'>adups_info.txt';open ADUPS,'<$(strip $(FIXPATH))\adups\hal\include\adups_define.h';while (my $$line=<ADUPS>) {if($$line =~ /#define ADUPS_FOTA/) {print F $$line ;}} close ADUPS;print F 'version=$(VERNO)_$(strip $(BUILD_DATE_TIME))'; close F;system('move /Y adups_info.txt $(strip $(TARGDIR))\$(ADUPS_SW_RELEASE_VERNOS)');")


	@echo --------ADUPS BUILD VERNO END-----------
#adups bin floder end
send:
ifeq ($(strip $(PARTIAL_SOURCE)),TRUE)
...
backup : getoptions
   # mbis time probe
	...
	@if exist $(strip $(TARGDIR))\TINYSYSTEM_DEBUG (rd /S /Q $(strip $(TARGDIR))\TINYSYSTEM_DEBUG)
	@if exist $(strip $(TARGDIR))\UBIN_DEBUG (rd /S /Q $(strip $(TARGDIR))\UBIN_DEBUG)
   # mbis time probe
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_E,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))
else
backup:
...
ifdef UPDATE_BINARY_FILE
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE)) \
		(perl $(FIXPATH)\tools\cfgGen.pl 2 $(strip $(TARGDIR)) $(strip $(BIN_FILE)) $(strip $(BTLD_BIN_FILE)) $(strip $(FOTA_BIN_FILE)) $(strip $(UBIN_BIN_FILE)) $(strip $(THE_MF)) $(FIXPATH)\custom\system\$(strip $(BOARD_VER)) $(SCATTERFILE) "$(CC)" "$(VIA)" make\~customIncDef.tmp "$(strip $(NET_PATH))" > $(strip $(COMPLOGDIR))\ximgen_1.log)
else
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE)) \
...
ifdef UPDATE_BINARY_FILE
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE)) \
		(perl $(FIXPATH)\tools\cfgGen.pl 2 $(strip $(TARGDIR)) $(strip $(BIN_FILE)) $(strip $(BTLD_BIN_FILE)) $(strip $(FOTA_BIN_FILE)) $(strip $(UBIN_BIN_FILE)) $(strip $(THE_MF)) $(FIXPATH)\custom\system\$(strip $(BOARD_VER)) $(SCATTERFILE) "$(CC)" "$(VIA)" make\~customIncDef.tmp "$(strip $(NET_PATH))" > $(strip $(COMPLOGDIR))\$@_1.log) & \
