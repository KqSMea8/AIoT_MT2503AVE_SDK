...
COMPLIST += mtd system gfx_drv gfx_core16 gfx_core32 peripheral usb_driver ap_tssec
...
GPS_LLE_SUPPORT_PLATFORM = MT2502
ifeq ($(strip $(GPS_LLE_SUPPORT)),TRUE)
  ifeq ($(filter $(strip $(PLATFORM)),$(GPS_LLE_SUPPORT_PLATFORM)),)
    $(warning ERROR: PLATFORM $(strip $(PLATFORM)) does NOT support GPS_LLE_SUPPORT)
    DEPENDENCY_CONFLICT = TRUE
  endif
  ifeq ($(strip $(GPS_ADAPTOR_SUPPORT)),FALSE)
    $(call DEP_ERR_ONA_OR_OFFB,GPS_ADAPTOR_SUPPORT,GPS_LLE_SUPPORT)
    DEPENDENCY_CONFLICT = TRUE
  endif
endif
ifdef CUSTOM_GPS_FEATURE_SWITCH
  ifdef GPS_SUPPORT
    ifneq ($(strip $(GPS_SUPPORT)),NONE)
      ifeq ($(strip $(CUSTOM_GPS_FEATURE_SWITCH)),TRUE)
        COM_DEFS += __CUSTOM_GPS_FEATURE_SWITCH__
      endif
  ...
  ifeq ($(strip $(GPS_SUPPORT)),NONE)
    ifeq ($(strip $(CUSTOM_GPS_FEATURE_SWITCH)),TRUE)
      $(warning ERROR: Please turn off CUSTOM_GPS_FEATURE_SWITCH when GPS_SUPPORT is NONE.)
      DEPENDENCY_CONFLICT = TRUE
    endif
  else
    ifeq ($(strip $(CUSTOM_GPS_FEATURE_SWITCH)),FALSE)
      $(warning ERROR: Please turn on CUSTOM_GPS_FEATURE_SWITCH when GPS_SUPPORT is not NONE.)
      DEPENDENCY_CONFLICT = TRUE
    endif
  endif
else
...
ifdef MOTION_SENSOR_SUPPORT
  ifneq ($(strip $(MOTION_SENSOR_SUPPORT)),NONE)
    ...
    ifeq ($(strip $(MOTION_SENSOR_SUPPORT)),DA213_I2C)
      COM_DEFS += __MOTION_SENSOR_ADVANCED_GESTURE__
      COMPOBJS +=  custom\drv\motion_sensor\DA213_I2C\carMotion.lib
      CUS_REL_OBJ_LIST += custom\drv\motion_sensor\DA213_I2C\carMotion.lib
      CUS_REL_BASE_COMP += custom\drv\motion_sensor\DA213_I2C
    endif    
  endif
endif

ifdef OZ105_SWCHR_SUPPORT
  COM_DEFS += __OZ105_SWCHR_SUPPORT__
endif

ifdef BQ25601_SWCHR_SUPPORT
  COM_DEFS += __BQ25601_SWCHR_SUPPORT__
endif
...
ifdef EDGE_SUPPORT
  ifeq ($(strip $(EDGE_SUPPORT)),TRUE)
    EDGE_SUPPORT_PLATFORM = MT6229 MT6230 MT6238 MT6239 MT6235 MT6235B MT6236 MT6236B MT6268 TK6516 MT6516 MT6268T MT6268H MT6268A MT6268 MT6270A MT6256 MT6276 MT6573 MT6921 MT6255 MT6922 MT6250 MT6260 MT2501 MT2502
...
ifdef ACCDET_SUPPORT
  ...
  ifeq ($(strip $(ACCDET_SUPPORT)),DISABLE_EARPHONE)
    COM_DEFS    +=  __EARPHONE_DISABLE__
  endif
  ifeq ($(strip $(ACCDET_SUPPORT)),FALSE)
    ACCDET_CANNOT_BE_FALSE_PLATFORM := MT6260 MT6261 MT2501 MT2502
    ifneq ($(filter $(strip $(PLATFORM)),$(ACCDET_CANNOT_BE_FALSE_PLATFORM)),)
      #error on MT6260 and post-MT6260 chip
      $(warning ERROR: PLATFORM $(strip $(PLATFORM)) does not support ACCDET_SUPPORT=FALSE.)
      DEPENDENCY_CONFLICT = TRUE
    endif
  endif
endif
...
ifeq ($(strip $(KAL_RECORD_BOOTUP_LOG)),TRUE)
  ...
  ifneq ($(filter __DUAL_TALK_MODEM_SUPPORT__ ,$(COM_DEFS)),)
    ifeq ($(strip $(USB_BOOTUP_TRACE)),TRUE)
      $(warning ERROR: Please disable USB_BOOTUP_TRACE when __DUAL_TALK_MODEM_SUPPORT__ is defined in CUSTOM_OPTION!)
      DEPENDENCY_CONFLICT = TRUE
    endif
    COM_DEFS += __KAL_RECORD_BOOTUP_LOG__
  endif
...
ifdef MMI_VERSION
  ifeq ($(strip $(MMI_VERSION)),IOT_MMI)
    ifdef IOT_LCD_SUPPORT
      ifeq ($(strip $(IOT_LCD_SUPPORT)),NONE)
        COM_DEFS := $(filter-out __LCD_SUPPORT__,$(COM_DEFS))
      endif
      ifeq ($(strip $(IOT_LCD_SUPPORT)),SLIM)
        COM_DEFS  += __DISPLAY_LEVEL_GDI_ONLY__
      endif
      ifeq ($(strip $(IOT_LCD_SUPPORT)),FULL)
        COM_DEFS  += __DISPLAY_LEVEL_FULL__
      endif
    else
      COM_DEFS := $(filter-out __LCD_SUPPORT__,$(COM_DEFS))
    endif
...
ifdef BW_FLATTEN_ONE_LAYER
  ifeq ($(strip $(BW_FLATTEN_ONE_LAYER)),TRUE)
    COM_DEFS    += __BW_FLATTEN_ONE_LAYER__
  endif
endif
