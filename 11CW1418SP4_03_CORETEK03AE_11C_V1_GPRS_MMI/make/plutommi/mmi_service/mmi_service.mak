...
ifneq ($(filter __MMI_UNIFIEDMESSAGE_APP_SUPPORT__, $(strip $(MODULE_DEFS))),)               
...
SRC_LIST += plutommi\service\CertmanSrv\certManAgentSrv.c \
...
#ifneq ($(filter __IOT__ , $(strip $(MODULE_DEFS))),)
     SRC_LIST += plutommi\Service\DateTimeSrv\DateTimeSrv.c \
     			 plutommi\Service\SettingSrv\CommSettings.c \
           		 plutommi\Service\SettingSrv\CommSettingSpof.c
#endif

ifneq ($(filter __FUNET_ENABLE__ , $(strip $(MODULE_DEFS))),)
    SRC_LIST += plutommi\Service\FUNetSrv\FUnetSrv.c
endif

ifneq ($(filter __FUDIFFNET_ENABLE__ , $(strip $(MODULE_DEFS))),)
    SRC_LIST += plutommi\Service\FUDiffNetSrv\FUDiffNetSrv.c
endif
...
ifneq ($(filter __MMI_PHB_SUPPORT__ , $(strip $(MODULE_DEFS))),)
...
SRC_LIST += plutommi\Service\UcmSrv\UcmSrvAtHdlr.c \
...
ifneq ($(filter __MMI_SETTINGS_SUPPORT__ , $(strip $(MODULE_DEFS))),)             
SRC_LIST += plutommi\Service\SettingSrv\PenCalibrationSrv.c \
           plutommi\Service\SettingSrv\GeneralSettingSrv.c \
           plutommi\Service\SettingSrv\RestoreSrv.c 
endif

ifneq ($(filter __MMI_ORGANIZER_SUPPORT__ , $(strip $(MODULE_DEFS))),)           
SRC_LIST += plutommi\Service\ReminderSrv\ReminderSrv.c \
           plutommi\Service\ReminderSrv\ReminderSrvUtil.c \
           plutommi\Service\ReminderSrv\ReminderSrvQueue.c  
endif

ifneq ($(filter __MMI_ORGANIZER_SUPPORT__ , $(strip $(MODULE_DEFS))),) 
SRC_LIST += plutommi\Service\AlarmSrv\AlarmSrv.c
endif

SRC_LIST += plutommi\Service\EonsSrv\EonsSrvMain.c \
...
ifeq ($(filter __IOT_NO_MULTIMEDIA__ , $(strip $(MODULE_DEFS))),) 
SRC_LIST += plutommi\service\mdi\mdisrc\mdi_audio.c \
...
ifneq ($(filter __MMI_TELEPHONY_SUPPORT__ , $(strip $(MODULE_DEFS))),) 
SRC_LIST += plutommi\Service\BootupSrv\BootupSrvProtocol.c \
...
ifneq ($(filter __MMI_BT_DIALER_SUPPORT__, $(strip $(MODULE_DEFS))),)
    SRC_LIST += plutommi\Service\BtcmSrv\BtcmSrvDialerInterface.c
endif

ifneq ($(filter __BT_4_0_BLE__, $(strip $(MODULE_DEFS))),)
    SRC_LIST += plutommi\Service\BLEcmSrv\BLEcmSrvInterface.c
    SRC_LIST += plutommi\Service\BLEcmSrv\BLEcmSrvPrmt.c
    SRC_LIST += plutommi\Service\BLEcmSrv\BLEcmSrvMain.c
endif

    ifneq ($(filter __BT_DIALER_SUPPORT__, $(strip $(MODULE_DEFS))),)
        SRC_LIST += plutommi\Service\DialerSrv\BTDialerSrv.c
        SRC_LIST += plutommi\Service\UcmSrv\UcmBTSrv.c
    endif  
    
    ifneq ($(filter __MMI_BT_MAP_CLIENT__, $(strip $(MODULE_DEFS))),)
        SRC_LIST += plutommi\Service\MapcSrv\MapcSrv.c \
                    plutommi\Service\MapcSrv\MapObjSrv.c \
                    plutommi\Service\MapcSrv\MapcSrvAdap.c         
    endif                
    ...
    ifneq ($(filter __MMI_A2DP_SUPPORT__, $(strip $(MODULE_DEFS))),)
        ...
        SRC_LIST += plutommi\Service\A2dpSrv\A2dpSrvUtil.c
    endif
    ifneq ($(filter __MMI_AVRCP_SUPPORT__, $(strip $(MODULE_DEFS))),)
        SRC_LIST += plutommi\Service\AvrcpSrv\AvrcpSrv.c \
                    plutommi\Service\AvrcpSrv\AvrcpSrvUtil.c
    endif
endif

ifneq ($(filter __MMI_BT_NOTI_SRV__, $(strip $(MODULE_DEFS))),)
    SRC_LIST += plutommi\Service\BTPushRecvSrv\BTPushRecvSrv.c
    ifneq ($(filter __FUDIFFNET_ENABLE__, $(strip $(MODULE_DEFS))),)
        SRC_LIST += plutommi\Service\BTPushRecvSrv\BTPRSrvDiffFOTA.c
    endif
    ifneq ($(filter __UPDATE_BINARY_FILE__, $(strip $(MODULE_DEFS))),)
        SRC_LIST += plutommi\Service\BTPushRecvSrv\BTPRSrvUBinFOTA.c
    endif
    ifneq ($(filter __FUNET_ENABLE__, $(strip $(MODULE_DEFS))),)
        ifeq ($(filter __UPDATE_BINARY_FILE__, $(strip $(MODULE_DEFS))),)
            SRC_LIST += plutommi\Service\BTPushRecvSrv\BTPRSrvFullBinFOTA.c
        endif
    endif
    ifneq ($(filter __GPS_ADAPTOR_SUPPORT__, $(strip $(MODULE_DEFS))),)
    		SRC_LIST += plutommi\Service\BTPushRecvSrv\BTPRSrvEPODown.c
    endif
endif

ifneq ($(filter __MMI_BT_NOTI_SRV__, $(strip $(MODULE_DEFS))),)
    SRC_LIST += plutommi\Service\BTNotificationSrv\BTNotiSrvExApi.c
    SRC_LIST += plutommi\Service\BTNotificationSrv\BTNotiSrvData.c
    SRC_LIST += plutommi\Service\BTNotificationSrv\BtNotifySrvParser.c   
endif    
...
ifneq ($(filter __WIFI_SCAN_ONLY__, $(strip $(MODULE_DEFS))),)
    SRC_LIST += plutommi\service\WlanSrv\ScanOnly\wlansrvscanonly.c 
    ifneq ($(filter __WIFI_SCAN_ONLY_AT_CMD__, $(strip $(MODULE_DEFS))),)
        SRC_LIST += plutommi\Service\WlanSrv\ScanOnly\wlansrvscanonly_atcmd.c
    endif
else
  SRC_LIST += plutommi\Service\WlanSrv\WlanSrvApi.c
  ifneq ($(filter __TCPIP__, $(strip $(MODULE_DEFS))),)
    ifneq ($(filter __MMI_WLAN_FEATURES__, $(strip $(MODULE_DEFS))),)
      SRC_LIST += plutommi\Service\WlanSrv\WlanSrvUtil.c    
    endif
  endif
endif
...
ifneq ($(filter __MMI_TELEPHONY_SUPPORT__, $(strip $(MODULE_DEFS))),)
  ...
  ifneq ($(filter __MMI_CLOG_SLIM_FOR_MT6261__, $(strip $(MODULE_DEFS))),)
    SRC_LIST += plutommi\Service\CallLogSrv\CallLogSrvCompEn.c \
           plutommi\Service\CallLogSrv\CallLogSrvIdxTblEn.c \
           plutommi\Service\CallLogSrv\CallLogSrvIfEn.c \
           plutommi\Service\CallLogSrv\CallLogSrvMainEn.c \
           plutommi\Service\CallLogSrv\CallLogSrvPhbSyncEn.c \
           plutommi\Service\CallLogSrv\CallLogSrvPsAdpEn.c \
           plutommi\Service\CallLogSrv\CallLogSrvSiTblEn.c \
           plutommi\Service\CallLogSrv\CallLogSrvStoreEn.c \
           plutommi\Service\CallLogSrv\CallLogSrvUtilsEn.c
  else
    SRC_LIST += plutommi\Service\CallLogSrv\CallLogSrvAtCmd.c \
           plutommi\Service\CallLogSrv\CallLogSrvSimSync.c \
...
SRC_LIST += plutommi\Service\GattSrv\GattSrv.c

ifneq ($(filter __BT_GATTS_PROFILE__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Service\GattSrv\GattsSrv.c \
            plutommi\Service\GattSrv\GattServicesFactorySrv.c \
            plutommi\Service\GattSrv\pxp\PrxrSrv.c \
            plutommi\Service\GattSrv\fmp\FmptSrv.c \
            plutommi\Service\GattSrv\sogp\SogpsSrv.c \
            plutommi\Service\GattSrv\sogp\SogpsAdpSrv.c \
            plutommi\Service\GattSrv\pedometer\PdmsSrv.c \
            plutommi\Service\GattSrv\bas\BasSrv.c    
endif

ifneq ($(filter __BT_GATTC_PROFILE__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Service\GattSrv\GattcSrv.c \
            plutommi\Service\GattSrv\fmp\FmplSrv.c \
            plutommi\Service\GattSrv\pxp\PrxmSrv.c \
            plutommi\Service\GattSrv\Anpc\AnpcSrv.c \
            plutommi\Service\GattSrv\tip\TimeSrv.c \
            plutommi\Service\GattSrv\tip\TipcSrv.c
endif

ifneq ($(filter __MMI_BLE_ANCS_SUPPORT__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Service\BTNotificationANCS\BTNotiAncsSrvExt.c
INC_DIR += plutommi\Service\BTNotificationANCS
endif

ifneq ($(filter __MMI_ONE_KEY_SOS_SUPPORT__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Service\OneKeySosSrv\OneKeySosSrv.c
INC_DIR += plutommi\Service\OneKeySosSrv
endif
...
ifeq ($(filter __BT_MUSIC_REMOVE__, $(strip $(MODULE_DEFS))),)        
ifneq ($(filter __BT_GATTC_PROFILE__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Service\BtmVolSrv\BmvSrv.c
endif
endif
...
INC_DIR = hal\video\demuxer\provider\inc \
          ...
          hal\system\fudiffnet\inc \
          hal\system\bootloader\inc \
          hal\system\GFH\public \
          sss\interface\inc \
          plutommi\Service\GattSrv \
          plutommi\Service\BTPushRecvSrv \
          plutommi\Service\FUDiffNetSrv \
          plutommi\Service\FUNetSrv \
          plutommi\Service\GattSrv\Anpc \
          plutommi\Service\GattSrv\tip \
          plutommi\Service\GattSrv\pxp \
          plutommi\Service\GattSrv\fmp \
          plutommi\Service\GattSrv\sogp \
          plutommi\Service\GattSrv\pedometer \
          plutommi\Service\GattSrv\bas

ifneq ($(filter __TINY_SYS__, $(strip $(MODULE_DEFS))),)
...
#ifneq ($(filter __IOT__ , $(strip $(MODULE_DEFS))),)
INC_DIR += plutommi\Service\DateTimeSrv
#endif

ifneq ($(filter __WIFI_SCAN_ONLY__, $(strip $(MODULE_DEFS))),)
INC_DIR += plutommi\Service\WlanSrv\ScanOnly
endif
...
ifneq ($(filter __MMI_AUDPLY_WALK_PLAY__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Service\PlaylistcacheSrv\playlistcacheSrv.c \
           plutommi\Service\PlaylistcacheSrv\plcacheSrv_agent_audply.c
endif

ifneq ($(filter __IOT__, $(strip $(MODULE_DEFS))),)
ifneq ($(filter __BT_GATTS_PROFILE__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Service\SrvApp\fmpt_srv_app.c \
           plutommi\Service\SrvApp\pxp_srv_app.c \
           plutommi\Service\SrvApp\bt_demo_mode_switch.c \
           plutommi\Service\SrvApp\gatt_demo_client_app.c
INC_DIR += plutommi\Service\SrvApp
endif
endif


ifneq ($(filter __BT_GATTS_PROFILE__, $(strip $(MODULE_DEFS))),)
INC_DIR += plutommi\Service\BtmVolSrv
endif
