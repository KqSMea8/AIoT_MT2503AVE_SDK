...
static kal_bool vsm_init = KAL_TRUE;

static kal_uint8 vsm_eint_num;
cmd_event_t vsm_event;
#ifdef ENABLE_DISABLE_TEST
kal_bool ppg_first_use = KAL_TRUE;
#endif
    ...
    switch (command) {
        case SENSOR_DELAY:
            if ((buff_in == NULL) || (size_in < sizeof(int32_t))) {
                ...
                value = *(int32_t *)buff_in;
                LOGI("BIO BISI SENSOR_DELAY ms (%d) \r\n", value);
            }
            break;
    ...
    switch (command) {
        case SENSOR_DELAY:
            if ((buff_in == NULL) || (size_in < sizeof(int32_t))) {
                ...
                value = *(int32_t *)buff_in;
                LOGI("BIO EEG SENSOR_DELAY ms (%d) \r\n", value);
            }
            break;
    ...
    switch (command) {
        case SENSOR_DELAY:
            if ((buff_in == NULL) || (size_in < sizeof(int32_t))) {
                ...
                value = *(int32_t *)buff_in;
                LOGI("BIO EMG SENSOR_DELAY ms (%d) \r\n", value);
            }
            break;
    ...
    switch (command) {
        case SENSOR_DELAY:
            if ((buff_in == NULL) || (size_in < sizeof(int32_t))) {
                ...
                value = *(int32_t *)buff_in;
                LOGI("BIO GSR SENSOR_DELAY ms (%d) \r\n", value);
            }
            break;
    ...
    switch (command) {
        case SENSOR_DELAY:
            if ((buff_in == NULL) || (size_in < sizeof(int32_t))) {
                ...
                value = *(int32_t *)buff_in;
                LOGI("BIO PPG1_512HZ SENSOR_DELAY ms (%d) \r\n", value);
            }
            break;
        ...
        case SENSOR_ENABLE:
            if ((buff_in == NULL) || (size_in < sizeof(int))) {
                ...
                value = *(int32_t *)buff_in;
                LOGI("BIO PPG1_512HZ SENSOR_ENABLE (%d) \r\n", value);
...
void vsm_eint_set(kal_uint32 eint_num)
{
    vsm_eint_num = (kal_uint8)eint_num;
}

void vsm_eint_handler(void *parameter)
{
    IRQMask(vsm_eint_num);
    IRQUnmask(vsm_eint_num);
...
vsm_status_t vsm_driver_sensor_subsys_init(void)
{
    ...
    if (err != VSM_STATUS_OK) {
		LOGE("vsm_driver_sensor_subsys_init:ERROR at vsm_driver_init (%d)", err);
        //vsm_init = KAL_FALSE;
    }
