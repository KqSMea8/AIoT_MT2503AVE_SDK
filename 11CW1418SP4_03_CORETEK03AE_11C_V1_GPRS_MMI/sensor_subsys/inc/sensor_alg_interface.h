...
typedef struct {
  ...
  int32_t bpm; /**<Heart rate measurement. Beats per minute.*/
  int32_t status; /**<Quality of the measurement, subject to fusion algorithm design.*/
...
typedef struct {
    ...
    int32_t bpm; /**<Heart rate measurement. Beats per minute.*/
    int32_t status; /**<Quality of the measurement, subject to fusion algorithm design.*/
    int32_t *pwtt; /**<The sequence of each pulse wave transition time interval between ECG and PPG.*/
    int32_t numPwtt;  /**<The total number of pwtt sequence.*/
...
*  - Step1: Call #sensor_manager_init() to initialize the sensor manager.
*  - Step2: Prepare the sensor data subscription structure. Refer to #sensor_subscriber_t.
*  - Step3: Call #sensor_subscribe_sensor() to subscribe the specified sensor in application.
*  - Sample code:
*    @code
* int32_t acc_sensor_send_digest_callback(sensor_data_t *const output);
*
* sensor_subscriber_t acc_subscriber = {
*     "ap0", 0, SENSOR_TYPE_ACCELEROMETER, 10, acc_sensor_send_digest_callback
* };
* int32_t acc_sensor_send_digest_callback(sensor_data_t *const output)
* {
...
/** @brief This function initializes the data structures insensor manager.
*   @return >=0, if the operation completed successfully.*/
