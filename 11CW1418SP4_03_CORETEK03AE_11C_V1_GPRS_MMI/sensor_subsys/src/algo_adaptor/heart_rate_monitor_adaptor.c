...
uint32_t ohrm_fs_acc_div_ppg = 1 << MOTION_PPG_RATIO_DECIMAL_POINT_BITS; // fs_acc / fs_ppg
...
static int32_t get_heart_rate_monitor_result(sensor_data_t *const output)
{
    ...
    //hrm_data->heart_rate_t.status = 0xBFFFFFFF;
...
int heart_rate_monitor_register(void)
{
    ...
    input_heart_rate_monitor_comp_acc.sampling_delay = HEART_RATE_MONITOR_ACC_POLLING_TIME; // ms
