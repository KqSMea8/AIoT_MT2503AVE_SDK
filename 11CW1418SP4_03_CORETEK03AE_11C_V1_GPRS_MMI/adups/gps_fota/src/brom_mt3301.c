	...
	while(1)
	{
		...
		if(1 == GPS_UART_GetByte_NO_TIMEOUT(&data8))
		{
			 ...
			 if(tmp8 == data8)
			 {
					...
					BL_PRINT(LOG_DEBUG, "[FUNET]First Sync: received the ACK: 0X5F PASS\n\r");
		...
		data8 = 0xDD;
		//connCOM.m_COM_PutByte(data8);
		BL_PRINT(LOG_DEBUG, "[FUNET]Received data:%x cnt=%d\n\r", data8,cnt);
		if (cnt > 10000)
		{
			data8 = 0xCC;
			//connCOM.m_COM_PutByte(data8);
			BL_PRINT(LOG_DEBUG, "[FUNET]Received data:%x cnt=%d\n\r", data8,cnt);
			BL_PRINT(LOG_DEBUG, "[FUNET]First Sync: wait > 10000, FAIL!!!\n\r");
			break;
		}
	}
	BL_PRINT(LOG_DEBUG, "[FUNET]First Sync: Method1 shanke FAIL!!! try Method2 handshake...\n\r");
	...
	for (fail_retry = 0; fail_retry < 5; ++fail_retry)
	{
		...
		*(volatile int *)(0xa0020304) = 0x00080000;
		...
		while(1)
		{
			...
			{
				 tmp8 = 0x5F;
				 BL_PRINT(LOG_DEBUG, "[FUNET-2]Received data:%x \n\r", data8); //brucezuo add
				 if(tmp8 == data8)
				 {
					//connCOM.m_COM_PutByte(data8);
					GPS_UART_PurgeFIFO();
					//break;	
					BL_PRINT(LOG_DEBUG, "[FUNET2]First Sync: received the ACK: 0X5F pass\n\r");//brucezuo add
					goto SECOND_CHAR;
				 }
				 else
				 {
			...
			if (cnt > 50000)
			{
				...
				if (fail_retry == 4)
				{
					BL_PRINT(LOG_DEBUG, "[FUNET]First Sync: Method2 handshake FAIL!!!\n\r");//brucezuo add
					break;	//while(1);
				}
				else 
				{
					BL_PRINT(LOG_DEBUG, "[FUNET]First Sync: wait > 50000, FAIL!!!\n\r");//brucezuo add
					break;
				}
			}
		}
...
SECOND_CHAR:
	...
	BL_PRINT(LOG_DEBUG, "[FUNET]Sync second char111(?==0xF5):%x \n\r", data8); //brucezuo add
	if(tmp8 != data8)
	{
		//connCOM.m_COM_PutByte(data8);	
		return -1;//while(1);
	...
	tmp8 = 0xAF;
	data8 = GPS_UART_GetByte();
	BL_PRINT(LOG_DEBUG, "[FUNET]Sync second char222(?==0xAF):%x \n\r", data8); //brucezuo add
	if(tmp8 != data8)
	{
		//connCOM.m_COM_PutByte(data8);	
		return -1;//while(1);
	...
	tmp8 = 0xFA;
	data8 = GPS_UART_GetByte();
	BL_PRINT(LOG_DEBUG, "[FUNET]Sync second char222(?==0xFA):%x \n\r", data8); //brucezuo add
	if(tmp8 != data8)
	{
		//connCOM.m_COM_PutByte(data8);	
		return -1;//while(1);
