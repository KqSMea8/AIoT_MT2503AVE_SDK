...
int CMD_SetMemBlock(GPS_Download_Arg arg, ROM_HANDLE_T  *dl_handle)
{
   ...
   if( NULL==dl_handle )
   {
	  ...
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_SetMemBlock(): invalid arguments! dl_handle(%x).\n\r", dl_handle);
      return 1;
   }
   ...
   if( ACK != buf[0] )
   {
	  ...
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_SetMemBlock(): non-ACK(%x) return.\n\r", buf[0]);
	  return 6;
   }
      ...
      CALLBACK_DOWNLOAD_PROGRESS  cb_download_flash,  void *cb_download_flash_arg)
{
   ...
   if( NULL==dl_handle )
   {
	  ...
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_WriteData(): invalid arguments! dl_handle( %x)..\n\r", dl_handle);
      return 1;
   }
   ...
   if( NULL != cb_download_flash_init )
   {
	  ...
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_WriteData(): CALLBACK: cb_download_flash_init().\n\r");
   ...
   if( 0 != (ret=ReadData(arg, buf, 1)) )
   {
	  ...
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_WriteData(): ReadData(): fail, Err(%d).\n\r", ret);
      return 4;
   }
   ...
   if( ACK != buf[0] )
   {
	  ...
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_WriteData(): %s( %x): fail to save all the unchanged data from flash!\n\r", buf[0]>UNKNOWN_ERROR?"UNKNOWN_ACK":ErrAckTable[buf[0]], buf[0]);
	  return 5;
   }
   ...
   if( ACK != buf[0] )
   {
	  ...
	  //sprintf(Dbgbuf, "DA_cmd::CMD_WriteData(): ReadData(): fail, Err(%d)\n", ret);
	  //fwrite(Dbgbuf, 1, strlen("DA_cmd::CMD_WriteData(): ReadData(): fail, Err(%d)\n"), DbgFILE); 	
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_WriteData(): %s( %x): fail to erase the 1st sector!.\n\r", buf[0]>UNKNOWN_ERROR?"UNKNOWN_ACK":ErrAckTable[buf[0]], buf[0]);
	  return 7;
   }
...
read_cont_char:
   ...
   if( ACK != buf[0] )
   {
	  ...
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_WriteData(): %s( %x): fail to recover all the unchanged data to flash!\n\r", buf[0]>UNKNOWN_ERROR?"UNKNOWN_ACK":ErrAckTable[buf[0]], buf[0]);
	  return 17;
   }
   ...
   if( NACK == buf[0] )
   {
	  ...
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_WriteData(): NACK( %x) return, flash checksum error!\n\r", buf[0]);
	  return 19;
   }
   else if( ACK != buf[0] )
   {
	  ...
	  adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]DA_cmd::CMD_WriteData(): non-ACK( %x) return.\n\r", buf[0]);
      return 20;
   }
