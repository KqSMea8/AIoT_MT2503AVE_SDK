...
#define MAX_CLCYLE_TIME 5000000
...
/* under construction !*/
		...
		recile_time ++;
		if(recile_time >= MAX_CLCYLE_TIME)
			break;
	}
}
...
void GPS_UART_SetBaudRate(uint8 baud_rate)
{
	...
	ext_clock = 4; //brucezuo adups
	BL_PRINT(LOG_DEBUG, "[FUNET]adups Ext_clock = %d\n\r", ext_clock);
