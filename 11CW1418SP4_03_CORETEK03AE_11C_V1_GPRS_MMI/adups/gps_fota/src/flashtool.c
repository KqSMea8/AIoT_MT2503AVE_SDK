	...
	GPS_Download_Arg arg)
{
	...
	{
    	adups_bl_debug_print((void *)ADUPS_LOG_MCU, "[FUNET]Input parameters error \n\r");
	   return FT_INVALID_ARGUMENTS;
	}
...
void gps_download_progress_callback(unsigned char finished_percentage,
...
adups_int32 adups_read_gps_file(adups_wchar *fullpath, adups_uint32 offset,adups_uint8* dest,adups_uint32 len) 
{
	//adups_wchar *file_path;

	void *temp_buffer=NULL;
	adups_int32 temp_file;
	adups_int32 file_size=0;
	adups_int32 ret;
	adups_int32 read_len;
	
	temp_file=adups_Open_File(fullpath, ADUPS_READWRITE);

	adups_bl_debug_print(NULL, "adups_read_gps_file:file_handle=%d\r\n",temp_file);

	if(temp_file>0)
	{
		ret=adups_Seek_File(temp_file, offset, ADUPS_SEEK_START);

		if(ret==E_ADUPS_SUCCESS)
		{
			ret=adups_Read_File(temp_file,(adups_char*)dest, len, &read_len);
			...
			if(ret>=0)
			{
				...
				if(read_len==len)
				{
					adups_bl_debug_print(NULL, "adups_read_gps_file success:read_len=%d\r\n",read_len);
					return E_ADUPS_SUCCESS;
				}
				else
				{
					adups_bl_debug_print(NULL, "adups_read_gps_file fail:read_len=%d\r\n",read_len);
	...
	adups_bl_debug_print(NULL, "adups_read_gps_file fail\r\n");
	return E_ADUPS_FAILURE;
}

int GetGpsFirewareSize()
{
	char CharByte[4] = {NULL},cycle = 0;
	int GpsFirewareSize  = 0,shift = 0;
	char ReadFlashResult = 1;
	adups_uint32 delta_base=adups_bl_get_delta_base();

		if(AdupsUseFlash()==ADUPS_TRUE)
			ReadFlashResult = Adups_read_flash(6+delta_base,CharByte,4);
		else
			ReadFlashResult=adups_read_gps_file((adups_wchar *)adups_get_delta_file_path(),6+delta_base,CharByte,4);
	
	
	for(cycle = 0;cycle < 4; cycle++)
	{
		shift = cycle*8;
		GpsFirewareSize += (CharByte[cycle]&0xFF)<<shift;
	}
	return GpsFirewareSize;
}
int HashCheck(int size,unsigned char * GpsBinptr)
{
	unsigned char hash_result_final[20]={0};
	unsigned char hashmodel[20]={0},ReadFlashResult = 1;
	adups_uint32 delta_base=adups_bl_get_delta_base();
	adups_my_sha1(GpsBinptr,size,hash_result_final,20);

		if(AdupsUseFlash()==ADUPS_TRUE)
			ReadFlashResult = Adups_read_flash(9+delta_base,hashmodel,20);
		else
			ReadFlashResult = adups_read_gps_file((adups_wchar *)adups_get_delta_file_path(),9+delta_base,hashmodel,20);	
	
	if(ReadFlashResult != 0)
		return 0;
	if(!memcmp(hash_result_final,hashmodel,20))
		return 1;
	else 
		return 0;
}
...
int GpsFotaCheck(void)
{
	char GpsFotaGign[6] = {NULL};
	char ReadFlashResult = 1;
	int adups_gps_size=0;
    	const unsigned char *gps_ptr=NULL;
	adups_uint32 total_read=0;
	adups_uint32 delta_base=adups_bl_get_delta_base();
	

	if(AdupsUseFlash()==ADUPS_TRUE)
		ReadFlashResult = Adups_read_flash(delta_base,GpsFotaGign,6);
	else
		ReadFlashResult = adups_read_gps_file((adups_wchar *)adups_get_delta_file_path(),delta_base,GpsFotaGign,6);
		

	adups_bl_debug_print(NULL,"GpsFotaCheck:GpsFotaGign=%s",GpsFotaGign);	
	if(ReadFlashResult != 0 )		
		return 0;
	
	if((GpsFotaGign[0] == 'A')&&(GpsFotaGign[1] == 'D')&&(GpsFotaGign[2] == 'M')\
		&&(GpsFotaGign[3] == 'C')\
		&&(GpsFotaGign[4] == 'U')\
		&&(GpsFotaGign[5] == '1'))
	{
		 adups_gps_size = GetGpsFirewareSize();
		 gps_ptr=(adups_uint8 *)adups_bl_get_working_buffer();
		 if(AdupsUseFlash()==ADUPS_TRUE)
			 total_read = Adups_read_flash(64+delta_base,gps_ptr,adups_gps_size);
		 else
			 total_read = adups_read_gps_file((adups_wchar *)adups_get_delta_file_path(),64+delta_base,(adups_uint8 *)gps_ptr,adups_gps_size); 		 
...
int GPS_DL()
{
	...
	gps_da.m_image = MT3333_all_in_one_da;
	gps_da.m_size = sizeof(MT3333_all_in_one_da);
	gps_da.m_path[0] = '\0';
	gps_image.m_num = 1;
	
	gps_image.m_image_list[0].m_image = (adups_uint8 *)adups_bl_get_working_buffer();
	gps_image.m_image_list[0].m_path[0] = '\0';
	gps_image.m_image_list[0].m_size = (kal_uint32)adups_bl_get_working_buffer_len();
	...
	arg.m_cb_download_conn_da_arg = (void*) 0;
       ...
       gps_image.m_image_list[0].m_size = GetGpsFirewareSize();
	BL_PRINT(LOG_DEBUG, "[FUNET]GPS DL, file gps_image.m_image_list[0].m_size = %d \n\r", gps_image.m_image_list[0].m_size);


       BL_PRINT(LOG_DEBUG, "[FUNET]Enter FlashDownload \n\r");
	ptmp = gps_image.m_image_list[0].m_image + 0x60;
	
	if (memcmp(ptmp,checkch,16) != 0)
	{
    	BL_PRINT(LOG_DEBUG, "[FUNET]GPS Firmware is not correct... \n\r");
		ret= GPS_FW_DOWNLOAD_UPLOAD_ERR; 
	}
	
	ret = FlashDownload(&gps_da, &gps_image, arg);
	BL_PRINT(LOG_DEBUG, "[FUNET]Enter FlashDownload ret is %d \n\r",ret);
	if (FT_OK != ret)
		return ret;
	
	return GPS_FW_DOWNLOAD_OK;
}
