	...
	foreach $_(@env_key)
	{
		if($_ =~ /$arm_var_name/i)
		{
			...
			for (my $i = 0; $i < @arm_var_value; $i++)
			{
				...
				{
					if (($arm_var_value[$i] =~ /^\s*C:\\ARM\\Licenses\\rvds\.dat$/i) ||
						($arm_var_value[$i] =~ /^\s*C:\\ARM\\ADSv1_2\\licenses\\license\.dat$/i))
					{
...
elsif ($ARGV[2] =~ /-customer/)
{
	...
	foreach $_(@env_key)
	{
		...
		{          
			my $arm_para = $ENV{$_};
			if (($arm_para =~ /^\s*C:\\ARM\\Licenses\\rvds\.dat/i) ||
			($arm_para =~ /^\s*C:\\Progra~1\\ARM\\ADSv1_2\\licenses\\license\.dat/i))
			{         
				if (-e "C:\\ARM\\Licenses\\rvds\.dat")
				{
		 	...
		 	{
		 		print "C:\\ARM\\Licenses\\rvds\.dat is not existed!\n";
		 		if(($Flag =~ /-customer/i) && ($log_flag == 1)){
		 		  $reading ="C:\\ARM\\Licenses\\rvds\.dat is not existed!\n";
                  print LOG $reading;
                }
		 	}
		}
