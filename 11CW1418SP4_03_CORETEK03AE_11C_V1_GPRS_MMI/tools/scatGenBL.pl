...
# This size need sync with tools/IOTFota/gen_compressed_bin.pl 
# Need >= than $splitBinSize
my $IOT_IMAGE_BUFFER_SIZE = 0x00040000; 
...
# For IOT-ROCK-FOTA
#****************************************************************************
sub GenEXT_BL_IOT_ROCK_FOTA_WORKING_BUF
{
    my $template = '';
	my $ext_bl_IOT_ROCK_FOTA_support = &$FEATURE_QUERY_FUNCTION_PTR('ROCK_FOTA_SUPPORT');
	my %BBtbl_BL_IOT_ROCK_FOTA_WORKING_base =  #Sysram Base
    (
         'MT6260'  => '0x00050000',
		 'MT6261_Family'  => '0x00050000',
    );
	my $bb = (&sysUtil::is_MT6261_Family($BB))? 'MT6261_Family'  : $BB;
	my $IOT_ROCK_FOTA_WORKING_base = &config_query_hash_bb(\%BBtbl_BL_IOT_ROCK_FOTA_WORKING_base, $bb, __LINE__) ;
	$IOT_ROCK_FOTA_WORKING_base = "$IOT_ROCK_FOTA_WORKING_base EMPTY";
	if (defined $ext_bl_IOT_ROCK_FOTA_support and $ext_bl_IOT_ROCK_FOTA_support eq 'TRUE') 
  {
	    $template .= &format_execution_view('EXT_BL_IOT_ROCK_FOTA_WORKING_BUF',
	                                        $IOT_ROCK_FOTA_WORKING_base,
	                                        '0x380000',
	                                        [""]);
	}
    chomp($template);
...
# For RedBend-FOTA
#****************************************************************************
sub GenEXT_BL_REDBEND_FOTA_WORKING_BUF
{
    my $template = '';
	my $ext_bl_REDBEND_FOTA_support = &$FEATURE_QUERY_FUNCTION_PTR('FUDIFFNET_ENABLE');
    my $ext_bl_IOT_ROCK_FOTA_support = &$FEATURE_QUERY_FUNCTION_PTR('ROCK_FOTA_SUPPORT');
	my %BBtbl_BL_REDBEND_FOTA_WORKING_base =  #Sysram Base
    (
         'MT6260'  => '0x00050000',
		 'MT6261_Family'  => '0x00050000',
    );
	my $bb = (&sysUtil::is_MT6261_Family($BB))? 'MT6261_Family'  : $BB;
	my $REDBEND_FOTA_WORKING_base = &config_query_hash_bb(\%BBtbl_BL_REDBEND_FOTA_WORKING_base, $bb, __LINE__) ;
	$REDBEND_FOTA_WORKING_base = "$REDBEND_FOTA_WORKING_base EMPTY";
	# Only generate when "project".mak FUDIFFNET_ENABLE = DIFF_IMAGE_UPDATE
	if (defined $ext_bl_REDBEND_FOTA_support and $ext_bl_REDBEND_FOTA_support eq 'DIFF_IMAGE_UPDATE' and $ext_bl_IOT_ROCK_FOTA_support ne 'TRUE') # Only generate when "project".mak FUDIFFNET_ENABLE = DIFF_IMAGE_UPDATE
  {
	    $template .= &format_execution_view('EXT_BL_REDBEND_FOTA_WORKING_BUF',
	                                        $REDBEND_FOTA_WORKING_base,
	                                        '0x380000',
	                                        [""]);
	}
    chomp($template);
...
sub GenEXT_BL_IOT_FOTA_WORKING_BUF
{
	...
	my $ext_bl_ADUPS_FOTA_support = &$FEATURE_QUERY_FUNCTION_PTR('ADUPS_FOTA_DIFF_ENABLE');
	...
	if (defined $ext_bl_IOT_FOTA_support and $ext_bl_IOT_FOTA_support eq 'FULL_IMAGE_UPDATE' and defined $ext_bl_ADUPS_FOTA_support and $ext_bl_ADUPS_FOTA_support eq 'TRUE')  # Only generate when "project".mak FUNET_ENABLE = FULL_IMAGE_UPDATE
	{
	...
	elsif(defined $ext_bl_IOT_FOTA_support and $ext_bl_IOT_FOTA_support eq 'FULL_IMAGE_UPDATE')
	{
	    $template .= &format_execution_view('EXT_BL_IOT_FOTA_WORKING_BUF',
	                                        $IOT_FOTA_WORKING_base,
	                                        '0xC800',
	...
	elsif (defined $ext_bl_ADUPS_FOTA_support and $ext_bl_ADUPS_FOTA_support eq 'TRUE') 
	{
	    $template .= &format_execution_view('EXT_BL_ADUPS_FOTA_WORKING_BUF',
	                                        $IOT_FOTA_WORKING_base,
	                                        '0x380000',
	                                        [""]);
	}
...
sub GenEXT_BL_IOT_FOTA_IMAGE_BUF
{
	...
	my $IOT_IMAGE_BUF_SIZE =  sprintf("0x%x",$IOT_IMAGE_BUFFER_SIZE );
if(defined $ext_bl_ADUPS_FOTA_support and $ext_bl_ADUPS_FOTA_support eq 'TRUE')
{
	$IOT_IMAGE_BUF_SIZE =  sprintf("0x%x",1024 );
	$IOT_FOTA_IMAGE_base ="+0x0 EMPTY";
}
	# Only generate when "project".mak FUNET_ENABLE = FULL_IMAGE_UPDATE
...
sub GenEXT_BL_IOT_FOTA_DECOMPRESSION_BUF
{
	...
	# $IOT_IMAGE_BUF_SIZE should smaler than $IOT_DECOMPRESSION_BUF_SIZE
	# $IOT_DECOMPRESSION_BUF_SIZE + $IOT_IMAGE_BUF_SIZE should smaller than $IOT_TOTAL_BUFFER_SIZE
	my $IOT_DECOMPRESSION_BUF_SIZE = sprintf("0x%x",($IOT_TOTAL_BUFFER_SIZE - $IOT_IMAGE_BUFFER_SIZE-32) );
if(defined $ext_bl_ADUPS_FOTA_support and $ext_bl_ADUPS_FOTA_support eq 'TRUE')
{
	$IOT_DECOMPRESSION_BUF_SIZE = sprintf("0x%x",1024 );
}
	# Only generate when "project".mak FUNET_ENABLE = FULL_IMAGE_UPDATE
	if (defined $ext_bl_IOT_FOTA_support and $ext_bl_IOT_FOTA_support eq 'FULL_IMAGE_UPDATE')  
	{
...
sub GetEXT_UN_INITBase
{
    ...
    elsif (defined $ext_bl_REDBEND_FOTA_support and $ext_bl_REDBEND_FOTA_support eq 'DIFF_IMAGE_UPDATE')
	{
		$ext_un_init_base = "0x00020000"
	}
    elsif (defined $ext_bl_IOT_ROCK_FOTA_support and $ext_bl_IOT_ROCK_FOTA_support eq 'TRUE') 
	{
		$ext_un_init_base = "0x00020000"
	}
    elsif($BB eq 'MT6260' or $BB eq 'MT6261' or $BB eq 'MT2501' or $BB eq 'MT2502') #or $BB eq 'MT2501'or $BB eq 'MT2502' 
	{
		$ext_un_init_base = "0x00030000"
	}	
	else
    {
